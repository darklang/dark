      ...
      let newm, newcmd =
        let handleRPC params focus =
         ...
         possible *)
          ...
          if hasNonHandlers then (m, RPC.rpc (contextFromModel m) focus params)
            ...
            , Cmd.batch
                [ wfCmd
                ; RPC.rpc (contextFromModel withFocus) FocusNoChange params ]
            )
        in
        match mod_ with
        ...
        | DisplayAndReportHttpError (context, e) ->
            ...
            let cmds =
              if shouldRollbar then
                [Tea.Cmd.call (fun _ -> Rollbar.send msg url custom)]
              else []
            in
        ...
        | EndIntegrationTest ->
            let expectationFn =
              ...
              | IntegrationTestFinished _ ->
                  impossible
                    "Attempted to end integration test but one ran + was \
                     already finished"
              | NoIntegrationTest ->
                  impossible
        ...
        | SetToplevels (tls, updateCurrent) ->
           ...
           API call *)
            ...
            let m4 =
              { m3 with
                deletedToplevels= TL.removeByTLID m3.deletedToplevels tls }
            in
        ...
        | UpdateAnalysis (id, analysis) ->
            let m2 =
              {m with analyses= Analysis.record m.analyses id analysis}
            in
            processAutocompleteMods m2 [ACRegenerate]
        | UpdateTraces traces ->
            let newTraces =
        ...
        | UpdateTraceFunctionResult
            (tlid, traceID, callerID, fnName, hash, dval) ->
            let m2 =
              Analysis.replaceFunctionResult m tlid traceID callerID fnName
                hash dval
            in
            processAutocompleteMods m2 [ACRegenerate]
...
let update_ (msg : msg) (m : model) : modification =
  ...
  | RPCCallback
      ...
      if focus = FocusNoChange then
        Many
          ...
          ; UpdateDeletedToplevels newDeletedToplevels; UpdateTraces newTraces
          ; SetUserFunctions (userFuncs, false)
          ; SetUnlockedDBs unlockedDBs; RequestAnalysis newToplevels
          ; MakeCmd (Entry.focusEntry m) ]
      else
  ...
  | InitialLoadRPCCallback
    ...
    ->
      ...
      Many
        ...
        ; SetDeletedToplevels deletedToplevels; UpdateTraces newTraces
        ; SetUserFunctions (userFuncs, true)
        ; SetUnlockedDBs unlockedDBs; RequestAnalysis toplevels
        ; AutocompleteMod ACReset; ClearError; extraMod; newState ]
  ...
  | GetAnalysisRPCCallback (tlids, Ok (newTraces, f404s, unlockedDBs)) ->
      ...
      in
      Many
        [ TweakModel Sync.markResponseInModel; UpdateTraces newTraces
        ; Set404s f404s; SetUnlockedDBs unlockedDBs
...
let update (m : model) (msg : msg) : model * msg Cmd.t =
  ...
  Dom.Storage.setItem
    ...
    state Dom.Storage.localStorage ;
...
let subscriptions (m : model) : msg Tea.Sub.t =
  ...
  let timers =
    if m.timersEnabled then
      ...
      | Hidden -> []
      | Visible ->
          [ Patched_tea_time.every ~key:"refresh_analysis" Tea.Time.second
              (fun f -> TimerFire (RefreshAnalysis, f) ) ] )
      @ [ Patched_tea_time.every ~key:"check_url_hash_position" Tea.Time.second
  ...
  let visibility =
    [ Window.OnFocusChange.listen ~key:"window_on_focus_change" (fun v ->
          if v then PageVisibilityChange Visible
  ...
  Tea.Sub.batch
    (List.concat
       [ keySubs; dragSubs; resizes; timers; visibility; onError; mousewheelSubs
       ; analysisSubs ])

let debugging =
  let prog =
    Tea.Debug.debug show_msg
