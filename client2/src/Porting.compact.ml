...
module PageVisibility = struct
  type visibility = Hidden | Visible [@@deriving show]
end
...
module List = struct
  ...
  let uniqueBy (f : 'a -> string) (l : 'a list) : 'a list =
    let rec uniqueHelp (f : 'a -> string) (existing : Belt.Set.String.t)
        (remaining : 'a list) (accumulator : 'a list) =
      match remaining with
      | [] -> List.rev accumulator
      | first :: rest ->
          let computedFirst = f first in
          if Belt.Set.String.has existing computedFirst then
            uniqueHelp f existing rest accumulator
          else
            uniqueHelp f
              (Belt.Set.String.add existing computedFirst)
              rest (first :: accumulator)
    in
...
module Result = struct
  ...
  let pp (_ : Format.formatter -> 'err -> unit)
      (_ : Format.formatter -> 'ok -> unit) (_ : Format.formatter)
      (_ : ('err, 'ok) t) =
    ()
end
...
module Tuple2 = struct let create a b = (a, b) end
...
module IntDict = struct
  ...
  let fromStrDict ~(default : key) (d : 'value StrDict.t) : 'value t =
    ...
    |> Belt.List.toArray |> Map.fromArray
end

module Html = struct include Tea.Html

                     type 'a html = 'a Vdom.t end

module Native = struct
  type size = {width: int; height: int}
  type rect = {id: string; top: int; left: int; right: int; bottom: int}
  type list_pos = {atoms: rect list; nested: rect list}
  ...
  module Random = struct let random () : int = Js_math.random_int 0 2147483647
  end

  module Size = struct
    let _convert (key : string) (pos : jsRectArr) : rect list =
      Js.Dict.unsafeGet pos key |> List.fromArray
      |> List.map (fun jsRect ->
