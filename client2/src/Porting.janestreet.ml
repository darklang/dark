...
let registerGlobal name key tagger decoder =
  ...
  let enableCall callbacks_base =
    ...
    let fn ev =
      ...
      match decodeEvent decoder ev with Error _ -> None | Ok pos -> Some (tagger pos)
    in
...
module List = struct
  ...
  let elemIndex (a : 'a) (l : 'a list) : int option =
    l |> Array.of_list |> Js.Array.findIndex (( = ) a) |> toOption ~sentinel:(-1)
  ;;
  ...
  let filterMap (fn : 'a -> 'b option) (l : 'a list) : 'b list = Belt.List.keepMap l fn
  ...
  let partition (fn : 'a -> bool) (l : 'a list) : 'a list * 'a list = List.partition fn l
  ...
  let rec findIndexHelp (index : int) (predicate : 'a -> bool) (list : 'a list) :
    ...
    | [] -> None
    | x :: xs ->
      if predicate x then Some index else findIndexHelp (index + 1) predicate xs
  ;;

  let findIndex (fn : 'a -> bool) (l : 'a list) : int option = findIndexHelp 0 fn l
  ...
  let takeWhile (predicate : 'a -> bool) (l : 'a list) : 'a list =
    let rec takeWhileMemo memo list =
      match list with
      | [] -> List.rev memo
      | x :: xs -> if predicate x then takeWhileMemo (x :: memo) xs else List.rev memo
    in
    takeWhileMemo [] l
  ;;
  ...
  let splitAt (n : int) (xs : 'a list) : 'a list * 'a list = take n xs, drop n xs

  let insertAt (n : int) (xs : 'a list) (newVal : 'a) : 'a list =
    take n xs @ (newVal :: drop n xs)
  ;;

  let splitWhen (predicate : 'a -> bool) (list : 'a list) : ('a list * 'a list) option =
  ...
  let sortWith (fn : 'a -> 'a -> int) (l : 'a list) : 'a list = Belt.List.sort l fn
...
module Option = struct
  ...
  let withDefault (a : 'a) (o : 'a option) : 'a = Belt.Option.getWithDefault o a
  ...
  let toList (o : 'a option) : 'a list = match o with None -> [] | Some o -> [o]
...
module String = struct
  ...
  let uncons (s : string) : (char * string) option =
    match s with "" -> None | s -> Some (s.[0], String.sub s 1 (String.length s - 1))
  ;;
  ...
  let endsWith (needle : string) (haystack : string) = Js.String.endsWith needle haystack
  ...
  let fromList (l : char list) : string =
    l |> List.map Char.toCode |> List.map Js.String.fromCharCode |> String.concat ""
  ;;
...
module StrDict = struct
  ...
  let insert (k : key) (v : 'value) (map : 'value t) : 'value t = Map.set map k v
...
module IntDict = struct
  ...
  let fromStrDict ~(default : key) (d : 'value StrDict.t) : 'value t =
    d
    |> StrDict.toList
    |> List.map (fun (k, v) -> k |> String.toInt |> Result.withDefault default, v)
    |> Belt.List.toArray
    |> Map.fromArray
  ;;
end
...
module Window = struct
  module OnResize = struct
    let decode =
      ...
      let decodeDetail =
        map2 (fun width height -> width, height) (field "width" int) (field "height" int)
      in
