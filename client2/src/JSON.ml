...
let encodeSpec (spec : handlerSpec) : JSE.value =
  JSE.object_
    ...
    ; ( "types"
      , JSE.object_
          [ ("input", encodeBlankOr JSE.int (Blank.new_ ()))
          ; ("output", encodeBlankOr JSE.int (Blank.new_ ())) ] ) ]
...
let encodeUserFunctionMetadata (f : userFunctionMetadata) : JSE.value =
  JSE.object_
    ...
    ; ( "parameters"
      , JSE.list (List.map encodeUserFunctionParameter f.ufmParameters) )
...
let decodeFunctionResult : functionResult JSD.decoder =
  ...
  JSD.map toFunctionResult
    (JSONUtils.decodeQuadriple JSD.string decodeID JSD.string decodeDval)
    ...
    decodeVariants
      ...
      ; ( "Response"
        , dv2 Response JSD.int
            (JSD.list (JSONUtils.decodePair JSD.string JSD.string)) ) ]
  in
  decodeVariants
    ...
    ; ( "DResp"
      , dv1
          (fun (h, dv) -> DResp (h, dv))
          (JSONUtils.decodePair decodeDhttp dd) )
