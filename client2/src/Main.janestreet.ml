...
let init (flagString : string) (location : Web.Location.location) =
  ...
  in
  let shouldRunIntegrationTest = String.endsWith "/integration_test" location.pathname in
  ...
  then m2, Cmd.batch [RPC.integrationRPC (contextFromModel m2) integrationTestName]
  else
    ( m2
    , Cmd.batch
        [ RPC.initialLoadRPC
...
let processFocus (m : model) (focus : focus) : modification =
  ...
  | FocusSame ->
    ...
    | Selecting (tlid, mId) ->
      ...
      | Some tl, Some id -> if TL.isValidID tl id then NoChange else Select (tlid, None)
  ...
  | FocusPageAndCursor (page, cs) ->
    ...
    let tlOnPage tl =
      match page with
      | Toplevels _ ->
        (match tl.data with TLHandler _ -> true | TLDB _ -> true | TLFunc _ -> false)
      | Fn (id, _) -> tl.id = id
    in
    let nextCursor, acTarget =
      ...
      | Dragging (tlid, _, _, _) ->
        (match TL.get m tlid with
        | Some tl -> if tlOnPage tl then setCS, noTarget else Deselect, noTarget
        | _ -> Deselect, noTarget)
      | _ -> Deselect, noTarget
    in
...
let processAutocompleteMods (m : model) (mods : autocompleteMod list) : model * msg Cmd.t
    =
  if m.integrationTestState <> NoIntegrationTest
  then Debug.loG "autocompletemod update" (show_list show_autocompleteMod mods);
  let complete =
    List.foldl (fun mod_ complete_ -> AC.update m mod_ complete_) m.complete mods
  in
  let focus =
    match unwrapCursorState m.cursorState with
  ...
  | _ ->
    let closeBlanks newM =
      ...
      |> Option.map (fun tl ->
             ...
             | _ -> [] )
      |> Option.withDefault []
      |> fun rpc -> if tlidOf newM.cursorState = tlidOf m.cursorState then [] else rpc
    in
    let newm, newcmd =
      let handleRPC params focus =
        ...
        then m, RPC.rpc (contextFromModel m) focus params
          ...
          in
          ( withFocus
          , Cmd.batch [wfCmd; RPC.rpc (contextFromModel withFocus) FocusNoChange params]
          )
      in
      match mod_ with
      ...
      | ClearError -> {m with error = {message = None; showDetails = false}}, Cmd.none
      ...
      | EndIntegrationTest ->
        ...
        {m with integrationTestState = IntegrationTestFinished result}, Cmd.none
      | MakeCmd cmd -> m, cmd
      ...
      | SelectCommand (tlid, id) ->
        ...
        let m3, acCmd = processAutocompleteMods m2 [ACEnableCommandMode; ACRegenerate] in
        m3, Cmd.batch (closeBlanks m3 @ [acCmd; Entry.focusEntry m3])
      ...
      | SetToplevels (tls, updateCurrent) ->
        ...
        let m3 =
          ...
          | None -> m2
        in
        let m4 = {m3 with deletedToplevels = TL.removeByTLID m3.deletedToplevels tls} in
      ...
      | SetDeletedToplevels dtls ->
        let m2 =
          {m with deletedToplevels = dtls; toplevels = TL.removeByTLID m.toplevels dtls}
        in
        processAutocompleteMods m2 [ACRegenerate]
      | RequestAnalysis tls ->
        ...
        let req h =
          ...
          let param t =
            ...
            |> Js.Json.stringify
          in
          trace |> Option.map (fun t -> requestAnalysis (param t)) |> Option.toList
        in
      ...
      | Drag (tlid, offset, hasMoved, state) ->
        {m with cursorState = Dragging (tlid, offset, hasMoved, state)}, Cmd.none
      ...
      | ExecutingFunctionComplete targets ->
        ...
        let nexecutingFunctions = List.filter isComplete m.executingFunctions in
        {m with executingFunctions = nexecutingFunctions}, Cmd.none
...
let toggleTimers (m : model) : model = {m with timersEnabled = not m.timersEnabled}
...
let update_ (msg : msg) (m : model) : modification =
  ...
  | GlobalKeyPress event ->
    ...
    then
      ...
      | Selecting (tlid, mId) ->
              ...
              | Some id ->
                ...
                | PExpr _ ->
                  ...
                  then NoChange
                    ...
                    RPC
                      ( [SetHandler (tl.id, tl.pos, {h with ast = replacement})]
            ...
            | TLFunc f ->
              ...
              | Some id ->
                ...
                | PParamTipe _ ->
                  ...
                  RPC ([SetFunction replacement] @ newCalls, FocusNext (tlid, Some id))
                | PParamName _ ->
                  let replacement = Functions.extend f in
                  let newCalls = Refactor.addNewFunctionParameter m f in
                  RPC ([SetFunction replacement] @ newCalls, FocusNext (tlid, Some id))
        ...
        (*   else NoChange *)
        | Key.N ->
          if event.ctrlKey then Selection.selectNextSibling m tlid mId else NoChange
        | Key.P ->
          if event.ctrlKey then Selection.selectPreviousSibling m tlid mId else NoChange
        | Key.C ->
        ...
        | Key.Unknown _ ->
          ...
          | Some id ->
            if event.key = Some ":"
            then Many [SelectCommand (tlid, id); AutocompleteMod (ACSetQuery ":")]
            else NoChange)
        | _ -> NoChange)
      | Entering cursor ->
        if event.ctrlKey
        then
          ...
          | Key.Enter ->
            ...
            then Many [AutocompleteMod (ACAppendQuery "\n"); MakeCmd (Entry.focusEntry m)]
        ...
        then
          ...
          | Key.Enter ->
            ...
            then AutocompleteMod (ACSetQuery m.complete.value)
              ...
              | Creating pos ->
                ...
                | Some (ACOmniAction action) -> Entry.submitOmniAction pos action
          ...
          | Key.Tab ->
            ...
            | Filling (tlid, p) ->
              ...
              then
                ...
                | Some idx ->
                  let newQ = String.insertAt "\t" (idx + 1) m.complete.value in
          ...
          | Key.Up ->
            ...
            | false, _ -> AutocompleteMod ACSelectUp
            (* NB: see `stopKeys`
                                                             in ui.html *)
            | true, Creating _ -> NoChange
            | true, Filling (tlid, mId) ->
              Selection.moveUp ~andEnter:true m tlid (Some mId))
          | Key.Down ->
            (match VariantTesting.variantIsActive m ArrowMove, cursor with
            | false, _ -> AutocompleteMod ACSelectDown
            (* NB: see `stopKeys`
                                                               in ui.html *)
            | true, Creating _ -> NoChange
            | true, Filling (tlid, mId) ->
              Selection.moveDown ~andEnter:true m tlid (Some mId))
          | Key.Right ->
            (match VariantTesting.variantIsActive m ArrowMove, cursor with
            | false, _ -> AC.selectSharedPrefix m.complete
            | true, Creating _ -> NoChange
            | true, Filling (tlid, mId) ->
              Selection.moveRight ~andEnter:true m tlid (Some mId))
          | Key.Left ->
            (match VariantTesting.variantIsActive m ArrowMove, cursor with
            | false, _ -> NoChange
            | true, Creating _ -> NoChange
            | true, Filling (tlid, mId) ->
              Selection.moveLeft ~andEnter:true m tlid (Some mId))
          | Key.Backspace ->
            ...
            (* string. *)
            let v =
              if m.complete.value = "\"\"" && String.length m.complete.prevValue <= 2
              then ""
              else m.complete.value
            in
            Many [AutocompleteMod (ACSetQuery v); MakeCmd (Entry.focusEntry m)]
          | _ -> NoChange )
      | Deselected ->
        (match m.currentPage with
        | Fn (_, _) -> NoChange
        | Toplevels _ ->
          ...
          | Key.Tab ->
            Selection.selectNextToplevel m None (* NB: see `stopKeys` in ui.html *)
          | _ -> NoChange))
      | SelectingCommand (tlid, id) ->
        (match event.keyCode with
        | Key.Escape -> Commands.endCommandExecution tlid id
        | Key.Enter -> Commands.executeCommand m tlid id (AC.highlighted m.complete)
        | Key.P -> if event.ctrlKey then AutocompleteMod ACSelectUp else NoChange
        | Key.N -> if event.ctrlKey then AutocompleteMod ACSelectDown else NoChange
  ...
  | EntryInputMsg target ->
    ...
    then NoChange
    else Many [AutocompleteMod (ACSetQuery query); MakeCmd (Entry.focusEntry m)]
  ...
  | BlankOrClick (targetTLID, targetID, _) ->
    ...
    | Entering cursor ->
      (match cursor with
      | Filling (_, fillingID) ->
        if fillingID = targetID then NoChange else Select (targetTLID, Some targetID)
      | _ -> Select (targetTLID, Some targetID))
    | Selecting (_, maybeSelectingID) ->
      (match maybeSelectingID with
      | Some selectingID ->
        if selectingID = targetID then NoChange else Select (targetTLID, Some targetID)
      | None -> Select (targetTLID, Some targetID))
    | SelectingCommand (_, selectingID) ->
      if selectingID = targetID then NoChange else Select (targetTLID, Some targetID))
  | BlankOrDoubleClick (targetTLID, targetID, _) -> Selection.enter m targetTLID targetID
  ...
  | ExecuteFunctionButton (tlid, id, name) ->
    Many [ExecutingFunctionBegan (tlid, id); ExecutingFunctionRPC (tlid, id, name)]
  ...
  | StartMigration tlid ->
    ...
    (match mdb with Some db -> DB.startMigration tlid db.cols | None -> NoChange)
  ...
  | RPCCallback
      ( focus
      , _
      , Ok (newToplevels, newDeletedToplevels, newTraces, userFuncs, unlockedDBs) ) ->
  ...
  | GetAnalysisRPCCallback (tlids, Ok (newTraces, f404s, unlockedDBs)) ->
    let analysisTLs = List.filter (fun tl -> List.member tl.id tlids) m.toplevels in
    Many
  ...
  | CreateHandlerFrom404 {space; path; modifier} ->
    ...
    let aHandler =
      { ast = B.new_ ()
      ; spec = {module_ = B.newF space; name = B.newF path; modifier = B.newF modifier}
      ; tlid = anId }
    in
...
let subscriptions (m : model) : msg Tea.Sub.t =
  ...
  let resizes =
    [Window.OnResize.listen ~key:"window_on_resize" (fun (w, h) -> WindowResize (w, h))]
  in
  let dragSubs =
    match m.cursorState with
  ...
  let timers =
    ...
    then
      ...
      | Hidden -> []
      | Visible ->
        [ Patched_tea_time.every ~key:"refresh_analysis" Tea.Time.second (fun f ->
              TimerFire (RefreshAnalysis, f) ) ])
      @ [ Patched_tea_time.every ~key:"check_url_hash_position" Tea.Time.second (fun f ->
              TimerFire (CheckUrlHashPosition, f) ) ]
    else []
  in
  let onError =
  ...
  let visibility =
    [ Window.OnFocusChange.listen ~key:"window_on_focus_change" (fun v ->
          if v then PageVisibilityChange Visible else PageVisibilityChange Hidden ) ]
  in
  let mousewheelSubs =
    [OnWheel.listen ~key:"on_wheel" (fun (dx, dy) -> MouseWheel (dx, dy))]
  in
  let analysisSubs =
    [Analysis.ReceiveAnalysis.listen ~key:"receive_analysis" (fun s -> ReceiveAnalysis s)]
  in
  Tea.Sub.batch
    (List.concat
       [ keySubs
...
let normal =
  let program : (string, model, msg) Tea.Navigation.navigationProgram =
    {init; view = View.view; update; subscriptions; shutdown = (fun _ -> Cmd.none)}
  in
