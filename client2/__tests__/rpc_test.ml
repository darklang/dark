...
let testRoundtrip decoder encoder
  ...
  test
    ("roundtrip " ^ name)
    (fun () -> Expect.expect (v) |> Expect.toEqual(v |> encoder |> decoder))
...
let () =
  describe "compatible with server JSON encoding" (fun () ->
    test "obj uses list" (fun () ->
      ...
      |> Expect.toEqual
        ((DObj (StrDict.fromList [("foo", DInt 5)])) |> Encoders.dval |> Js.Json.stringify));
    test "dresp shape" (fun () ->
      Expect.expect
        ("[\"DResp\",[[\"Response\",401,[]],[\"DNull\"]]]")
      |> Expect.toEqual
        (DResp (Response (401, []), DNull) |> Encoders.dval |> Js.Json.stringify));
