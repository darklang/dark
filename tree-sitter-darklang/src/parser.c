#include <tree_sitter/parser.h>

#if defined(__GNUC__) || defined(__clang__)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wmissing-field-initializers"
#endif

#define LANGUAGE_VERSION 14
#define STATE_COUNT 341
#define LARGE_STATE_COUNT 2
#define SYMBOL_COUNT 117
#define ALIAS_COUNT 0
#define TOKEN_COUNT 68
#define EXTERNAL_TOKEN_COUNT 0
#define FIELD_COUNT 38
#define MAX_ALIAS_SEQUENCE_LENGTH 7
#define PRODUCTION_ID_COUNT 28

enum
{
  anon_sym_let = 1,
  anon_sym_COLON = 2,
  anon_sym_EQ = 3,
  anon_sym_LPAREN = 4,
  anon_sym_RPAREN = 5,
  anon_sym_type = 6,
  aux_sym_bool_literal_token1 = 7,
  aux_sym_bool_literal_token2 = 8,
  anon_sym_LF = 9,
  anon_sym_DQUOTE = 10,
  aux_sym_string_content_token1 = 11,
  anon_sym_SQUOTE = 12,
  aux_sym_character_token1 = 13,
  sym_char_or_string_escape_sequence = 14,
  anon_sym_CARET = 15,
  anon_sym_STAR = 16,
  anon_sym_SLASH = 17,
  anon_sym_PERCENT = 18,
  anon_sym_PLUS = 19,
  anon_sym_DASH = 20,
  anon_sym_EQ_EQ = 21,
  anon_sym_BANG_EQ = 22,
  anon_sym_LT = 23,
  anon_sym_LT_EQ = 24,
  anon_sym_GT = 25,
  anon_sym_GT_EQ = 26,
  anon_sym_AMP_AMP = 27,
  anon_sym_PIPE_PIPE = 28,
  anon_sym_y = 29,
  anon_sym_uy = 30,
  anon_sym_s = 31,
  anon_sym_us = 32,
  anon_sym_l = 33,
  anon_sym_ul = 34,
  anon_sym_L = 35,
  anon_sym_UL = 36,
  anon_sym_Q = 37,
  anon_sym_Z = 38,
  sym_negative_digits = 39,
  sym_positive_digits = 40,
  sym_float_literal = 41,
  anon_sym_LBRACK = 42,
  anon_sym_RBRACK = 43,
  anon_sym_SEMI = 44,
  anon_sym_COMMA = 45,
  aux_sym_builtin_type_token1 = 46,
  aux_sym_builtin_type_token2 = 47,
  aux_sym_builtin_type_token3 = 48,
  aux_sym_builtin_type_token4 = 49,
  aux_sym_builtin_type_token5 = 50,
  aux_sym_builtin_type_token6 = 51,
  aux_sym_builtin_type_token7 = 52,
  aux_sym_builtin_type_token8 = 53,
  aux_sym_builtin_type_token9 = 54,
  aux_sym_builtin_type_token10 = 55,
  aux_sym_builtin_type_token11 = 56,
  aux_sym_builtin_type_token12 = 57,
  aux_sym_builtin_type_token13 = 58,
  aux_sym_builtin_type_token14 = 59,
  aux_sym_builtin_type_token15 = 60,
  aux_sym_builtin_type_token16 = 61,
  aux_sym_builtin_type_token17 = 62,
  anon_sym_List = 63,
  anon_sym_DOT = 64,
  aux_sym_variable_identifier_token1 = 65,
  aux_sym_type_identifier_token1 = 66,
  sym_unit = 67,
  sym_source_file = 68,
  sym_fn_decl = 69,
  sym_fn_decl_params = 70,
  sym_fn_decl_param = 71,
  sym_type_decl = 72,
  sym_expression = 73,
  sym_paren_expression = 74,
  sym_bool_literal = 75,
  sym_function_call = 76,
  sym_let_expression = 77,
  sym_string_literal = 78,
  sym_string_content = 79,
  sym_char_literal = 80,
  sym_character = 81,
  sym_infix_operation = 82,
  sym_int8_literal = 83,
  sym_uint8_literal = 84,
  sym_int16_literal = 85,
  sym_uint16_literal = 86,
  sym_int32_literal = 87,
  sym_uint32_literal = 88,
  sym_int64_literal = 89,
  sym_uint64_literal = 90,
  sym_int128_literal = 91,
  sym_uint128_literal = 92,
  sym_digits = 93,
  sym_list_literal = 94,
  sym_list_content = 95,
  sym_tuple_literal = 96,
  sym_tuple_literal_the_rest = 97,
  sym_type_reference = 98,
  sym_builtin_type = 99,
  sym_list_type_reference = 100,
  sym_tuple_type_reference = 101,
  sym_type_reference_tuple_the_rest = 102,
  sym_qualified_fn_name = 103,
  sym_qualified_type_name = 104,
  sym_variable_identifier = 105,
  sym_fn_identifier = 106,
  sym_type_identifier = 107,
  sym_module_identifier = 108,
  aux_sym_source_file_repeat1 = 109,
  aux_sym_source_file_repeat2 = 110,
  aux_sym_fn_decl_params_repeat1 = 111,
  aux_sym_string_content_repeat1 = 112,
  aux_sym_list_content_repeat1 = 113,
  aux_sym_tuple_literal_the_rest_repeat1 = 114,
  aux_sym_type_reference_tuple_the_rest_repeat1 = 115,
  aux_sym_qualified_fn_name_repeat1 = 116,
};

static const char *const ts_symbol_names[] = {
    [ts_builtin_sym_end] = "end",
    [anon_sym_let] = "keyword",
    [anon_sym_COLON] = "symbol",
    [anon_sym_EQ] = "symbol",
    [anon_sym_LPAREN] = "symbol",
    [anon_sym_RPAREN] = "symbol",
    [anon_sym_type] = "keyword",
    [aux_sym_bool_literal_token1] = "bool_literal_token1",
    [aux_sym_bool_literal_token2] = "bool_literal_token2",
    [anon_sym_LF] = "\n",
    [anon_sym_DQUOTE] = "symbol",
    [aux_sym_string_content_token1] = "string_content_token1",
    [anon_sym_SQUOTE] = "symbol",
    [aux_sym_character_token1] = "character_token1",
    [sym_char_or_string_escape_sequence] = "char_or_string_escape_sequence",
    [anon_sym_CARET] = "operator",
    [anon_sym_STAR] = "symbol",
    [anon_sym_SLASH] = "operator",
    [anon_sym_PERCENT] = "operator",
    [anon_sym_PLUS] = "operator",
    [anon_sym_DASH] = "operator",
    [anon_sym_EQ_EQ] = "operator",
    [anon_sym_BANG_EQ] = "operator",
    [anon_sym_LT] = "operator",
    [anon_sym_LT_EQ] = "operator",
    [anon_sym_GT] = "operator",
    [anon_sym_GT_EQ] = "operator",
    [anon_sym_AMP_AMP] = "operator",
    [anon_sym_PIPE_PIPE] = "operator",
    [anon_sym_y] = "symbol",
    [anon_sym_uy] = "symbol",
    [anon_sym_s] = "symbol",
    [anon_sym_us] = "symbol",
    [anon_sym_l] = "symbol",
    [anon_sym_ul] = "symbol",
    [anon_sym_L] = "symbol",
    [anon_sym_UL] = "symbol",
    [anon_sym_Q] = "symbol",
    [anon_sym_Z] = "symbol",
    [sym_negative_digits] = "negative_digits",
    [sym_positive_digits] = "positive_digits",
    [sym_float_literal] = "float_literal",
    [anon_sym_LBRACK] = "symbol",
    [anon_sym_RBRACK] = "symbol",
    [anon_sym_SEMI] = "symbol",
    [anon_sym_COMMA] = "symbol",
    [aux_sym_builtin_type_token1] = "builtin_type_token1",
    [aux_sym_builtin_type_token2] = "builtin_type_token2",
    [aux_sym_builtin_type_token3] = "builtin_type_token3",
    [aux_sym_builtin_type_token4] = "builtin_type_token4",
    [aux_sym_builtin_type_token5] = "builtin_type_token5",
    [aux_sym_builtin_type_token6] = "builtin_type_token6",
    [aux_sym_builtin_type_token7] = "builtin_type_token7",
    [aux_sym_builtin_type_token8] = "builtin_type_token8",
    [aux_sym_builtin_type_token9] = "builtin_type_token9",
    [aux_sym_builtin_type_token10] = "builtin_type_token10",
    [aux_sym_builtin_type_token11] = "builtin_type_token11",
    [aux_sym_builtin_type_token12] = "builtin_type_token12",
    [aux_sym_builtin_type_token13] = "builtin_type_token13",
    [aux_sym_builtin_type_token14] = "builtin_type_token14",
    [aux_sym_builtin_type_token15] = "builtin_type_token15",
    [aux_sym_builtin_type_token16] = "builtin_type_token16",
    [aux_sym_builtin_type_token17] = "builtin_type_token17",
    [anon_sym_List] = "keyword",
    [anon_sym_DOT] = "symbol",
    [aux_sym_variable_identifier_token1] = "variable_identifier_token1",
    [aux_sym_type_identifier_token1] = "type_identifier_token1",
    [sym_unit] = "unit",
    [sym_source_file] = "source_file",
    [sym_fn_decl] = "fn_decl",
    [sym_fn_decl_params] = "fn_decl_params",
    [sym_fn_decl_param] = "fn_decl_param",
    [sym_type_decl] = "type_decl",
    [sym_expression] = "expression",
    [sym_paren_expression] = "paren_expression",
    [sym_bool_literal] = "bool_literal",
    [sym_function_call] = "function_call",
    [sym_let_expression] = "let_expression",
    [sym_string_literal] = "string_literal",
    [sym_string_content] = "string_content",
    [sym_char_literal] = "char_literal",
    [sym_character] = "character",
    [sym_infix_operation] = "infix_operation",
    [sym_int8_literal] = "int8_literal",
    [sym_uint8_literal] = "uint8_literal",
    [sym_int16_literal] = "int16_literal",
    [sym_uint16_literal] = "uint16_literal",
    [sym_int32_literal] = "int32_literal",
    [sym_uint32_literal] = "uint32_literal",
    [sym_int64_literal] = "int64_literal",
    [sym_uint64_literal] = "uint64_literal",
    [sym_int128_literal] = "int128_literal",
    [sym_uint128_literal] = "uint128_literal",
    [sym_digits] = "digits",
    [sym_list_literal] = "list_literal",
    [sym_list_content] = "list_content",
    [sym_tuple_literal] = "tuple_literal",
    [sym_tuple_literal_the_rest] = "tuple_literal_the_rest",
    [sym_type_reference] = "type_reference",
    [sym_builtin_type] = "builtin_type",
    [sym_list_type_reference] = "list_type_reference",
    [sym_tuple_type_reference] = "tuple_type_reference",
    [sym_type_reference_tuple_the_rest] = "type_reference_tuple_the_rest",
    [sym_qualified_fn_name] = "qualified_fn_name",
    [sym_qualified_type_name] = "qualified_type_name",
    [sym_variable_identifier] = "variable_identifier",
    [sym_fn_identifier] = "fn_identifier",
    [sym_type_identifier] = "type_identifier",
    [sym_module_identifier] = "module_identifier",
    [aux_sym_source_file_repeat1] = "source_file_repeat1",
    [aux_sym_source_file_repeat2] = "source_file_repeat2",
    [aux_sym_fn_decl_params_repeat1] = "fn_decl_params_repeat1",
    [aux_sym_string_content_repeat1] = "string_content_repeat1",
    [aux_sym_list_content_repeat1] = "list_content_repeat1",
    [aux_sym_tuple_literal_the_rest_repeat1] = "tuple_literal_the_rest_repeat1",
    [aux_sym_type_reference_tuple_the_rest_repeat1] = "type_reference_tuple_the_rest_repeat1",
    [aux_sym_qualified_fn_name_repeat1] = "qualified_fn_name_repeat1",
};

static const TSSymbol ts_symbol_map[] = {
    [ts_builtin_sym_end] = ts_builtin_sym_end,
    [anon_sym_let] = anon_sym_let,
    [anon_sym_COLON] = anon_sym_COLON,
    [anon_sym_EQ] = anon_sym_COLON,
    [anon_sym_LPAREN] = anon_sym_COLON,
    [anon_sym_RPAREN] = anon_sym_COLON,
    [anon_sym_type] = anon_sym_let,
    [aux_sym_bool_literal_token1] = aux_sym_bool_literal_token1,
    [aux_sym_bool_literal_token2] = aux_sym_bool_literal_token2,
    [anon_sym_LF] = anon_sym_LF,
    [anon_sym_DQUOTE] = anon_sym_COLON,
    [aux_sym_string_content_token1] = aux_sym_string_content_token1,
    [anon_sym_SQUOTE] = anon_sym_COLON,
    [aux_sym_character_token1] = aux_sym_character_token1,
    [sym_char_or_string_escape_sequence] = sym_char_or_string_escape_sequence,
    [anon_sym_CARET] = anon_sym_CARET,
    [anon_sym_STAR] = anon_sym_COLON,
    [anon_sym_SLASH] = anon_sym_CARET,
    [anon_sym_PERCENT] = anon_sym_CARET,
    [anon_sym_PLUS] = anon_sym_CARET,
    [anon_sym_DASH] = anon_sym_CARET,
    [anon_sym_EQ_EQ] = anon_sym_CARET,
    [anon_sym_BANG_EQ] = anon_sym_CARET,
    [anon_sym_LT] = anon_sym_CARET,
    [anon_sym_LT_EQ] = anon_sym_CARET,
    [anon_sym_GT] = anon_sym_CARET,
    [anon_sym_GT_EQ] = anon_sym_CARET,
    [anon_sym_AMP_AMP] = anon_sym_CARET,
    [anon_sym_PIPE_PIPE] = anon_sym_CARET,
    [anon_sym_y] = anon_sym_COLON,
    [anon_sym_uy] = anon_sym_COLON,
    [anon_sym_s] = anon_sym_COLON,
    [anon_sym_us] = anon_sym_COLON,
    [anon_sym_l] = anon_sym_COLON,
    [anon_sym_ul] = anon_sym_COLON,
    [anon_sym_L] = anon_sym_COLON,
    [anon_sym_UL] = anon_sym_COLON,
    [anon_sym_Q] = anon_sym_COLON,
    [anon_sym_Z] = anon_sym_COLON,
    [sym_negative_digits] = sym_negative_digits,
    [sym_positive_digits] = sym_positive_digits,
    [sym_float_literal] = sym_float_literal,
    [anon_sym_LBRACK] = anon_sym_COLON,
    [anon_sym_RBRACK] = anon_sym_COLON,
    [anon_sym_SEMI] = anon_sym_COLON,
    [anon_sym_COMMA] = anon_sym_COLON,
    [aux_sym_builtin_type_token1] = aux_sym_builtin_type_token1,
    [aux_sym_builtin_type_token2] = aux_sym_builtin_type_token2,
    [aux_sym_builtin_type_token3] = aux_sym_builtin_type_token3,
    [aux_sym_builtin_type_token4] = aux_sym_builtin_type_token4,
    [aux_sym_builtin_type_token5] = aux_sym_builtin_type_token5,
    [aux_sym_builtin_type_token6] = aux_sym_builtin_type_token6,
    [aux_sym_builtin_type_token7] = aux_sym_builtin_type_token7,
    [aux_sym_builtin_type_token8] = aux_sym_builtin_type_token8,
    [aux_sym_builtin_type_token9] = aux_sym_builtin_type_token9,
    [aux_sym_builtin_type_token10] = aux_sym_builtin_type_token10,
    [aux_sym_builtin_type_token11] = aux_sym_builtin_type_token11,
    [aux_sym_builtin_type_token12] = aux_sym_builtin_type_token12,
    [aux_sym_builtin_type_token13] = aux_sym_builtin_type_token13,
    [aux_sym_builtin_type_token14] = aux_sym_builtin_type_token14,
    [aux_sym_builtin_type_token15] = aux_sym_builtin_type_token15,
    [aux_sym_builtin_type_token16] = aux_sym_builtin_type_token16,
    [aux_sym_builtin_type_token17] = aux_sym_builtin_type_token17,
    [anon_sym_List] = anon_sym_let,
    [anon_sym_DOT] = anon_sym_COLON,
    [aux_sym_variable_identifier_token1] = aux_sym_variable_identifier_token1,
    [aux_sym_type_identifier_token1] = aux_sym_type_identifier_token1,
    [sym_unit] = sym_unit,
    [sym_source_file] = sym_source_file,
    [sym_fn_decl] = sym_fn_decl,
    [sym_fn_decl_params] = sym_fn_decl_params,
    [sym_fn_decl_param] = sym_fn_decl_param,
    [sym_type_decl] = sym_type_decl,
    [sym_expression] = sym_expression,
    [sym_paren_expression] = sym_paren_expression,
    [sym_bool_literal] = sym_bool_literal,
    [sym_function_call] = sym_function_call,
    [sym_let_expression] = sym_let_expression,
    [sym_string_literal] = sym_string_literal,
    [sym_string_content] = sym_string_content,
    [sym_char_literal] = sym_char_literal,
    [sym_character] = sym_character,
    [sym_infix_operation] = sym_infix_operation,
    [sym_int8_literal] = sym_int8_literal,
    [sym_uint8_literal] = sym_uint8_literal,
    [sym_int16_literal] = sym_int16_literal,
    [sym_uint16_literal] = sym_uint16_literal,
    [sym_int32_literal] = sym_int32_literal,
    [sym_uint32_literal] = sym_uint32_literal,
    [sym_int64_literal] = sym_int64_literal,
    [sym_uint64_literal] = sym_uint64_literal,
    [sym_int128_literal] = sym_int128_literal,
    [sym_uint128_literal] = sym_uint128_literal,
    [sym_digits] = sym_digits,
    [sym_list_literal] = sym_list_literal,
    [sym_list_content] = sym_list_content,
    [sym_tuple_literal] = sym_tuple_literal,
    [sym_tuple_literal_the_rest] = sym_tuple_literal_the_rest,
    [sym_type_reference] = sym_type_reference,
    [sym_builtin_type] = sym_builtin_type,
    [sym_list_type_reference] = sym_list_type_reference,
    [sym_tuple_type_reference] = sym_tuple_type_reference,
    [sym_type_reference_tuple_the_rest] = sym_type_reference_tuple_the_rest,
    [sym_qualified_fn_name] = sym_qualified_fn_name,
    [sym_qualified_type_name] = sym_qualified_type_name,
    [sym_variable_identifier] = sym_variable_identifier,
    [sym_fn_identifier] = sym_fn_identifier,
    [sym_type_identifier] = sym_type_identifier,
    [sym_module_identifier] = sym_module_identifier,
    [aux_sym_source_file_repeat1] = aux_sym_source_file_repeat1,
    [aux_sym_source_file_repeat2] = aux_sym_source_file_repeat2,
    [aux_sym_fn_decl_params_repeat1] = aux_sym_fn_decl_params_repeat1,
    [aux_sym_string_content_repeat1] = aux_sym_string_content_repeat1,
    [aux_sym_list_content_repeat1] = aux_sym_list_content_repeat1,
    [aux_sym_tuple_literal_the_rest_repeat1] = aux_sym_tuple_literal_the_rest_repeat1,
    [aux_sym_type_reference_tuple_the_rest_repeat1] = aux_sym_type_reference_tuple_the_rest_repeat1,
    [aux_sym_qualified_fn_name_repeat1] = aux_sym_qualified_fn_name_repeat1,
};

static const TSSymbolMetadata ts_symbol_metadata[] = {
    [ts_builtin_sym_end] = {
        .visible = false,
        .named = true,
    },
    [anon_sym_let] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_COLON] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_EQ] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_LPAREN] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_RPAREN] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_type] = {
        .visible = true,
        .named = true,
    },
    [aux_sym_bool_literal_token1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_bool_literal_token2] = {
        .visible = false,
        .named = false,
    },
    [anon_sym_LF] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_DQUOTE] = {
        .visible = true,
        .named = true,
    },
    [aux_sym_string_content_token1] = {
        .visible = false,
        .named = false,
    },
    [anon_sym_SQUOTE] = {
        .visible = true,
        .named = true,
    },
    [aux_sym_character_token1] = {
        .visible = false,
        .named = false,
    },
    [sym_char_or_string_escape_sequence] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_CARET] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_STAR] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_SLASH] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_PERCENT] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_PLUS] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_DASH] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_EQ_EQ] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_BANG_EQ] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_LT] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_LT_EQ] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_GT] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_GT_EQ] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_AMP_AMP] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_PIPE_PIPE] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_y] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_uy] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_s] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_us] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_l] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_ul] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_L] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_UL] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_Q] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_Z] = {
        .visible = true,
        .named = true,
    },
    [sym_negative_digits] = {
        .visible = true,
        .named = true,
    },
    [sym_positive_digits] = {
        .visible = true,
        .named = true,
    },
    [sym_float_literal] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_LBRACK] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_RBRACK] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_SEMI] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_COMMA] = {
        .visible = true,
        .named = true,
    },
    [aux_sym_builtin_type_token1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token2] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token3] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token4] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token5] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token6] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token7] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token8] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token9] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token10] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token11] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token12] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token13] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token14] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token15] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token16] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_builtin_type_token17] = {
        .visible = false,
        .named = false,
    },
    [anon_sym_List] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_DOT] = {
        .visible = true,
        .named = true,
    },
    [aux_sym_variable_identifier_token1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_type_identifier_token1] = {
        .visible = false,
        .named = false,
    },
    [sym_unit] = {
        .visible = true,
        .named = true,
    },
    [sym_source_file] = {
        .visible = true,
        .named = true,
    },
    [sym_fn_decl] = {
        .visible = true,
        .named = true,
    },
    [sym_fn_decl_params] = {
        .visible = true,
        .named = true,
    },
    [sym_fn_decl_param] = {
        .visible = true,
        .named = true,
    },
    [sym_type_decl] = {
        .visible = true,
        .named = true,
    },
    [sym_expression] = {
        .visible = true,
        .named = true,
    },
    [sym_paren_expression] = {
        .visible = true,
        .named = true,
    },
    [sym_bool_literal] = {
        .visible = true,
        .named = true,
    },
    [sym_function_call] = {
        .visible = true,
        .named = true,
    },
    [sym_let_expression] = {
        .visible = true,
        .named = true,
    },
    [sym_string_literal] = {
        .visible = true,
        .named = true,
    },
    [sym_string_content] = {
        .visible = true,
        .named = true,
    },
    [sym_char_literal] = {
        .visible = true,
        .named = true,
    },
    [sym_character] = {
        .visible = true,
        .named = true,
    },
    [sym_infix_operation] = {
        .visible = true,
        .named = true,
    },
    [sym_int8_literal] = {
        .visible = true,
        .named = true,
    },
    [sym_uint8_literal] = {
        .visible = true,
        .named = true,
    },
    [sym_int16_literal] = {
        .visible = true,
        .named = true,
    },
    [sym_uint16_literal] = {
        .visible = true,
        .named = true,
    },
    [sym_int32_literal] = {
        .visible = true,
        .named = true,
    },
    [sym_uint32_literal] = {
        .visible = true,
        .named = true,
    },
    [sym_int64_literal] = {
        .visible = true,
        .named = true,
    },
    [sym_uint64_literal] = {
        .visible = true,
        .named = true,
    },
    [sym_int128_literal] = {
        .visible = true,
        .named = true,
    },
    [sym_uint128_literal] = {
        .visible = true,
        .named = true,
    },
    [sym_digits] = {
        .visible = true,
        .named = true,
    },
    [sym_list_literal] = {
        .visible = true,
        .named = true,
    },
    [sym_list_content] = {
        .visible = true,
        .named = true,
    },
    [sym_tuple_literal] = {
        .visible = true,
        .named = true,
    },
    [sym_tuple_literal_the_rest] = {
        .visible = true,
        .named = true,
    },
    [sym_type_reference] = {
        .visible = true,
        .named = true,
    },
    [sym_builtin_type] = {
        .visible = true,
        .named = true,
    },
    [sym_list_type_reference] = {
        .visible = true,
        .named = true,
    },
    [sym_tuple_type_reference] = {
        .visible = true,
        .named = true,
    },
    [sym_type_reference_tuple_the_rest] = {
        .visible = true,
        .named = true,
    },
    [sym_qualified_fn_name] = {
        .visible = true,
        .named = true,
    },
    [sym_qualified_type_name] = {
        .visible = true,
        .named = true,
    },
    [sym_variable_identifier] = {
        .visible = true,
        .named = true,
    },
    [sym_fn_identifier] = {
        .visible = true,
        .named = true,
    },
    [sym_type_identifier] = {
        .visible = true,
        .named = true,
    },
    [sym_module_identifier] = {
        .visible = true,
        .named = true,
    },
    [aux_sym_source_file_repeat1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_source_file_repeat2] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_fn_decl_params_repeat1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_string_content_repeat1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_list_content_repeat1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_tuple_literal_the_rest_repeat1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_type_reference_tuple_the_rest_repeat1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_qualified_fn_name_repeat1] = {
        .visible = false,
        .named = false,
    },
};

enum
{
  field_args = 1,
  field_body = 2,
  field_content = 3,
  field_digits = 4,
  field_expr = 5,
  field_first = 6,
  field_fn = 7,
  field_identifier = 8,
  field_keyword_let = 9,
  field_keyword_type = 10,
  field_keyword_type_constructor = 11,
  field_left = 12,
  field_list_separator = 13,
  field_name = 14,
  field_operator = 15,
  field_params = 16,
  field_rest = 17,
  field_return_type = 18,
  field_right = 19,
  field_second = 20,
  field_suffix = 21,
  field_symbol_asterisk = 22,
  field_symbol_close_angle = 23,
  field_symbol_close_bracket = 24,
  field_symbol_close_quote = 25,
  field_symbol_close_single_quote = 26,
  field_symbol_colon = 27,
  field_symbol_comma = 28,
  field_symbol_equals = 29,
  field_symbol_left_paren = 30,
  field_symbol_open_angle = 31,
  field_symbol_open_bracket = 32,
  field_symbol_open_quote = 33,
  field_symbol_open_single_quote = 34,
  field_symbol_right_paren = 35,
  field_typ = 36,
  field_typ_param = 37,
  field_type = 38,
};

static const char *const ts_field_names[] = {
    [0] = NULL,
    [field_args] = "args",
    [field_body] = "body",
    [field_content] = "content",
    [field_digits] = "digits",
    [field_expr] = "expr",
    [field_first] = "first",
    [field_fn] = "fn",
    [field_identifier] = "identifier",
    [field_keyword_let] = "keyword_let",
    [field_keyword_type] = "keyword_type",
    [field_keyword_type_constructor] = "keyword_type_constructor",
    [field_left] = "left",
    [field_list_separator] = "list_separator",
    [field_name] = "name",
    [field_operator] = "operator",
    [field_params] = "params",
    [field_rest] = "rest",
    [field_return_type] = "return_type",
    [field_right] = "right",
    [field_second] = "second",
    [field_suffix] = "suffix",
    [field_symbol_asterisk] = "symbol_asterisk",
    [field_symbol_close_angle] = "symbol_close_angle",
    [field_symbol_close_bracket] = "symbol_close_bracket",
    [field_symbol_close_quote] = "symbol_close_quote",
    [field_symbol_close_single_quote] = "symbol_close_single_quote",
    [field_symbol_colon] = "symbol_colon",
    [field_symbol_comma] = "symbol_comma",
    [field_symbol_equals] = "symbol_equals",
    [field_symbol_left_paren] = "symbol_left_paren",
    [field_symbol_open_angle] = "symbol_open_angle",
    [field_symbol_open_bracket] = "symbol_open_bracket",
    [field_symbol_open_quote] = "symbol_open_quote",
    [field_symbol_open_single_quote] = "symbol_open_single_quote",
    [field_symbol_right_paren] = "symbol_right_paren",
    [field_typ] = "typ",
    [field_typ_param] = "typ_param",
    [field_type] = "type",
};

static const TSFieldMapSlice ts_field_map_slices[PRODUCTION_ID_COUNT] = {
    [1] = {.index = 0, .length = 2},
    [2] = {.index = 2, .length = 2},
    [3] = {.index = 4, .length = 2},
    [4] = {.index = 6, .length = 3},
    [5] = {.index = 9, .length = 3},
    [6] = {.index = 12, .length = 3},
    [7] = {.index = 15, .length = 1},
    [8] = {.index = 16, .length = 3},
    [9] = {.index = 19, .length = 3},
    [10] = {.index = 22, .length = 4},
    [11] = {.index = 26, .length = 4},
    [12] = {.index = 30, .length = 1},
    [13] = {.index = 31, .length = 2},
    [14] = {.index = 33, .length = 5},
    [15] = {.index = 38, .length = 2},
    [16] = {.index = 40, .length = 5},
    [17] = {.index = 45, .length = 2},
    [18] = {.index = 47, .length = 6},
    [19] = {.index = 53, .length = 4},
    [20] = {.index = 57, .length = 5},
    [21] = {.index = 62, .length = 7},
    [22] = {.index = 69, .length = 4},
    [23] = {.index = 73, .length = 5},
    [24] = {.index = 78, .length = 2},
    [25] = {.index = 80, .length = 2},
    [26] = {.index = 82, .length = 6},
    [27] = {.index = 88, .length = 4},
};

static const TSFieldMapEntry ts_field_map_entries[] = {
    [0] =
        {field_symbol_close_quote, 1},
    {field_symbol_open_quote, 0},
    [2] =
        {field_digits, 0},
    {field_suffix, 1},
    [4] =
        {field_symbol_close_bracket, 1},
    {field_symbol_open_bracket, 0},
    [6] =
        {field_expr, 1},
    {field_symbol_left_paren, 0},
    {field_symbol_right_paren, 2},
    [9] =
        {field_content, 1},
    {field_symbol_close_quote, 2},
    {field_symbol_open_quote, 0},
    [12] =
        {field_content, 1},
    {field_symbol_close_single_quote, 2},
    {field_symbol_open_single_quote, 0},
    [15] =
        {field_list_separator, 1, .inherited = true},
    [16] =
        {field_content, 1},
    {field_symbol_close_bracket, 2},
    {field_symbol_open_bracket, 0},
    [19] =
        {field_left, 0},
    {field_operator, 1},
    {field_right, 2},
    [22] =
        {field_args, 2},
    {field_fn, 1},
    {field_symbol_left_paren, 0},
    {field_symbol_right_paren, 3},
    [26] =
        {field_keyword_type, 0},
    {field_name, 1},
    {field_symbol_equals, 2},
    {field_typ, 3},
    [30] =
        {field_list_separator, 0},
    [31] =
        {field_list_separator, 0, .inherited = true},
    {field_list_separator, 1, .inherited = true},
    [33] =
        {field_first, 1},
    {field_second, 3},
    {field_symbol_comma, 2},
    {field_symbol_left_paren, 0},
    {field_symbol_right_paren, 4},
    [38] =
        {field_expr, 0, .inherited = true},
    {field_symbol_comma, 0, .inherited = true},
    [40] =
        {field_body, 5},
    {field_expr, 3},
    {field_identifier, 1},
    {field_keyword_let, 0},
    {field_symbol_equals, 2},
    [45] =
        {field_expr, 1},
    {field_symbol_comma, 0},
    [47] =
        {field_first, 1},
    {field_rest, 4},
    {field_second, 3},
    {field_symbol_comma, 2},
    {field_symbol_left_paren, 0},
    {field_symbol_right_paren, 5},
    [53] =
        {field_expr, 0, .inherited = true},
    {field_expr, 1, .inherited = true},
    {field_symbol_comma, 0, .inherited = true},
    {field_symbol_comma, 1, .inherited = true},
    [57] =
        {field_identifier, 1},
    {field_symbol_colon, 2},
    {field_symbol_left_paren, 0},
    {field_symbol_right_paren, 4},
    {field_typ, 3},
    [62] =
        {field_body, 6},
    {field_keyword_let, 0},
    {field_name, 1},
    {field_params, 2},
    {field_return_type, 4},
    {field_symbol_colon, 3},
    {field_symbol_equals, 5},
    [69] =
        {field_keyword_type_constructor, 0},
    {field_symbol_close_angle, 3},
    {field_symbol_open_angle, 1},
    {field_typ_param, 2},
    [73] =
        {field_first, 1},
    {field_second, 3},
    {field_symbol_asterisk, 2},
    {field_symbol_left_paren, 0},
    {field_symbol_right_paren, 4},
    [78] =
        {field_symbol_asterisk, 0, .inherited = true},
    {field_type, 0, .inherited = true},
    [80] =
        {field_symbol_asterisk, 0},
    {field_type, 1},
    [82] =
        {field_first, 1},
    {field_rest, 4},
    {field_second, 3},
    {field_symbol_asterisk, 2},
    {field_symbol_left_paren, 0},
    {field_symbol_right_paren, 5},
    [88] =
        {field_symbol_asterisk, 0, .inherited = true},
    {field_symbol_asterisk, 1, .inherited = true},
    {field_type, 0, .inherited = true},
    {field_type, 1, .inherited = true},
};

static const TSSymbol ts_alias_sequences[PRODUCTION_ID_COUNT][MAX_ALIAS_SEQUENCE_LENGTH] = {
    [0] = {0},
    [9] = {
        [1] = anon_sym_CARET,
    },
    [22] = {
        [1] = anon_sym_COLON,
        [3] = anon_sym_COLON,
    },
};

static const uint16_t ts_non_terminal_alias_map[] = {
    0,
};

static const TSStateId ts_primary_state_ids[STATE_COUNT] = {
    [0] = 0,
    [1] = 1,
    [2] = 2,
    [3] = 3,
    [4] = 3,
    [5] = 3,
    [6] = 3,
    [7] = 3,
    [8] = 8,
    [9] = 9,
    [10] = 8,
    [11] = 9,
    [12] = 12,
    [13] = 13,
    [14] = 14,
    [15] = 12,
    [16] = 9,
    [17] = 12,
    [18] = 9,
    [19] = 12,
    [20] = 9,
    [21] = 12,
    [22] = 22,
    [23] = 23,
    [24] = 22,
    [25] = 25,
    [26] = 22,
    [27] = 22,
    [28] = 22,
    [29] = 29,
    [30] = 29,
    [31] = 31,
    [32] = 32,
    [33] = 33,
    [34] = 34,
    [35] = 29,
    [36] = 36,
    [37] = 37,
    [38] = 38,
    [39] = 39,
    [40] = 40,
    [41] = 36,
    [42] = 32,
    [43] = 34,
    [44] = 29,
    [45] = 37,
    [46] = 39,
    [47] = 40,
    [48] = 40,
    [49] = 36,
    [50] = 39,
    [51] = 34,
    [52] = 37,
    [53] = 37,
    [54] = 31,
    [55] = 55,
    [56] = 32,
    [57] = 39,
    [58] = 40,
    [59] = 39,
    [60] = 32,
    [61] = 31,
    [62] = 31,
    [63] = 36,
    [64] = 37,
    [65] = 29,
    [66] = 34,
    [67] = 34,
    [68] = 31,
    [69] = 32,
    [70] = 40,
    [71] = 36,
    [72] = 72,
    [73] = 73,
    [74] = 74,
    [75] = 75,
    [76] = 72,
    [77] = 77,
    [78] = 78,
    [79] = 79,
    [80] = 80,
    [81] = 81,
    [82] = 82,
    [83] = 83,
    [84] = 84,
    [85] = 85,
    [86] = 86,
    [87] = 87,
    [88] = 88,
    [89] = 89,
    [90] = 90,
    [91] = 91,
    [92] = 92,
    [93] = 93,
    [94] = 94,
    [95] = 95,
    [96] = 96,
    [97] = 97,
    [98] = 98,
    [99] = 99,
    [100] = 100,
    [101] = 101,
    [102] = 102,
    [103] = 103,
    [104] = 104,
    [105] = 105,
    [106] = 106,
    [107] = 107,
    [108] = 108,
    [109] = 109,
    [110] = 103,
    [111] = 100,
    [112] = 107,
    [113] = 78,
    [114] = 102,
    [115] = 84,
    [116] = 77,
    [117] = 117,
    [118] = 101,
    [119] = 100,
    [120] = 99,
    [121] = 97,
    [122] = 96,
    [123] = 95,
    [124] = 74,
    [125] = 94,
    [126] = 93,
    [127] = 92,
    [128] = 91,
    [129] = 90,
    [130] = 117,
    [131] = 89,
    [132] = 87,
    [133] = 85,
    [134] = 84,
    [135] = 107,
    [136] = 104,
    [137] = 103,
    [138] = 75,
    [139] = 102,
    [140] = 101,
    [141] = 99,
    [142] = 104,
    [143] = 97,
    [144] = 96,
    [145] = 95,
    [146] = 74,
    [147] = 94,
    [148] = 83,
    [149] = 93,
    [150] = 92,
    [151] = 91,
    [152] = 90,
    [153] = 89,
    [154] = 79,
    [155] = 80,
    [156] = 87,
    [157] = 85,
    [158] = 79,
    [159] = 81,
    [160] = 80,
    [161] = 81,
    [162] = 83,
    [163] = 78,
    [164] = 77,
    [165] = 72,
    [166] = 75,
    [167] = 167,
    [168] = 87,
    [169] = 89,
    [170] = 103,
    [171] = 102,
    [172] = 101,
    [173] = 173,
    [174] = 84,
    [175] = 175,
    [176] = 176,
    [177] = 99,
    [178] = 97,
    [179] = 96,
    [180] = 95,
    [181] = 74,
    [182] = 94,
    [183] = 93,
    [184] = 92,
    [185] = 91,
    [186] = 90,
    [187] = 173,
    [188] = 188,
    [189] = 189,
    [190] = 173,
    [191] = 191,
    [192] = 100,
    [193] = 173,
    [194] = 83,
    [195] = 79,
    [196] = 80,
    [197] = 81,
    [198] = 85,
    [199] = 75,
    [200] = 72,
    [201] = 77,
    [202] = 78,
    [203] = 173,
    [204] = 104,
    [205] = 205,
    [206] = 107,
    [207] = 207,
    [208] = 208,
    [209] = 209,
    [210] = 210,
    [211] = 211,
    [212] = 211,
    [213] = 211,
    [214] = 211,
    [215] = 211,
    [216] = 216,
    [217] = 102,
    [218] = 81,
    [219] = 95,
    [220] = 96,
    [221] = 97,
    [222] = 99,
    [223] = 100,
    [224] = 101,
    [225] = 225,
    [226] = 103,
    [227] = 225,
    [228] = 104,
    [229] = 107,
    [230] = 84,
    [231] = 225,
    [232] = 94,
    [233] = 93,
    [234] = 92,
    [235] = 91,
    [236] = 90,
    [237] = 89,
    [238] = 225,
    [239] = 78,
    [240] = 77,
    [241] = 72,
    [242] = 75,
    [243] = 83,
    [244] = 225,
    [245] = 80,
    [246] = 79,
    [247] = 85,
    [248] = 87,
    [249] = 74,
    [250] = 250,
    [251] = 251,
    [252] = 252,
    [253] = 253,
    [254] = 254,
    [255] = 254,
    [256] = 254,
    [257] = 254,
    [258] = 254,
    [259] = 259,
    [260] = 260,
    [261] = 261,
    [262] = 262,
    [263] = 261,
    [264] = 264,
    [265] = 262,
    [266] = 262,
    [267] = 267,
    [268] = 261,
    [269] = 269,
    [270] = 261,
    [271] = 262,
    [272] = 262,
    [273] = 261,
    [274] = 274,
    [275] = 275,
    [276] = 276,
    [277] = 277,
    [278] = 278,
    [279] = 279,
    [280] = 280,
    [281] = 281,
    [282] = 279,
    [283] = 283,
    [284] = 284,
    [285] = 285,
    [286] = 286,
    [287] = 279,
    [288] = 288,
    [289] = 279,
    [290] = 279,
    [291] = 291,
    [292] = 292,
    [293] = 293,
    [294] = 294,
    [295] = 294,
    [296] = 294,
    [297] = 294,
    [298] = 298,
    [299] = 299,
    [300] = 300,
    [301] = 294,
    [302] = 302,
    [303] = 303,
    [304] = 304,
    [305] = 305,
    [306] = 303,
    [307] = 304,
    [308] = 308,
    [309] = 309,
    [310] = 308,
    [311] = 311,
    [312] = 302,
    [313] = 313,
    [314] = 302,
    [315] = 302,
    [316] = 316,
    [317] = 317,
    [318] = 308,
    [319] = 304,
    [320] = 320,
    [321] = 303,
    [322] = 322,
    [323] = 323,
    [324] = 324,
    [325] = 325,
    [326] = 304,
    [327] = 308,
    [328] = 328,
    [329] = 329,
    [330] = 330,
    [331] = 302,
    [332] = 303,
    [333] = 320,
    [334] = 334,
    [335] = 320,
    [336] = 320,
    [337] = 320,
    [338] = 308,
    [339] = 304,
    [340] = 303,
};

static bool ts_lex(TSLexer *lexer, TSStateId state)
{
  START_LEXER();
  eof = lexer->eof(lexer);
  switch (state)
  {
  case 0:
    if (eof)
      ADVANCE(71);
    if (lookahead == '!')
      ADVANCE(17);
    if (lookahead == '"')
      ADVANCE(85);
    if (lookahead == '%')
      ADVANCE(94);
    if (lookahead == '&')
      ADVANCE(5);
    if (lookahead == '\'')
      ADVANCE(88);
    if (lookahead == '(')
      ADVANCE(76);
    if (lookahead == ')')
      ADVANCE(77);
    if (lookahead == '*')
      ADVANCE(92);
    if (lookahead == '+')
      ADVANCE(96);
    if (lookahead == ',')
      ADVANCE(124);
    if (lookahead == '-')
      ADVANCE(98);
    if (lookahead == '.')
      ADVANCE(160);
    if (lookahead == '/')
      ADVANCE(93);
    if (lookahead == ':')
      ADVANCE(73);
    if (lookahead == ';')
      ADVANCE(123);
    if (lookahead == '<')
      ADVANCE(101);
    if (lookahead == '=')
      ADVANCE(75);
    if (lookahead == '>')
      ADVANCE(104);
    if (lookahead == 'B')
      ADVANCE(46);
    if (lookahead == 'C')
      ADVANCE(33);
    if (lookahead == 'D')
      ADVANCE(23);
    if (lookahead == 'F')
      ADVANCE(41);
    if (lookahead == 'I')
      ADVANCE(44);
    if (lookahead == 'L')
      ADVANCE(114);
    if (lookahead == 'Q')
      ADVANCE(116);
    if (lookahead == 'S')
      ADVANCE(54);
    if (lookahead == 'U')
      ADVANCE(19);
    if (lookahead == 'Z')
      ADVANCE(117);
    if (lookahead == '[')
      ADVANCE(121);
    if (lookahead == '\\')
      ADVANCE(62);
    if (lookahead == ']')
      ADVANCE(122);
    if (lookahead == '^')
      ADVANCE(91);
    if (lookahead == 'f')
      ADVANCE(22);
    if (lookahead == 'l')
      ADVANCE(112);
    if (lookahead == 's')
      ADVANCE(110);
    if (lookahead == 't')
      ADVANCE(50);
    if (lookahead == 'u')
      ADVANCE(38);
    if (lookahead == 'y')
      ADVANCE(108);
    if (lookahead == '|')
      ADVANCE(61);
    if (lookahead == '\t' ||
        lookahead == '\n' ||
        lookahead == '\r' ||
        lookahead == ' ')
      SKIP(66)
    if (('0' <= lookahead && lookahead <= '9'))
      ADVANCE(119);
    END_STATE();
  case 1:
    if (lookahead == '\n')
      ADVANCE(84);
    if (lookahead == '!')
      ADVANCE(17);
    if (lookahead == '%')
      ADVANCE(94);
    if (lookahead == '&')
      ADVANCE(5);
    if (lookahead == '*')
      ADVANCE(92);
    if (lookahead == '+')
      ADVANCE(95);
    if (lookahead == '-')
      ADVANCE(97);
    if (lookahead == '/')
      ADVANCE(93);
    if (lookahead == '<')
      ADVANCE(101);
    if (lookahead == '=')
      ADVANCE(18);
    if (lookahead == '>')
      ADVANCE(104);
    if (lookahead == '^')
      ADVANCE(91);
    if (lookahead == '|')
      ADVANCE(61);
    if (lookahead == '\t' ||
        lookahead == '\r' ||
        lookahead == ' ')
      SKIP(1)
    END_STATE();
  case 2:
    if (lookahead == '\n')
      SKIP(4)
    if (lookahead == '"')
      ADVANCE(85);
    if (lookahead == '\\')
      ADVANCE(62);
    if (lookahead == '\t' ||
        lookahead == '\r' ||
        lookahead == ' ')
      ADVANCE(86);
    if (lookahead != 0)
      ADVANCE(87);
    END_STATE();
  case 3:
    if (lookahead == '!')
      ADVANCE(17);
    if (lookahead == '%')
      ADVANCE(94);
    if (lookahead == '&')
      ADVANCE(5);
    if (lookahead == ')')
      ADVANCE(77);
    if (lookahead == '*')
      ADVANCE(92);
    if (lookahead == '+')
      ADVANCE(95);
    if (lookahead == ',')
      ADVANCE(124);
    if (lookahead == '-')
      ADVANCE(97);
    if (lookahead == '/')
      ADVANCE(93);
    if (lookahead == ';')
      ADVANCE(123);
    if (lookahead == '<')
      ADVANCE(101);
    if (lookahead == '=')
      ADVANCE(18);
    if (lookahead == '>')
      ADVANCE(104);
    if (lookahead == ']')
      ADVANCE(122);
    if (lookahead == '^')
      ADVANCE(91);
    if (lookahead == '|')
      ADVANCE(61);
    if (lookahead == '\t' ||
        lookahead == '\n' ||
        lookahead == '\r' ||
        lookahead == ' ')
      SKIP(3)
    if (('A' <= lookahead && lookahead <= 'Z'))
      ADVANCE(218);
    if (('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 4:
    if (lookahead == '"')
      ADVANCE(85);
    if (lookahead == '\t' ||
        lookahead == '\n' ||
        lookahead == '\r' ||
        lookahead == ' ')
      SKIP(4)
    END_STATE();
  case 5:
    if (lookahead == '&')
      ADVANCE(106);
    END_STATE();
  case 6:
    if (lookahead == '.')
      ADVANCE(65);
    if (('0' <= lookahead && lookahead <= '9'))
      ADVANCE(6);
    END_STATE();
  case 7:
    if (lookahead == '1')
      ADVANCE(9);
    if (lookahead == '3')
      ADVANCE(10);
    if (lookahead == '6')
      ADVANCE(13);
    if (lookahead == '8')
      ADVANCE(129);
    END_STATE();
  case 8:
    if (lookahead == '1')
      ADVANCE(12);
    if (lookahead == '3')
      ADVANCE(11);
    if (lookahead == '6')
      ADVANCE(14);
    if (lookahead == '8')
      ADVANCE(131);
    END_STATE();
  case 9:
    if (lookahead == '2')
      ADVANCE(15);
    if (lookahead == '6')
      ADVANCE(133);
    END_STATE();
  case 10:
    if (lookahead == '2')
      ADVANCE(137);
    END_STATE();
  case 11:
    if (lookahead == '2')
      ADVANCE(139);
    END_STATE();
  case 12:
    if (lookahead == '2')
      ADVANCE(16);
    if (lookahead == '6')
      ADVANCE(135);
    END_STATE();
  case 13:
    if (lookahead == '4')
      ADVANCE(141);
    END_STATE();
  case 14:
    if (lookahead == '4')
      ADVANCE(143);
    END_STATE();
  case 15:
    if (lookahead == '8')
      ADVANCE(145);
    END_STATE();
  case 16:
    if (lookahead == '8')
      ADVANCE(147);
    END_STATE();
  case 17:
    if (lookahead == '=')
      ADVANCE(100);
    END_STATE();
  case 18:
    if (lookahead == '=')
      ADVANCE(99);
    END_STATE();
  case 19:
    if (lookahead == 'I')
      ADVANCE(45);
    if (lookahead == 'L')
      ADVANCE(115);
    if (lookahead == 'n')
      ADVANCE(37);
    if (lookahead == 'u')
      ADVANCE(34);
    END_STATE();
  case 20:
    if (lookahead == 'T')
      ADVANCE(35);
    END_STATE();
  case 21:
    if (lookahead == '\\')
      ADVANCE(62);
    if (lookahead != 0 &&
        lookahead != '\n' &&
        lookahead != '\'')
      ADVANCE(89);
    END_STATE();
  case 22:
    if (lookahead == 'a')
      ADVANCE(39);
    END_STATE();
  case 23:
    if (lookahead == 'a')
      ADVANCE(55);
    END_STATE();
  case 24:
    if (lookahead == 'a')
      ADVANCE(51);
    END_STATE();
  case 25:
    if (lookahead == 'a')
      ADVANCE(58);
    END_STATE();
  case 26:
    if (lookahead == 'd')
      ADVANCE(157);
    END_STATE();
  case 27:
    if (lookahead == 'e')
      ADVANCE(20);
    END_STATE();
  case 28:
    if (lookahead == 'e')
      ADVANCE(80);
    END_STATE();
  case 29:
    if (lookahead == 'e')
      ADVANCE(78);
    END_STATE();
  case 30:
    if (lookahead == 'e')
      ADVANCE(82);
    END_STATE();
  case 31:
    if (lookahead == 'e')
      ADVANCE(155);
    END_STATE();
  case 32:
    if (lookahead == 'g')
      ADVANCE(153);
    END_STATE();
  case 33:
    if (lookahead == 'h')
      ADVANCE(24);
    END_STATE();
  case 34:
    if (lookahead == 'i')
      ADVANCE(26);
    END_STATE();
  case 35:
    if (lookahead == 'i')
      ADVANCE(42);
    END_STATE();
  case 36:
    if (lookahead == 'i')
      ADVANCE(43);
    END_STATE();
  case 37:
    if (lookahead == 'i')
      ADVANCE(57);
    END_STATE();
  case 38:
    if (lookahead == 'l')
      ADVANCE(113);
    if (lookahead == 's')
      ADVANCE(111);
    if (lookahead == 'y')
      ADVANCE(109);
    END_STATE();
  case 39:
    if (lookahead == 'l')
      ADVANCE(53);
    END_STATE();
  case 40:
    if (lookahead == 'l')
      ADVANCE(127);
    END_STATE();
  case 41:
    if (lookahead == 'l')
      ADVANCE(48);
    END_STATE();
  case 42:
    if (lookahead == 'm')
      ADVANCE(31);
    END_STATE();
  case 43:
    if (lookahead == 'n')
      ADVANCE(32);
    END_STATE();
  case 44:
    if (lookahead == 'n')
      ADVANCE(56);
    END_STATE();
  case 45:
    if (lookahead == 'n')
      ADVANCE(59);
    END_STATE();
  case 46:
    if (lookahead == 'o')
      ADVANCE(47);
    END_STATE();
  case 47:
    if (lookahead == 'o')
      ADVANCE(40);
    END_STATE();
  case 48:
    if (lookahead == 'o')
      ADVANCE(25);
    END_STATE();
  case 49:
    if (lookahead == 'p')
      ADVANCE(29);
    END_STATE();
  case 50:
    if (lookahead == 'r')
      ADVANCE(60);
    if (lookahead == 'y')
      ADVANCE(49);
    END_STATE();
  case 51:
    if (lookahead == 'r')
      ADVANCE(151);
    END_STATE();
  case 52:
    if (lookahead == 'r')
      ADVANCE(36);
    END_STATE();
  case 53:
    if (lookahead == 's')
      ADVANCE(30);
    END_STATE();
  case 54:
    if (lookahead == 't')
      ADVANCE(52);
    END_STATE();
  case 55:
    if (lookahead == 't')
      ADVANCE(27);
    END_STATE();
  case 56:
    if (lookahead == 't')
      ADVANCE(7);
    END_STATE();
  case 57:
    if (lookahead == 't')
      ADVANCE(125);
    END_STATE();
  case 58:
    if (lookahead == 't')
      ADVANCE(149);
    END_STATE();
  case 59:
    if (lookahead == 't')
      ADVANCE(8);
    END_STATE();
  case 60:
    if (lookahead == 'u')
      ADVANCE(28);
    END_STATE();
  case 61:
    if (lookahead == '|')
      ADVANCE(107);
    END_STATE();
  case 62:
    if (lookahead == '"' ||
        lookahead == '/' ||
        lookahead == '\\' ||
        lookahead == 'b' ||
        lookahead == 'f' ||
        lookahead == 'n' ||
        lookahead == 'r' ||
        lookahead == 't' ||
        lookahead == 'u')
      ADVANCE(90);
    END_STATE();
  case 63:
    if (('0' <= lookahead && lookahead <= '9'))
      ADVANCE(6);
    END_STATE();
  case 64:
    if (('0' <= lookahead && lookahead <= '9'))
      ADVANCE(118);
    END_STATE();
  case 65:
    if (('0' <= lookahead && lookahead <= '9'))
      ADVANCE(120);
    END_STATE();
  case 66:
    if (eof)
      ADVANCE(71);
    if (lookahead == '!')
      ADVANCE(17);
    if (lookahead == '"')
      ADVANCE(85);
    if (lookahead == '%')
      ADVANCE(94);
    if (lookahead == '&')
      ADVANCE(5);
    if (lookahead == '\'')
      ADVANCE(88);
    if (lookahead == '(')
      ADVANCE(76);
    if (lookahead == ')')
      ADVANCE(77);
    if (lookahead == '*')
      ADVANCE(92);
    if (lookahead == '+')
      ADVANCE(96);
    if (lookahead == ',')
      ADVANCE(124);
    if (lookahead == '-')
      ADVANCE(98);
    if (lookahead == '.')
      ADVANCE(160);
    if (lookahead == '/')
      ADVANCE(93);
    if (lookahead == ':')
      ADVANCE(73);
    if (lookahead == ';')
      ADVANCE(123);
    if (lookahead == '<')
      ADVANCE(101);
    if (lookahead == '=')
      ADVANCE(75);
    if (lookahead == '>')
      ADVANCE(104);
    if (lookahead == 'B')
      ADVANCE(46);
    if (lookahead == 'C')
      ADVANCE(33);
    if (lookahead == 'D')
      ADVANCE(23);
    if (lookahead == 'F')
      ADVANCE(41);
    if (lookahead == 'I')
      ADVANCE(44);
    if (lookahead == 'L')
      ADVANCE(114);
    if (lookahead == 'Q')
      ADVANCE(116);
    if (lookahead == 'S')
      ADVANCE(54);
    if (lookahead == 'U')
      ADVANCE(19);
    if (lookahead == 'Z')
      ADVANCE(117);
    if (lookahead == '[')
      ADVANCE(121);
    if (lookahead == ']')
      ADVANCE(122);
    if (lookahead == '^')
      ADVANCE(91);
    if (lookahead == 'f')
      ADVANCE(22);
    if (lookahead == 'l')
      ADVANCE(112);
    if (lookahead == 's')
      ADVANCE(110);
    if (lookahead == 't')
      ADVANCE(50);
    if (lookahead == 'u')
      ADVANCE(38);
    if (lookahead == 'y')
      ADVANCE(108);
    if (lookahead == '|')
      ADVANCE(61);
    if (lookahead == '\t' ||
        lookahead == '\n' ||
        lookahead == '\r' ||
        lookahead == ' ')
      SKIP(66)
    if (('0' <= lookahead && lookahead <= '9'))
      ADVANCE(119);
    END_STATE();
  case 67:
    if (eof)
      ADVANCE(71);
    if (lookahead == '!')
      ADVANCE(17);
    if (lookahead == '"')
      ADVANCE(85);
    if (lookahead == '%')
      ADVANCE(94);
    if (lookahead == '&')
      ADVANCE(5);
    if (lookahead == '\'')
      ADVANCE(88);
    if (lookahead == '(')
      ADVANCE(76);
    if (lookahead == ')')
      ADVANCE(77);
    if (lookahead == '*')
      ADVANCE(92);
    if (lookahead == '+')
      ADVANCE(96);
    if (lookahead == ',')
      ADVANCE(124);
    if (lookahead == '-')
      ADVANCE(98);
    if (lookahead == '/')
      ADVANCE(93);
    if (lookahead == ':')
      ADVANCE(73);
    if (lookahead == '<')
      ADVANCE(101);
    if (lookahead == '=')
      ADVANCE(75);
    if (lookahead == '>')
      ADVANCE(104);
    if (lookahead == '[')
      ADVANCE(121);
    if (lookahead == ']')
      ADVANCE(122);
    if (lookahead == '^')
      ADVANCE(91);
    if (lookahead == 'f')
      ADVANCE(161);
    if (lookahead == 'l')
      ADVANCE(162);
    if (lookahead == 't')
      ADVANCE(169);
    if (lookahead == '|')
      ADVANCE(61);
    if (lookahead == '\t' ||
        lookahead == '\n' ||
        lookahead == '\r' ||
        lookahead == ' ')
      SKIP(67)
    if (('0' <= lookahead && lookahead <= '9'))
      ADVANCE(119);
    if (('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    if (('A' <= lookahead && lookahead <= 'Z'))
      ADVANCE(218);
    END_STATE();
  case 68:
    if (eof)
      ADVANCE(71);
    if (lookahead == '!')
      ADVANCE(17);
    if (lookahead == '"')
      ADVANCE(85);
    if (lookahead == '%')
      ADVANCE(94);
    if (lookahead == '&')
      ADVANCE(5);
    if (lookahead == '\'')
      ADVANCE(88);
    if (lookahead == '(')
      ADVANCE(76);
    if (lookahead == ')')
      ADVANCE(77);
    if (lookahead == '*')
      ADVANCE(92);
    if (lookahead == '+')
      ADVANCE(96);
    if (lookahead == '-')
      ADVANCE(98);
    if (lookahead == '/')
      ADVANCE(93);
    if (lookahead == '<')
      ADVANCE(101);
    if (lookahead == '=')
      ADVANCE(18);
    if (lookahead == '>')
      ADVANCE(104);
    if (lookahead == '[')
      ADVANCE(121);
    if (lookahead == '^')
      ADVANCE(91);
    if (lookahead == 'f')
      ADVANCE(161);
    if (lookahead == 'l')
      ADVANCE(162);
    if (lookahead == 't')
      ADVANCE(169);
    if (lookahead == '|')
      ADVANCE(61);
    if (lookahead == '\t' ||
        lookahead == '\n' ||
        lookahead == '\r' ||
        lookahead == ' ')
      SKIP(68)
    if (('0' <= lookahead && lookahead <= '9'))
      ADVANCE(119);
    if (('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 69:
    if (eof)
      ADVANCE(71);
    if (lookahead == '!')
      ADVANCE(17);
    if (lookahead == '"')
      ADVANCE(85);
    if (lookahead == '%')
      ADVANCE(94);
    if (lookahead == '&')
      ADVANCE(5);
    if (lookahead == '\'')
      ADVANCE(88);
    if (lookahead == '(')
      ADVANCE(76);
    if (lookahead == '*')
      ADVANCE(92);
    if (lookahead == '+')
      ADVANCE(96);
    if (lookahead == '-')
      ADVANCE(98);
    if (lookahead == '/')
      ADVANCE(93);
    if (lookahead == '<')
      ADVANCE(101);
    if (lookahead == '=')
      ADVANCE(18);
    if (lookahead == '>')
      ADVANCE(104);
    if (lookahead == 'B')
      ADVANCE(205);
    if (lookahead == 'C')
      ADVANCE(193);
    if (lookahead == 'D')
      ADVANCE(186);
    if (lookahead == 'F')
      ADVANCE(200);
    if (lookahead == 'I')
      ADVANCE(203);
    if (lookahead == 'L')
      ADVANCE(194);
    if (lookahead == 'S')
      ADVANCE(211);
    if (lookahead == 'U')
      ADVANCE(184);
    if (lookahead == '[')
      ADVANCE(121);
    if (lookahead == '^')
      ADVANCE(91);
    if (lookahead == 'f')
      ADVANCE(161);
    if (lookahead == 'l')
      ADVANCE(162);
    if (lookahead == 't')
      ADVANCE(168);
    if (lookahead == '|')
      ADVANCE(61);
    if (lookahead == '\t' ||
        lookahead == '\n' ||
        lookahead == '\r' ||
        lookahead == ' ')
      SKIP(69)
    if (('0' <= lookahead && lookahead <= '9'))
      ADVANCE(119);
    if (('A' <= lookahead && lookahead <= 'Z'))
      ADVANCE(218);
    if (('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 70:
    if (eof)
      ADVANCE(71);
    if (lookahead == '"')
      ADVANCE(85);
    if (lookahead == '\'')
      ADVANCE(88);
    if (lookahead == '(')
      ADVANCE(76);
    if (lookahead == ')')
      ADVANCE(77);
    if (lookahead == '*')
      ADVANCE(92);
    if (lookahead == '+')
      ADVANCE(63);
    if (lookahead == '-')
      ADVANCE(64);
    if (lookahead == '.')
      ADVANCE(160);
    if (lookahead == '=')
      ADVANCE(74);
    if (lookahead == '>')
      ADVANCE(103);
    if (lookahead == '[')
      ADVANCE(121);
    if (lookahead == 'f')
      ADVANCE(161);
    if (lookahead == 'l')
      ADVANCE(162);
    if (lookahead == 't')
      ADVANCE(168);
    if (lookahead == '\t' ||
        lookahead == '\n' ||
        lookahead == '\r' ||
        lookahead == ' ')
      SKIP(70)
    if (('0' <= lookahead && lookahead <= '9'))
      ADVANCE(119);
    if (('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 71:
    ACCEPT_TOKEN(ts_builtin_sym_end);
    END_STATE();
  case 72:
    ACCEPT_TOKEN(anon_sym_let);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 73:
    ACCEPT_TOKEN(anon_sym_COLON);
    END_STATE();
  case 74:
    ACCEPT_TOKEN(anon_sym_EQ);
    END_STATE();
  case 75:
    ACCEPT_TOKEN(anon_sym_EQ);
    if (lookahead == '=')
      ADVANCE(99);
    END_STATE();
  case 76:
    ACCEPT_TOKEN(anon_sym_LPAREN);
    if (lookahead == ')')
      ADVANCE(219);
    END_STATE();
  case 77:
    ACCEPT_TOKEN(anon_sym_RPAREN);
    END_STATE();
  case 78:
    ACCEPT_TOKEN(anon_sym_type);
    END_STATE();
  case 79:
    ACCEPT_TOKEN(anon_sym_type);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 80:
    ACCEPT_TOKEN(aux_sym_bool_literal_token1);
    END_STATE();
  case 81:
    ACCEPT_TOKEN(aux_sym_bool_literal_token1);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 82:
    ACCEPT_TOKEN(aux_sym_bool_literal_token2);
    END_STATE();
  case 83:
    ACCEPT_TOKEN(aux_sym_bool_literal_token2);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 84:
    ACCEPT_TOKEN(anon_sym_LF);
    if (lookahead == '\n')
      ADVANCE(84);
    END_STATE();
  case 85:
    ACCEPT_TOKEN(anon_sym_DQUOTE);
    END_STATE();
  case 86:
    ACCEPT_TOKEN(aux_sym_string_content_token1);
    if (lookahead == '\t' ||
        lookahead == '\r' ||
        lookahead == ' ')
      ADVANCE(86);
    if (lookahead != 0 &&
        lookahead != '\n' &&
        lookahead != '"' &&
        lookahead != '\\')
      ADVANCE(87);
    END_STATE();
  case 87:
    ACCEPT_TOKEN(aux_sym_string_content_token1);
    if (lookahead != 0 &&
        lookahead != '\n' &&
        lookahead != '"' &&
        lookahead != '\\')
      ADVANCE(87);
    END_STATE();
  case 88:
    ACCEPT_TOKEN(anon_sym_SQUOTE);
    END_STATE();
  case 89:
    ACCEPT_TOKEN(aux_sym_character_token1);
    END_STATE();
  case 90:
    ACCEPT_TOKEN(sym_char_or_string_escape_sequence);
    END_STATE();
  case 91:
    ACCEPT_TOKEN(anon_sym_CARET);
    END_STATE();
  case 92:
    ACCEPT_TOKEN(anon_sym_STAR);
    END_STATE();
  case 93:
    ACCEPT_TOKEN(anon_sym_SLASH);
    END_STATE();
  case 94:
    ACCEPT_TOKEN(anon_sym_PERCENT);
    END_STATE();
  case 95:
    ACCEPT_TOKEN(anon_sym_PLUS);
    END_STATE();
  case 96:
    ACCEPT_TOKEN(anon_sym_PLUS);
    if (('0' <= lookahead && lookahead <= '9'))
      ADVANCE(6);
    END_STATE();
  case 97:
    ACCEPT_TOKEN(anon_sym_DASH);
    END_STATE();
  case 98:
    ACCEPT_TOKEN(anon_sym_DASH);
    if (('0' <= lookahead && lookahead <= '9'))
      ADVANCE(118);
    END_STATE();
  case 99:
    ACCEPT_TOKEN(anon_sym_EQ_EQ);
    END_STATE();
  case 100:
    ACCEPT_TOKEN(anon_sym_BANG_EQ);
    END_STATE();
  case 101:
    ACCEPT_TOKEN(anon_sym_LT);
    if (lookahead == '=')
      ADVANCE(102);
    END_STATE();
  case 102:
    ACCEPT_TOKEN(anon_sym_LT_EQ);
    END_STATE();
  case 103:
    ACCEPT_TOKEN(anon_sym_GT);
    END_STATE();
  case 104:
    ACCEPT_TOKEN(anon_sym_GT);
    if (lookahead == '=')
      ADVANCE(105);
    END_STATE();
  case 105:
    ACCEPT_TOKEN(anon_sym_GT_EQ);
    END_STATE();
  case 106:
    ACCEPT_TOKEN(anon_sym_AMP_AMP);
    END_STATE();
  case 107:
    ACCEPT_TOKEN(anon_sym_PIPE_PIPE);
    END_STATE();
  case 108:
    ACCEPT_TOKEN(anon_sym_y);
    END_STATE();
  case 109:
    ACCEPT_TOKEN(anon_sym_uy);
    END_STATE();
  case 110:
    ACCEPT_TOKEN(anon_sym_s);
    END_STATE();
  case 111:
    ACCEPT_TOKEN(anon_sym_us);
    END_STATE();
  case 112:
    ACCEPT_TOKEN(anon_sym_l);
    END_STATE();
  case 113:
    ACCEPT_TOKEN(anon_sym_ul);
    END_STATE();
  case 114:
    ACCEPT_TOKEN(anon_sym_L);
    END_STATE();
  case 115:
    ACCEPT_TOKEN(anon_sym_UL);
    END_STATE();
  case 116:
    ACCEPT_TOKEN(anon_sym_Q);
    END_STATE();
  case 117:
    ACCEPT_TOKEN(anon_sym_Z);
    END_STATE();
  case 118:
    ACCEPT_TOKEN(sym_negative_digits);
    if (lookahead == '.')
      ADVANCE(65);
    if (('0' <= lookahead && lookahead <= '9'))
      ADVANCE(118);
    END_STATE();
  case 119:
    ACCEPT_TOKEN(sym_positive_digits);
    if (lookahead == '.')
      ADVANCE(65);
    if (('0' <= lookahead && lookahead <= '9'))
      ADVANCE(119);
    END_STATE();
  case 120:
    ACCEPT_TOKEN(sym_float_literal);
    if (('0' <= lookahead && lookahead <= '9'))
      ADVANCE(120);
    END_STATE();
  case 121:
    ACCEPT_TOKEN(anon_sym_LBRACK);
    END_STATE();
  case 122:
    ACCEPT_TOKEN(anon_sym_RBRACK);
    END_STATE();
  case 123:
    ACCEPT_TOKEN(anon_sym_SEMI);
    END_STATE();
  case 124:
    ACCEPT_TOKEN(anon_sym_COMMA);
    END_STATE();
  case 125:
    ACCEPT_TOKEN(aux_sym_builtin_type_token1);
    END_STATE();
  case 126:
    ACCEPT_TOKEN(aux_sym_builtin_type_token1);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 127:
    ACCEPT_TOKEN(aux_sym_builtin_type_token2);
    END_STATE();
  case 128:
    ACCEPT_TOKEN(aux_sym_builtin_type_token2);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 129:
    ACCEPT_TOKEN(aux_sym_builtin_type_token3);
    END_STATE();
  case 130:
    ACCEPT_TOKEN(aux_sym_builtin_type_token3);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 131:
    ACCEPT_TOKEN(aux_sym_builtin_type_token4);
    END_STATE();
  case 132:
    ACCEPT_TOKEN(aux_sym_builtin_type_token4);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 133:
    ACCEPT_TOKEN(aux_sym_builtin_type_token5);
    END_STATE();
  case 134:
    ACCEPT_TOKEN(aux_sym_builtin_type_token5);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 135:
    ACCEPT_TOKEN(aux_sym_builtin_type_token6);
    END_STATE();
  case 136:
    ACCEPT_TOKEN(aux_sym_builtin_type_token6);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 137:
    ACCEPT_TOKEN(aux_sym_builtin_type_token7);
    END_STATE();
  case 138:
    ACCEPT_TOKEN(aux_sym_builtin_type_token7);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 139:
    ACCEPT_TOKEN(aux_sym_builtin_type_token8);
    END_STATE();
  case 140:
    ACCEPT_TOKEN(aux_sym_builtin_type_token8);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 141:
    ACCEPT_TOKEN(aux_sym_builtin_type_token9);
    END_STATE();
  case 142:
    ACCEPT_TOKEN(aux_sym_builtin_type_token9);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 143:
    ACCEPT_TOKEN(aux_sym_builtin_type_token10);
    END_STATE();
  case 144:
    ACCEPT_TOKEN(aux_sym_builtin_type_token10);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 145:
    ACCEPT_TOKEN(aux_sym_builtin_type_token11);
    END_STATE();
  case 146:
    ACCEPT_TOKEN(aux_sym_builtin_type_token11);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 147:
    ACCEPT_TOKEN(aux_sym_builtin_type_token12);
    END_STATE();
  case 148:
    ACCEPT_TOKEN(aux_sym_builtin_type_token12);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 149:
    ACCEPT_TOKEN(aux_sym_builtin_type_token13);
    END_STATE();
  case 150:
    ACCEPT_TOKEN(aux_sym_builtin_type_token13);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 151:
    ACCEPT_TOKEN(aux_sym_builtin_type_token14);
    END_STATE();
  case 152:
    ACCEPT_TOKEN(aux_sym_builtin_type_token14);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 153:
    ACCEPT_TOKEN(aux_sym_builtin_type_token15);
    END_STATE();
  case 154:
    ACCEPT_TOKEN(aux_sym_builtin_type_token15);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 155:
    ACCEPT_TOKEN(aux_sym_builtin_type_token16);
    END_STATE();
  case 156:
    ACCEPT_TOKEN(aux_sym_builtin_type_token16);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 157:
    ACCEPT_TOKEN(aux_sym_builtin_type_token17);
    END_STATE();
  case 158:
    ACCEPT_TOKEN(aux_sym_builtin_type_token17);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 159:
    ACCEPT_TOKEN(anon_sym_List);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 160:
    ACCEPT_TOKEN(anon_sym_DOT);
    END_STATE();
  case 161:
    ACCEPT_TOKEN(aux_sym_variable_identifier_token1);
    if (lookahead == 'a')
      ADVANCE(166);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('b' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 162:
    ACCEPT_TOKEN(aux_sym_variable_identifier_token1);
    if (lookahead == 'e')
      ADVANCE(171);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 163:
    ACCEPT_TOKEN(aux_sym_variable_identifier_token1);
    if (lookahead == 'e')
      ADVANCE(81);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 164:
    ACCEPT_TOKEN(aux_sym_variable_identifier_token1);
    if (lookahead == 'e')
      ADVANCE(79);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 165:
    ACCEPT_TOKEN(aux_sym_variable_identifier_token1);
    if (lookahead == 'e')
      ADVANCE(83);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 166:
    ACCEPT_TOKEN(aux_sym_variable_identifier_token1);
    if (lookahead == 'l')
      ADVANCE(170);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 167:
    ACCEPT_TOKEN(aux_sym_variable_identifier_token1);
    if (lookahead == 'p')
      ADVANCE(164);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 168:
    ACCEPT_TOKEN(aux_sym_variable_identifier_token1);
    if (lookahead == 'r')
      ADVANCE(172);
    if (lookahead == 'y')
      ADVANCE(167);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 169:
    ACCEPT_TOKEN(aux_sym_variable_identifier_token1);
    if (lookahead == 'r')
      ADVANCE(172);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 170:
    ACCEPT_TOKEN(aux_sym_variable_identifier_token1);
    if (lookahead == 's')
      ADVANCE(165);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 171:
    ACCEPT_TOKEN(aux_sym_variable_identifier_token1);
    if (lookahead == 't')
      ADVANCE(72);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 172:
    ACCEPT_TOKEN(aux_sym_variable_identifier_token1);
    if (lookahead == 'u')
      ADVANCE(163);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 173:
    ACCEPT_TOKEN(aux_sym_variable_identifier_token1);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(173);
    END_STATE();
  case 174:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == '1')
      ADVANCE(176);
    if (lookahead == '3')
      ADVANCE(177);
    if (lookahead == '6')
      ADVANCE(180);
    if (lookahead == '8')
      ADVANCE(130);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 175:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == '1')
      ADVANCE(179);
    if (lookahead == '3')
      ADVANCE(178);
    if (lookahead == '6')
      ADVANCE(181);
    if (lookahead == '8')
      ADVANCE(132);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 176:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == '2')
      ADVANCE(182);
    if (lookahead == '6')
      ADVANCE(134);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 177:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == '2')
      ADVANCE(138);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 178:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == '2')
      ADVANCE(140);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 179:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == '2')
      ADVANCE(183);
    if (lookahead == '6')
      ADVANCE(136);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 180:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == '4')
      ADVANCE(142);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 181:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == '4')
      ADVANCE(144);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 182:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == '8')
      ADVANCE(146);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 183:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == '8')
      ADVANCE(148);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 184:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'I')
      ADVANCE(204);
    if (lookahead == 'n')
      ADVANCE(198);
    if (lookahead == 'u')
      ADVANCE(195);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 185:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'T')
      ADVANCE(196);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 186:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'a')
      ADVANCE(212);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('b' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 187:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'a')
      ADVANCE(208);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('b' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 188:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'a')
      ADVANCE(216);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('b' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 189:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'd')
      ADVANCE(158);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 190:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'e')
      ADVANCE(185);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 191:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'e')
      ADVANCE(156);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 192:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'g')
      ADVANCE(154);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 193:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'h')
      ADVANCE(187);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 194:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'i')
      ADVANCE(210);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 195:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'i')
      ADVANCE(189);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 196:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'i')
      ADVANCE(201);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 197:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'i')
      ADVANCE(202);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 198:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'i')
      ADVANCE(215);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 199:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'l')
      ADVANCE(128);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 200:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'l')
      ADVANCE(207);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 201:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'm')
      ADVANCE(191);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 202:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'n')
      ADVANCE(192);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 203:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'n')
      ADVANCE(213);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 204:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'n')
      ADVANCE(217);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 205:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'o')
      ADVANCE(206);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 206:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'o')
      ADVANCE(199);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 207:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'o')
      ADVANCE(188);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 208:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'r')
      ADVANCE(152);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 209:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 'r')
      ADVANCE(197);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 210:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 's')
      ADVANCE(214);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 211:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 't')
      ADVANCE(209);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 212:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 't')
      ADVANCE(190);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 213:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 't')
      ADVANCE(174);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 214:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 't')
      ADVANCE(159);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 215:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 't')
      ADVANCE(126);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 216:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 't')
      ADVANCE(150);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 217:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (lookahead == 't')
      ADVANCE(175);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 218:
    ACCEPT_TOKEN(aux_sym_type_identifier_token1);
    if (('0' <= lookahead && lookahead <= '9') ||
        ('A' <= lookahead && lookahead <= 'Z') ||
        lookahead == '_' ||
        ('a' <= lookahead && lookahead <= 'z'))
      ADVANCE(218);
    END_STATE();
  case 219:
    ACCEPT_TOKEN(sym_unit);
    END_STATE();
  default:
    return false;
  }
}

static const TSLexMode ts_lex_modes[STATE_COUNT] = {
    [0] = {.lex_state = 0},
    [1] = {.lex_state = 69},
    [2] = {.lex_state = 69},
    [3] = {.lex_state = 67},
    [4] = {.lex_state = 67},
    [5] = {.lex_state = 67},
    [6] = {.lex_state = 67},
    [7] = {.lex_state = 67},
    [8] = {.lex_state = 67},
    [9] = {.lex_state = 67},
    [10] = {.lex_state = 67},
    [11] = {.lex_state = 67},
    [12] = {.lex_state = 67},
    [13] = {.lex_state = 67},
    [14] = {.lex_state = 67},
    [15] = {.lex_state = 67},
    [16] = {.lex_state = 67},
    [17] = {.lex_state = 67},
    [18] = {.lex_state = 67},
    [19] = {.lex_state = 67},
    [20] = {.lex_state = 67},
    [21] = {.lex_state = 67},
    [22] = {.lex_state = 67},
    [23] = {.lex_state = 67},
    [24] = {.lex_state = 67},
    [25] = {.lex_state = 67},
    [26] = {.lex_state = 67},
    [27] = {.lex_state = 67},
    [28] = {.lex_state = 67},
    [29] = {.lex_state = 67},
    [30] = {.lex_state = 67},
    [31] = {.lex_state = 67},
    [32] = {.lex_state = 67},
    [33] = {.lex_state = 67},
    [34] = {.lex_state = 67},
    [35] = {.lex_state = 67},
    [36] = {.lex_state = 67},
    [37] = {.lex_state = 67},
    [38] = {.lex_state = 67},
    [39] = {.lex_state = 67},
    [40] = {.lex_state = 67},
    [41] = {.lex_state = 67},
    [42] = {.lex_state = 67},
    [43] = {.lex_state = 67},
    [44] = {.lex_state = 67},
    [45] = {.lex_state = 67},
    [46] = {.lex_state = 67},
    [47] = {.lex_state = 67},
    [48] = {.lex_state = 67},
    [49] = {.lex_state = 67},
    [50] = {.lex_state = 67},
    [51] = {.lex_state = 67},
    [52] = {.lex_state = 67},
    [53] = {.lex_state = 67},
    [54] = {.lex_state = 67},
    [55] = {.lex_state = 67},
    [56] = {.lex_state = 67},
    [57] = {.lex_state = 67},
    [58] = {.lex_state = 67},
    [59] = {.lex_state = 67},
    [60] = {.lex_state = 67},
    [61] = {.lex_state = 67},
    [62] = {.lex_state = 67},
    [63] = {.lex_state = 67},
    [64] = {.lex_state = 67},
    [65] = {.lex_state = 67},
    [66] = {.lex_state = 67},
    [67] = {.lex_state = 67},
    [68] = {.lex_state = 67},
    [69] = {.lex_state = 67},
    [70] = {.lex_state = 67},
    [71] = {.lex_state = 67},
    [72] = {.lex_state = 67},
    [73] = {.lex_state = 67},
    [74] = {.lex_state = 69},
    [75] = {.lex_state = 69},
    [76] = {.lex_state = 69},
    [77] = {.lex_state = 69},
    [78] = {.lex_state = 69},
    [79] = {.lex_state = 69},
    [80] = {.lex_state = 69},
    [81] = {.lex_state = 69},
    [82] = {.lex_state = 69},
    [83] = {.lex_state = 69},
    [84] = {.lex_state = 69},
    [85] = {.lex_state = 69},
    [86] = {.lex_state = 69},
    [87] = {.lex_state = 69},
    [88] = {.lex_state = 69},
    [89] = {.lex_state = 69},
    [90] = {.lex_state = 69},
    [91] = {.lex_state = 69},
    [92] = {.lex_state = 69},
    [93] = {.lex_state = 69},
    [94] = {.lex_state = 69},
    [95] = {.lex_state = 69},
    [96] = {.lex_state = 69},
    [97] = {.lex_state = 69},
    [98] = {.lex_state = 69},
    [99] = {.lex_state = 69},
    [100] = {.lex_state = 69},
    [101] = {.lex_state = 69},
    [102] = {.lex_state = 69},
    [103] = {.lex_state = 69},
    [104] = {.lex_state = 69},
    [105] = {.lex_state = 69},
    [106] = {.lex_state = 69},
    [107] = {.lex_state = 69},
    [108] = {.lex_state = 69},
    [109] = {.lex_state = 69},
    [110] = {.lex_state = 68},
    [111] = {.lex_state = 68},
    [112] = {.lex_state = 68},
    [113] = {.lex_state = 68},
    [114] = {.lex_state = 68},
    [115] = {.lex_state = 68},
    [116] = {.lex_state = 68},
    [117] = {.lex_state = 68},
    [118] = {.lex_state = 68},
    [119] = {.lex_state = 68},
    [120] = {.lex_state = 68},
    [121] = {.lex_state = 68},
    [122] = {.lex_state = 68},
    [123] = {.lex_state = 68},
    [124] = {.lex_state = 68},
    [125] = {.lex_state = 68},
    [126] = {.lex_state = 68},
    [127] = {.lex_state = 68},
    [128] = {.lex_state = 68},
    [129] = {.lex_state = 68},
    [130] = {.lex_state = 68},
    [131] = {.lex_state = 68},
    [132] = {.lex_state = 68},
    [133] = {.lex_state = 68},
    [134] = {.lex_state = 68},
    [135] = {.lex_state = 68},
    [136] = {.lex_state = 68},
    [137] = {.lex_state = 68},
    [138] = {.lex_state = 68},
    [139] = {.lex_state = 68},
    [140] = {.lex_state = 68},
    [141] = {.lex_state = 68},
    [142] = {.lex_state = 68},
    [143] = {.lex_state = 68},
    [144] = {.lex_state = 68},
    [145] = {.lex_state = 68},
    [146] = {.lex_state = 68},
    [147] = {.lex_state = 68},
    [148] = {.lex_state = 68},
    [149] = {.lex_state = 68},
    [150] = {.lex_state = 68},
    [151] = {.lex_state = 68},
    [152] = {.lex_state = 68},
    [153] = {.lex_state = 68},
    [154] = {.lex_state = 68},
    [155] = {.lex_state = 68},
    [156] = {.lex_state = 68},
    [157] = {.lex_state = 68},
    [158] = {.lex_state = 68},
    [159] = {.lex_state = 68},
    [160] = {.lex_state = 68},
    [161] = {.lex_state = 68},
    [162] = {.lex_state = 68},
    [163] = {.lex_state = 68},
    [164] = {.lex_state = 68},
    [165] = {.lex_state = 68},
    [166] = {.lex_state = 68},
    [167] = {.lex_state = 70},
    [168] = {.lex_state = 3},
    [169] = {.lex_state = 3},
    [170] = {.lex_state = 3},
    [171] = {.lex_state = 3},
    [172] = {.lex_state = 3},
    [173] = {.lex_state = 3},
    [174] = {.lex_state = 3},
    [175] = {.lex_state = 70},
    [176] = {.lex_state = 70},
    [177] = {.lex_state = 3},
    [178] = {.lex_state = 3},
    [179] = {.lex_state = 3},
    [180] = {.lex_state = 3},
    [181] = {.lex_state = 3},
    [182] = {.lex_state = 3},
    [183] = {.lex_state = 3},
    [184] = {.lex_state = 3},
    [185] = {.lex_state = 3},
    [186] = {.lex_state = 3},
    [187] = {.lex_state = 3},
    [188] = {.lex_state = 70},
    [189] = {.lex_state = 70},
    [190] = {.lex_state = 3},
    [191] = {.lex_state = 70},
    [192] = {.lex_state = 3},
    [193] = {.lex_state = 3},
    [194] = {.lex_state = 3},
    [195] = {.lex_state = 3},
    [196] = {.lex_state = 3},
    [197] = {.lex_state = 3},
    [198] = {.lex_state = 3},
    [199] = {.lex_state = 3},
    [200] = {.lex_state = 3},
    [201] = {.lex_state = 3},
    [202] = {.lex_state = 3},
    [203] = {.lex_state = 3},
    [204] = {.lex_state = 3},
    [205] = {.lex_state = 70},
    [206] = {.lex_state = 3},
    [207] = {.lex_state = 70},
    [208] = {.lex_state = 69},
    [209] = {.lex_state = 3},
    [210] = {.lex_state = 3},
    [211] = {.lex_state = 3},
    [212] = {.lex_state = 3},
    [213] = {.lex_state = 3},
    [214] = {.lex_state = 3},
    [215] = {.lex_state = 3},
    [216] = {.lex_state = 3},
    [217] = {.lex_state = 1},
    [218] = {.lex_state = 1},
    [219] = {.lex_state = 1},
    [220] = {.lex_state = 1},
    [221] = {.lex_state = 1},
    [222] = {.lex_state = 1},
    [223] = {.lex_state = 1},
    [224] = {.lex_state = 1},
    [225] = {.lex_state = 1},
    [226] = {.lex_state = 1},
    [227] = {.lex_state = 1},
    [228] = {.lex_state = 1},
    [229] = {.lex_state = 1},
    [230] = {.lex_state = 1},
    [231] = {.lex_state = 1},
    [232] = {.lex_state = 1},
    [233] = {.lex_state = 1},
    [234] = {.lex_state = 1},
    [235] = {.lex_state = 1},
    [236] = {.lex_state = 1},
    [237] = {.lex_state = 1},
    [238] = {.lex_state = 1},
    [239] = {.lex_state = 1},
    [240] = {.lex_state = 1},
    [241] = {.lex_state = 1},
    [242] = {.lex_state = 1},
    [243] = {.lex_state = 1},
    [244] = {.lex_state = 1},
    [245] = {.lex_state = 1},
    [246] = {.lex_state = 1},
    [247] = {.lex_state = 1},
    [248] = {.lex_state = 1},
    [249] = {.lex_state = 1},
    [250] = {.lex_state = 69},
    [251] = {.lex_state = 67},
    [252] = {.lex_state = 67},
    [253] = {.lex_state = 67},
    [254] = {.lex_state = 0},
    [255] = {.lex_state = 0},
    [256] = {.lex_state = 0},
    [257] = {.lex_state = 0},
    [258] = {.lex_state = 0},
    [259] = {.lex_state = 0},
    [260] = {.lex_state = 0},
    [261] = {.lex_state = 2},
    [262] = {.lex_state = 0},
    [263] = {.lex_state = 2},
    [264] = {.lex_state = 0},
    [265] = {.lex_state = 0},
    [266] = {.lex_state = 0},
    [267] = {.lex_state = 0},
    [268] = {.lex_state = 2},
    [269] = {.lex_state = 3},
    [270] = {.lex_state = 2},
    [271] = {.lex_state = 0},
    [272] = {.lex_state = 0},
    [273] = {.lex_state = 2},
    [274] = {.lex_state = 2},
    [275] = {.lex_state = 67},
    [276] = {.lex_state = 3},
    [277] = {.lex_state = 0},
    [278] = {.lex_state = 2},
    [279] = {.lex_state = 21},
    [280] = {.lex_state = 0},
    [281] = {.lex_state = 0},
    [282] = {.lex_state = 21},
    [283] = {.lex_state = 3},
    [284] = {.lex_state = 0},
    [285] = {.lex_state = 0},
    [286] = {.lex_state = 0},
    [287] = {.lex_state = 21},
    [288] = {.lex_state = 0},
    [289] = {.lex_state = 21},
    [290] = {.lex_state = 21},
    [291] = {.lex_state = 0},
    [292] = {.lex_state = 3},
    [293] = {.lex_state = 3},
    [294] = {.lex_state = 3},
    [295] = {.lex_state = 3},
    [296] = {.lex_state = 3},
    [297] = {.lex_state = 3},
    [298] = {.lex_state = 67},
    [299] = {.lex_state = 0},
    [300] = {.lex_state = 3},
    [301] = {.lex_state = 3},
    [302] = {.lex_state = 0},
    [303] = {.lex_state = 0},
    [304] = {.lex_state = 0},
    [305] = {.lex_state = 70},
    [306] = {.lex_state = 0},
    [307] = {.lex_state = 0},
    [308] = {.lex_state = 0},
    [309] = {.lex_state = 0},
    [310] = {.lex_state = 0},
    [311] = {.lex_state = 0},
    [312] = {.lex_state = 0},
    [313] = {.lex_state = 0},
    [314] = {.lex_state = 0},
    [315] = {.lex_state = 0},
    [316] = {.lex_state = 0},
    [317] = {.lex_state = 0},
    [318] = {.lex_state = 0},
    [319] = {.lex_state = 0},
    [320] = {.lex_state = 70},
    [321] = {.lex_state = 0},
    [322] = {.lex_state = 70},
    [323] = {.lex_state = 0},
    [324] = {.lex_state = 0},
    [325] = {.lex_state = 70},
    [326] = {.lex_state = 0},
    [327] = {.lex_state = 0},
    [328] = {.lex_state = 0},
    [329] = {.lex_state = 0},
    [330] = {.lex_state = 0},
    [331] = {.lex_state = 0},
    [332] = {.lex_state = 0},
    [333] = {.lex_state = 70},
    [334] = {.lex_state = 70},
    [335] = {.lex_state = 70},
    [336] = {.lex_state = 70},
    [337] = {.lex_state = 70},
    [338] = {.lex_state = 0},
    [339] = {.lex_state = 0},
    [340] = {.lex_state = 0},
};

static const uint16_t ts_parse_table[LARGE_STATE_COUNT][SYMBOL_COUNT] = {
    [0] = {
        [ts_builtin_sym_end] = ACTIONS(1),
        [anon_sym_COLON] = ACTIONS(1),
        [anon_sym_EQ] = ACTIONS(1),
        [anon_sym_LPAREN] = ACTIONS(1),
        [anon_sym_RPAREN] = ACTIONS(1),
        [anon_sym_type] = ACTIONS(1),
        [aux_sym_bool_literal_token1] = ACTIONS(1),
        [aux_sym_bool_literal_token2] = ACTIONS(1),
        [anon_sym_DQUOTE] = ACTIONS(1),
        [anon_sym_SQUOTE] = ACTIONS(1),
        [sym_char_or_string_escape_sequence] = ACTIONS(1),
        [anon_sym_CARET] = ACTIONS(1),
        [anon_sym_STAR] = ACTIONS(1),
        [anon_sym_SLASH] = ACTIONS(1),
        [anon_sym_PERCENT] = ACTIONS(1),
        [anon_sym_PLUS] = ACTIONS(1),
        [anon_sym_DASH] = ACTIONS(1),
        [anon_sym_EQ_EQ] = ACTIONS(1),
        [anon_sym_BANG_EQ] = ACTIONS(1),
        [anon_sym_LT] = ACTIONS(1),
        [anon_sym_LT_EQ] = ACTIONS(1),
        [anon_sym_GT] = ACTIONS(1),
        [anon_sym_GT_EQ] = ACTIONS(1),
        [anon_sym_AMP_AMP] = ACTIONS(1),
        [anon_sym_PIPE_PIPE] = ACTIONS(1),
        [anon_sym_y] = ACTIONS(1),
        [anon_sym_uy] = ACTIONS(1),
        [anon_sym_s] = ACTIONS(1),
        [anon_sym_us] = ACTIONS(1),
        [anon_sym_l] = ACTIONS(1),
        [anon_sym_ul] = ACTIONS(1),
        [anon_sym_L] = ACTIONS(1),
        [anon_sym_UL] = ACTIONS(1),
        [anon_sym_Q] = ACTIONS(1),
        [anon_sym_Z] = ACTIONS(1),
        [sym_negative_digits] = ACTIONS(1),
        [sym_positive_digits] = ACTIONS(1),
        [sym_float_literal] = ACTIONS(1),
        [anon_sym_LBRACK] = ACTIONS(1),
        [anon_sym_RBRACK] = ACTIONS(1),
        [anon_sym_SEMI] = ACTIONS(1),
        [anon_sym_COMMA] = ACTIONS(1),
        [aux_sym_builtin_type_token1] = ACTIONS(1),
        [aux_sym_builtin_type_token2] = ACTIONS(1),
        [aux_sym_builtin_type_token3] = ACTIONS(1),
        [aux_sym_builtin_type_token4] = ACTIONS(1),
        [aux_sym_builtin_type_token5] = ACTIONS(1),
        [aux_sym_builtin_type_token6] = ACTIONS(1),
        [aux_sym_builtin_type_token7] = ACTIONS(1),
        [aux_sym_builtin_type_token8] = ACTIONS(1),
        [aux_sym_builtin_type_token9] = ACTIONS(1),
        [aux_sym_builtin_type_token10] = ACTIONS(1),
        [aux_sym_builtin_type_token11] = ACTIONS(1),
        [aux_sym_builtin_type_token12] = ACTIONS(1),
        [aux_sym_builtin_type_token13] = ACTIONS(1),
        [aux_sym_builtin_type_token14] = ACTIONS(1),
        [aux_sym_builtin_type_token15] = ACTIONS(1),
        [aux_sym_builtin_type_token16] = ACTIONS(1),
        [aux_sym_builtin_type_token17] = ACTIONS(1),
        [anon_sym_DOT] = ACTIONS(1),
        [sym_unit] = ACTIONS(1),
    },
    [1] = {
        [sym_source_file] = STATE(323),
        [sym_fn_decl] = STATE(2),
        [sym_type_decl] = STATE(2),
        [sym_expression] = STATE(117),
        [sym_paren_expression] = STATE(116),
        [sym_bool_literal] = STATE(116),
        [sym_function_call] = STATE(116),
        [sym_let_expression] = STATE(116),
        [sym_string_literal] = STATE(116),
        [sym_char_literal] = STATE(116),
        [sym_infix_operation] = STATE(116),
        [sym_int8_literal] = STATE(116),
        [sym_uint8_literal] = STATE(116),
        [sym_int16_literal] = STATE(116),
        [sym_uint16_literal] = STATE(116),
        [sym_int32_literal] = STATE(116),
        [sym_uint32_literal] = STATE(116),
        [sym_int64_literal] = STATE(116),
        [sym_uint64_literal] = STATE(116),
        [sym_int128_literal] = STATE(116),
        [sym_uint128_literal] = STATE(116),
        [sym_digits] = STATE(266),
        [sym_list_literal] = STATE(116),
        [sym_tuple_literal] = STATE(116),
        [sym_variable_identifier] = STATE(116),
        [aux_sym_source_file_repeat1] = STATE(2),
        [aux_sym_source_file_repeat2] = STATE(13),
        [ts_builtin_sym_end] = ACTIONS(3),
        [anon_sym_let] = ACTIONS(5),
        [anon_sym_LPAREN] = ACTIONS(7),
        [anon_sym_type] = ACTIONS(9),
        [aux_sym_bool_literal_token1] = ACTIONS(11),
        [aux_sym_bool_literal_token2] = ACTIONS(11),
        [anon_sym_DQUOTE] = ACTIONS(13),
        [anon_sym_SQUOTE] = ACTIONS(15),
        [sym_negative_digits] = ACTIONS(17),
        [sym_positive_digits] = ACTIONS(19),
        [sym_float_literal] = ACTIONS(21),
        [anon_sym_LBRACK] = ACTIONS(23),
        [aux_sym_variable_identifier_token1] = ACTIONS(25),
        [sym_unit] = ACTIONS(21),
    },
};

static const uint16_t ts_small_parse_table[] = {
    [0] = 17,
    ACTIONS(5),
    1,
    anon_sym_let,
    ACTIONS(7),
    1,
    anon_sym_LPAREN,
    ACTIONS(9),
    1,
    anon_sym_type,
    ACTIONS(13),
    1,
    anon_sym_DQUOTE,
    ACTIONS(15),
    1,
    anon_sym_SQUOTE,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(19),
    1,
    sym_positive_digits,
    ACTIONS(23),
    1,
    anon_sym_LBRACK,
    ACTIONS(25),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(27),
    1,
    ts_builtin_sym_end,
    STATE(14),
    1,
    aux_sym_source_file_repeat2,
    STATE(117),
    1,
    sym_expression,
    STATE(266),
    1,
    sym_digits,
    ACTIONS(11),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(21),
    2,
    sym_float_literal,
    sym_unit,
    STATE(208),
    3,
    sym_fn_decl,
    sym_type_decl,
    aux_sym_source_file_repeat1,
    STATE(116),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [75] = 18,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(45),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(47),
    1,
    aux_sym_type_identifier_token1,
    STATE(27),
    1,
    sym_qualified_fn_name,
    STATE(214),
    1,
    sym_expression,
    STATE(251),
    1,
    sym_fn_identifier,
    STATE(265),
    1,
    sym_digits,
    STATE(269),
    1,
    aux_sym_qualified_fn_name_repeat1,
    STATE(309),
    1,
    sym_module_identifier,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [151] = 18,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(45),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(47),
    1,
    aux_sym_type_identifier_token1,
    STATE(28),
    1,
    sym_qualified_fn_name,
    STATE(215),
    1,
    sym_expression,
    STATE(251),
    1,
    sym_fn_identifier,
    STATE(265),
    1,
    sym_digits,
    STATE(269),
    1,
    aux_sym_qualified_fn_name_repeat1,
    STATE(309),
    1,
    sym_module_identifier,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [227] = 18,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(45),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(47),
    1,
    aux_sym_type_identifier_token1,
    STATE(24),
    1,
    sym_qualified_fn_name,
    STATE(212),
    1,
    sym_expression,
    STATE(251),
    1,
    sym_fn_identifier,
    STATE(265),
    1,
    sym_digits,
    STATE(269),
    1,
    aux_sym_qualified_fn_name_repeat1,
    STATE(309),
    1,
    sym_module_identifier,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [303] = 18,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(45),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(47),
    1,
    aux_sym_type_identifier_token1,
    STATE(22),
    1,
    sym_qualified_fn_name,
    STATE(211),
    1,
    sym_expression,
    STATE(251),
    1,
    sym_fn_identifier,
    STATE(265),
    1,
    sym_digits,
    STATE(269),
    1,
    aux_sym_qualified_fn_name_repeat1,
    STATE(309),
    1,
    sym_module_identifier,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [379] = 18,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(45),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(47),
    1,
    aux_sym_type_identifier_token1,
    STATE(26),
    1,
    sym_qualified_fn_name,
    STATE(213),
    1,
    sym_expression,
    STATE(251),
    1,
    sym_fn_identifier,
    STATE(265),
    1,
    sym_digits,
    STATE(269),
    1,
    aux_sym_qualified_fn_name_repeat1,
    STATE(309),
    1,
    sym_module_identifier,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [455] = 15,
    ACTIONS(49),
    1,
    ts_builtin_sym_end,
    ACTIONS(51),
    1,
    anon_sym_let,
    ACTIONS(54),
    1,
    anon_sym_LPAREN,
    ACTIONS(60),
    1,
    anon_sym_DQUOTE,
    ACTIONS(63),
    1,
    anon_sym_SQUOTE,
    ACTIONS(66),
    1,
    sym_negative_digits,
    ACTIONS(69),
    1,
    sym_positive_digits,
    ACTIONS(75),
    1,
    anon_sym_LBRACK,
    ACTIONS(78),
    1,
    aux_sym_variable_identifier_token1,
    STATE(8),
    1,
    aux_sym_source_file_repeat2,
    STATE(117),
    1,
    sym_expression,
    STATE(266),
    1,
    sym_digits,
    ACTIONS(57),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(72),
    2,
    sym_float_literal,
    sym_unit,
    STATE(116),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [522] = 15,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(85),
    1,
    anon_sym_RPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    STATE(10),
    1,
    aux_sym_source_file_repeat2,
    STATE(130),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [589] = 15,
    ACTIONS(49),
    1,
    anon_sym_RPAREN,
    ACTIONS(66),
    1,
    sym_negative_digits,
    ACTIONS(101),
    1,
    anon_sym_let,
    ACTIONS(104),
    1,
    anon_sym_LPAREN,
    ACTIONS(110),
    1,
    anon_sym_DQUOTE,
    ACTIONS(113),
    1,
    anon_sym_SQUOTE,
    ACTIONS(116),
    1,
    sym_positive_digits,
    ACTIONS(122),
    1,
    anon_sym_LBRACK,
    ACTIONS(125),
    1,
    aux_sym_variable_identifier_token1,
    STATE(10),
    1,
    aux_sym_source_file_repeat2,
    STATE(130),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(107),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(119),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [656] = 15,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(128),
    1,
    anon_sym_RPAREN,
    STATE(10),
    1,
    aux_sym_source_file_repeat2,
    STATE(130),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [723] = 15,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(130),
    1,
    anon_sym_RBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    STATE(209),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    STATE(327),
    1,
    sym_list_content,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [790] = 15,
    ACTIONS(7),
    1,
    anon_sym_LPAREN,
    ACTIONS(13),
    1,
    anon_sym_DQUOTE,
    ACTIONS(15),
    1,
    anon_sym_SQUOTE,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(19),
    1,
    sym_positive_digits,
    ACTIONS(23),
    1,
    anon_sym_LBRACK,
    ACTIONS(25),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(27),
    1,
    ts_builtin_sym_end,
    ACTIONS(134),
    1,
    anon_sym_let,
    STATE(8),
    1,
    aux_sym_source_file_repeat2,
    STATE(117),
    1,
    sym_expression,
    STATE(266),
    1,
    sym_digits,
    ACTIONS(11),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(21),
    2,
    sym_float_literal,
    sym_unit,
    STATE(116),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [857] = 15,
    ACTIONS(7),
    1,
    anon_sym_LPAREN,
    ACTIONS(13),
    1,
    anon_sym_DQUOTE,
    ACTIONS(15),
    1,
    anon_sym_SQUOTE,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(19),
    1,
    sym_positive_digits,
    ACTIONS(23),
    1,
    anon_sym_LBRACK,
    ACTIONS(25),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(134),
    1,
    anon_sym_let,
    ACTIONS(136),
    1,
    ts_builtin_sym_end,
    STATE(8),
    1,
    aux_sym_source_file_repeat2,
    STATE(117),
    1,
    sym_expression,
    STATE(266),
    1,
    sym_digits,
    ACTIONS(11),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(21),
    2,
    sym_float_literal,
    sym_unit,
    STATE(116),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [924] = 15,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(138),
    1,
    anon_sym_RBRACK,
    STATE(209),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    STATE(310),
    1,
    sym_list_content,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [991] = 15,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(140),
    1,
    anon_sym_RPAREN,
    STATE(10),
    1,
    aux_sym_source_file_repeat2,
    STATE(130),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [1058] = 15,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(142),
    1,
    anon_sym_RBRACK,
    STATE(209),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    STATE(318),
    1,
    sym_list_content,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [1125] = 15,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(144),
    1,
    anon_sym_RPAREN,
    STATE(10),
    1,
    aux_sym_source_file_repeat2,
    STATE(130),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [1192] = 15,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(146),
    1,
    anon_sym_RBRACK,
    STATE(209),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    STATE(308),
    1,
    sym_list_content,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [1259] = 15,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(148),
    1,
    anon_sym_RPAREN,
    STATE(10),
    1,
    aux_sym_source_file_repeat2,
    STATE(130),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [1326] = 15,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(150),
    1,
    anon_sym_RBRACK,
    STATE(209),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    STATE(338),
    1,
    sym_list_content,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [1393] = 14,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    STATE(18),
    1,
    aux_sym_source_file_repeat2,
    STATE(130),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [1457] = 14,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(152),
    1,
    anon_sym_RBRACK,
    STATE(210),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [1521] = 14,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    STATE(20),
    1,
    aux_sym_source_file_repeat2,
    STATE(130),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [1585] = 14,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(154),
    1,
    anon_sym_RBRACK,
    STATE(210),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [1649] = 14,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    STATE(9),
    1,
    aux_sym_source_file_repeat2,
    STATE(130),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [1713] = 14,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    STATE(16),
    1,
    aux_sym_source_file_repeat2,
    STATE(130),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [1777] = 14,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    STATE(11),
    1,
    aux_sym_source_file_repeat2,
    STATE(130),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [1841] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(156),
    1,
    anon_sym_let,
    ACTIONS(158),
    1,
    anon_sym_LPAREN,
    ACTIONS(162),
    1,
    anon_sym_DQUOTE,
    ACTIONS(164),
    1,
    anon_sym_SQUOTE,
    ACTIONS(166),
    1,
    sym_positive_digits,
    ACTIONS(170),
    1,
    anon_sym_LBRACK,
    ACTIONS(172),
    1,
    aux_sym_variable_identifier_token1,
    STATE(222),
    1,
    sym_expression,
    STATE(262),
    1,
    sym_digits,
    ACTIONS(160),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(168),
    2,
    sym_float_literal,
    sym_unit,
    STATE(240),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [1902] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    STATE(141),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [1963] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    STATE(190),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2024] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(156),
    1,
    anon_sym_let,
    ACTIONS(158),
    1,
    anon_sym_LPAREN,
    ACTIONS(162),
    1,
    anon_sym_DQUOTE,
    ACTIONS(164),
    1,
    anon_sym_SQUOTE,
    ACTIONS(166),
    1,
    sym_positive_digits,
    ACTIONS(170),
    1,
    anon_sym_LBRACK,
    ACTIONS(172),
    1,
    aux_sym_variable_identifier_token1,
    STATE(238),
    1,
    sym_expression,
    STATE(262),
    1,
    sym_digits,
    ACTIONS(160),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(168),
    2,
    sym_float_literal,
    sym_unit,
    STATE(240),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2085] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    STATE(210),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2146] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    STATE(178),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2207] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    STATE(177),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2268] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(174),
    1,
    anon_sym_let,
    ACTIONS(176),
    1,
    anon_sym_LPAREN,
    ACTIONS(180),
    1,
    anon_sym_DQUOTE,
    ACTIONS(182),
    1,
    anon_sym_SQUOTE,
    ACTIONS(184),
    1,
    sym_positive_digits,
    ACTIONS(188),
    1,
    anon_sym_LBRACK,
    ACTIONS(190),
    1,
    aux_sym_variable_identifier_token1,
    STATE(107),
    1,
    sym_expression,
    STATE(271),
    1,
    sym_digits,
    ACTIONS(178),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(186),
    2,
    sym_float_literal,
    sym_unit,
    STATE(77),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2329] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    STATE(192),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2390] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    STATE(216),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2451] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    STATE(172),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2512] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    STATE(171),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2573] = 13,
    ACTIONS(7),
    1,
    anon_sym_LPAREN,
    ACTIONS(13),
    1,
    anon_sym_DQUOTE,
    ACTIONS(15),
    1,
    anon_sym_SQUOTE,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(19),
    1,
    sym_positive_digits,
    ACTIONS(23),
    1,
    anon_sym_LBRACK,
    ACTIONS(25),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(134),
    1,
    anon_sym_let,
    STATE(112),
    1,
    sym_expression,
    STATE(266),
    1,
    sym_digits,
    ACTIONS(11),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(21),
    2,
    sym_float_literal,
    sym_unit,
    STATE(116),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2634] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(156),
    1,
    anon_sym_let,
    ACTIONS(158),
    1,
    anon_sym_LPAREN,
    ACTIONS(162),
    1,
    anon_sym_DQUOTE,
    ACTIONS(164),
    1,
    anon_sym_SQUOTE,
    ACTIONS(166),
    1,
    sym_positive_digits,
    ACTIONS(170),
    1,
    anon_sym_LBRACK,
    ACTIONS(172),
    1,
    aux_sym_variable_identifier_token1,
    STATE(225),
    1,
    sym_expression,
    STATE(262),
    1,
    sym_digits,
    ACTIONS(160),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(168),
    2,
    sym_float_literal,
    sym_unit,
    STATE(240),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2695] = 13,
    ACTIONS(7),
    1,
    anon_sym_LPAREN,
    ACTIONS(13),
    1,
    anon_sym_DQUOTE,
    ACTIONS(15),
    1,
    anon_sym_SQUOTE,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(19),
    1,
    sym_positive_digits,
    ACTIONS(23),
    1,
    anon_sym_LBRACK,
    ACTIONS(25),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(134),
    1,
    anon_sym_let,
    STATE(121),
    1,
    sym_expression,
    STATE(266),
    1,
    sym_digits,
    ACTIONS(11),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(21),
    2,
    sym_float_literal,
    sym_unit,
    STATE(116),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2756] = 13,
    ACTIONS(7),
    1,
    anon_sym_LPAREN,
    ACTIONS(13),
    1,
    anon_sym_DQUOTE,
    ACTIONS(15),
    1,
    anon_sym_SQUOTE,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(19),
    1,
    sym_positive_digits,
    ACTIONS(23),
    1,
    anon_sym_LBRACK,
    ACTIONS(25),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(134),
    1,
    anon_sym_let,
    STATE(120),
    1,
    sym_expression,
    STATE(266),
    1,
    sym_digits,
    ACTIONS(11),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(21),
    2,
    sym_float_literal,
    sym_unit,
    STATE(116),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2817] = 13,
    ACTIONS(7),
    1,
    anon_sym_LPAREN,
    ACTIONS(13),
    1,
    anon_sym_DQUOTE,
    ACTIONS(15),
    1,
    anon_sym_SQUOTE,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(19),
    1,
    sym_positive_digits,
    ACTIONS(23),
    1,
    anon_sym_LBRACK,
    ACTIONS(25),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(134),
    1,
    anon_sym_let,
    STATE(119),
    1,
    sym_expression,
    STATE(266),
    1,
    sym_digits,
    ACTIONS(11),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(21),
    2,
    sym_float_literal,
    sym_unit,
    STATE(116),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2878] = 13,
    ACTIONS(7),
    1,
    anon_sym_LPAREN,
    ACTIONS(13),
    1,
    anon_sym_DQUOTE,
    ACTIONS(15),
    1,
    anon_sym_SQUOTE,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(19),
    1,
    sym_positive_digits,
    ACTIONS(23),
    1,
    anon_sym_LBRACK,
    ACTIONS(25),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(134),
    1,
    anon_sym_let,
    STATE(118),
    1,
    sym_expression,
    STATE(266),
    1,
    sym_digits,
    ACTIONS(11),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(21),
    2,
    sym_float_literal,
    sym_unit,
    STATE(116),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [2939] = 13,
    ACTIONS(7),
    1,
    anon_sym_LPAREN,
    ACTIONS(13),
    1,
    anon_sym_DQUOTE,
    ACTIONS(15),
    1,
    anon_sym_SQUOTE,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(19),
    1,
    sym_positive_digits,
    ACTIONS(23),
    1,
    anon_sym_LBRACK,
    ACTIONS(25),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(134),
    1,
    anon_sym_let,
    STATE(114),
    1,
    sym_expression,
    STATE(266),
    1,
    sym_digits,
    ACTIONS(11),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(21),
    2,
    sym_float_literal,
    sym_unit,
    STATE(116),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3000] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(174),
    1,
    anon_sym_let,
    ACTIONS(176),
    1,
    anon_sym_LPAREN,
    ACTIONS(180),
    1,
    anon_sym_DQUOTE,
    ACTIONS(182),
    1,
    anon_sym_SQUOTE,
    ACTIONS(184),
    1,
    sym_positive_digits,
    ACTIONS(188),
    1,
    anon_sym_LBRACK,
    ACTIONS(190),
    1,
    aux_sym_variable_identifier_token1,
    STATE(102),
    1,
    sym_expression,
    STATE(271),
    1,
    sym_digits,
    ACTIONS(178),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(186),
    2,
    sym_float_literal,
    sym_unit,
    STATE(77),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3061] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    STATE(206),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3122] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(156),
    1,
    anon_sym_let,
    ACTIONS(158),
    1,
    anon_sym_LPAREN,
    ACTIONS(162),
    1,
    anon_sym_DQUOTE,
    ACTIONS(164),
    1,
    anon_sym_SQUOTE,
    ACTIONS(166),
    1,
    sym_positive_digits,
    ACTIONS(170),
    1,
    anon_sym_LBRACK,
    ACTIONS(172),
    1,
    aux_sym_variable_identifier_token1,
    STATE(224),
    1,
    sym_expression,
    STATE(262),
    1,
    sym_digits,
    ACTIONS(160),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(168),
    2,
    sym_float_literal,
    sym_unit,
    STATE(240),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3183] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    STATE(143),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3244] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(156),
    1,
    anon_sym_let,
    ACTIONS(158),
    1,
    anon_sym_LPAREN,
    ACTIONS(162),
    1,
    anon_sym_DQUOTE,
    ACTIONS(164),
    1,
    anon_sym_SQUOTE,
    ACTIONS(166),
    1,
    sym_positive_digits,
    ACTIONS(170),
    1,
    anon_sym_LBRACK,
    ACTIONS(172),
    1,
    aux_sym_variable_identifier_token1,
    STATE(223),
    1,
    sym_expression,
    STATE(262),
    1,
    sym_digits,
    ACTIONS(160),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(168),
    2,
    sym_float_literal,
    sym_unit,
    STATE(240),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3305] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    STATE(111),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3366] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    STATE(193),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3427] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(174),
    1,
    anon_sym_let,
    ACTIONS(176),
    1,
    anon_sym_LPAREN,
    ACTIONS(180),
    1,
    anon_sym_DQUOTE,
    ACTIONS(182),
    1,
    anon_sym_SQUOTE,
    ACTIONS(184),
    1,
    sym_positive_digits,
    ACTIONS(188),
    1,
    anon_sym_LBRACK,
    ACTIONS(190),
    1,
    aux_sym_variable_identifier_token1,
    STATE(108),
    1,
    sym_expression,
    STATE(271),
    1,
    sym_digits,
    ACTIONS(178),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(186),
    2,
    sym_float_literal,
    sym_unit,
    STATE(77),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3488] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(156),
    1,
    anon_sym_let,
    ACTIONS(158),
    1,
    anon_sym_LPAREN,
    ACTIONS(162),
    1,
    anon_sym_DQUOTE,
    ACTIONS(164),
    1,
    anon_sym_SQUOTE,
    ACTIONS(166),
    1,
    sym_positive_digits,
    ACTIONS(170),
    1,
    anon_sym_LBRACK,
    ACTIONS(172),
    1,
    aux_sym_variable_identifier_token1,
    STATE(227),
    1,
    sym_expression,
    STATE(262),
    1,
    sym_digits,
    ACTIONS(160),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(168),
    2,
    sym_float_literal,
    sym_unit,
    STATE(240),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3549] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    STATE(140),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3610] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    STATE(139),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3671] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(174),
    1,
    anon_sym_let,
    ACTIONS(176),
    1,
    anon_sym_LPAREN,
    ACTIONS(180),
    1,
    anon_sym_DQUOTE,
    ACTIONS(182),
    1,
    anon_sym_SQUOTE,
    ACTIONS(184),
    1,
    sym_positive_digits,
    ACTIONS(188),
    1,
    anon_sym_LBRACK,
    ACTIONS(190),
    1,
    aux_sym_variable_identifier_token1,
    STATE(101),
    1,
    sym_expression,
    STATE(271),
    1,
    sym_digits,
    ACTIONS(178),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(186),
    2,
    sym_float_literal,
    sym_unit,
    STATE(77),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3732] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(156),
    1,
    anon_sym_let,
    ACTIONS(158),
    1,
    anon_sym_LPAREN,
    ACTIONS(162),
    1,
    anon_sym_DQUOTE,
    ACTIONS(164),
    1,
    anon_sym_SQUOTE,
    ACTIONS(166),
    1,
    sym_positive_digits,
    ACTIONS(170),
    1,
    anon_sym_LBRACK,
    ACTIONS(172),
    1,
    aux_sym_variable_identifier_token1,
    STATE(231),
    1,
    sym_expression,
    STATE(262),
    1,
    sym_digits,
    ACTIONS(160),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(168),
    2,
    sym_float_literal,
    sym_unit,
    STATE(240),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3793] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    STATE(187),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3854] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    STATE(203),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3915] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(81),
    1,
    anon_sym_let,
    ACTIONS(83),
    1,
    anon_sym_LPAREN,
    ACTIONS(89),
    1,
    anon_sym_DQUOTE,
    ACTIONS(91),
    1,
    anon_sym_SQUOTE,
    ACTIONS(93),
    1,
    sym_positive_digits,
    ACTIONS(97),
    1,
    anon_sym_LBRACK,
    ACTIONS(99),
    1,
    aux_sym_variable_identifier_token1,
    STATE(135),
    1,
    sym_expression,
    STATE(272),
    1,
    sym_digits,
    ACTIONS(87),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(95),
    2,
    sym_float_literal,
    sym_unit,
    STATE(164),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [3976] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(174),
    1,
    anon_sym_let,
    ACTIONS(176),
    1,
    anon_sym_LPAREN,
    ACTIONS(180),
    1,
    anon_sym_DQUOTE,
    ACTIONS(182),
    1,
    anon_sym_SQUOTE,
    ACTIONS(184),
    1,
    sym_positive_digits,
    ACTIONS(188),
    1,
    anon_sym_LBRACK,
    ACTIONS(190),
    1,
    aux_sym_variable_identifier_token1,
    STATE(100),
    1,
    sym_expression,
    STATE(271),
    1,
    sym_digits,
    ACTIONS(178),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(186),
    2,
    sym_float_literal,
    sym_unit,
    STATE(77),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [4037] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(174),
    1,
    anon_sym_let,
    ACTIONS(176),
    1,
    anon_sym_LPAREN,
    ACTIONS(180),
    1,
    anon_sym_DQUOTE,
    ACTIONS(182),
    1,
    anon_sym_SQUOTE,
    ACTIONS(184),
    1,
    sym_positive_digits,
    ACTIONS(188),
    1,
    anon_sym_LBRACK,
    ACTIONS(190),
    1,
    aux_sym_variable_identifier_token1,
    STATE(99),
    1,
    sym_expression,
    STATE(271),
    1,
    sym_digits,
    ACTIONS(178),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(186),
    2,
    sym_float_literal,
    sym_unit,
    STATE(77),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [4098] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(174),
    1,
    anon_sym_let,
    ACTIONS(176),
    1,
    anon_sym_LPAREN,
    ACTIONS(180),
    1,
    anon_sym_DQUOTE,
    ACTIONS(182),
    1,
    anon_sym_SQUOTE,
    ACTIONS(184),
    1,
    sym_positive_digits,
    ACTIONS(188),
    1,
    anon_sym_LBRACK,
    ACTIONS(190),
    1,
    aux_sym_variable_identifier_token1,
    STATE(97),
    1,
    sym_expression,
    STATE(271),
    1,
    sym_digits,
    ACTIONS(178),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(186),
    2,
    sym_float_literal,
    sym_unit,
    STATE(77),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [4159] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(156),
    1,
    anon_sym_let,
    ACTIONS(158),
    1,
    anon_sym_LPAREN,
    ACTIONS(162),
    1,
    anon_sym_DQUOTE,
    ACTIONS(164),
    1,
    anon_sym_SQUOTE,
    ACTIONS(166),
    1,
    sym_positive_digits,
    ACTIONS(170),
    1,
    anon_sym_LBRACK,
    ACTIONS(172),
    1,
    aux_sym_variable_identifier_token1,
    STATE(221),
    1,
    sym_expression,
    STATE(262),
    1,
    sym_digits,
    ACTIONS(160),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(168),
    2,
    sym_float_literal,
    sym_unit,
    STATE(240),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [4220] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(29),
    1,
    anon_sym_let,
    ACTIONS(31),
    1,
    anon_sym_LPAREN,
    ACTIONS(35),
    1,
    anon_sym_DQUOTE,
    ACTIONS(37),
    1,
    anon_sym_SQUOTE,
    ACTIONS(39),
    1,
    sym_positive_digits,
    ACTIONS(43),
    1,
    anon_sym_LBRACK,
    ACTIONS(132),
    1,
    aux_sym_variable_identifier_token1,
    STATE(173),
    1,
    sym_expression,
    STATE(265),
    1,
    sym_digits,
    ACTIONS(33),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(41),
    2,
    sym_float_literal,
    sym_unit,
    STATE(201),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [4281] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(156),
    1,
    anon_sym_let,
    ACTIONS(158),
    1,
    anon_sym_LPAREN,
    ACTIONS(162),
    1,
    anon_sym_DQUOTE,
    ACTIONS(164),
    1,
    anon_sym_SQUOTE,
    ACTIONS(166),
    1,
    sym_positive_digits,
    ACTIONS(170),
    1,
    anon_sym_LBRACK,
    ACTIONS(172),
    1,
    aux_sym_variable_identifier_token1,
    STATE(244),
    1,
    sym_expression,
    STATE(262),
    1,
    sym_digits,
    ACTIONS(160),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(168),
    2,
    sym_float_literal,
    sym_unit,
    STATE(240),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [4342] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(156),
    1,
    anon_sym_let,
    ACTIONS(158),
    1,
    anon_sym_LPAREN,
    ACTIONS(162),
    1,
    anon_sym_DQUOTE,
    ACTIONS(164),
    1,
    anon_sym_SQUOTE,
    ACTIONS(166),
    1,
    sym_positive_digits,
    ACTIONS(170),
    1,
    anon_sym_LBRACK,
    ACTIONS(172),
    1,
    aux_sym_variable_identifier_token1,
    STATE(217),
    1,
    sym_expression,
    STATE(262),
    1,
    sym_digits,
    ACTIONS(160),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(168),
    2,
    sym_float_literal,
    sym_unit,
    STATE(240),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [4403] = 13,
    ACTIONS(17),
    1,
    sym_negative_digits,
    ACTIONS(156),
    1,
    anon_sym_let,
    ACTIONS(158),
    1,
    anon_sym_LPAREN,
    ACTIONS(162),
    1,
    anon_sym_DQUOTE,
    ACTIONS(164),
    1,
    anon_sym_SQUOTE,
    ACTIONS(166),
    1,
    sym_positive_digits,
    ACTIONS(170),
    1,
    anon_sym_LBRACK,
    ACTIONS(172),
    1,
    aux_sym_variable_identifier_token1,
    STATE(229),
    1,
    sym_expression,
    STATE(262),
    1,
    sym_digits,
    ACTIONS(160),
    2,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    ACTIONS(168),
    2,
    sym_float_literal,
    sym_unit,
    STATE(240),
    20,
    sym_paren_expression,
    sym_bool_literal,
    sym_function_call,
    sym_let_expression,
    sym_string_literal,
    sym_char_literal,
    sym_infix_operation,
    sym_int8_literal,
    sym_uint8_literal,
    sym_int16_literal,
    sym_uint16_literal,
    sym_int32_literal,
    sym_uint32_literal,
    sym_int64_literal,
    sym_uint64_literal,
    sym_int128_literal,
    sym_uint128_literal,
    sym_list_literal,
    sym_tuple_literal,
    sym_variable_identifier,
    [4464] = 2,
    ACTIONS(194),
    12,
    anon_sym_let,
    anon_sym_EQ,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(192),
    17,
    ts_builtin_sym_end,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [4498] = 4,
    ACTIONS(194),
    5,
    anon_sym_EQ,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(198),
    5,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    ACTIONS(196),
    7,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(192),
    12,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_COMMA,
    [4536] = 2,
    ACTIONS(202),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(200),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [4569] = 2,
    ACTIONS(206),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(204),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [4602] = 2,
    ACTIONS(194),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(192),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [4635] = 2,
    ACTIONS(210),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(208),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [4668] = 2,
    ACTIONS(214),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(212),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [4701] = 2,
    ACTIONS(218),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(216),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [4734] = 2,
    ACTIONS(222),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(220),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [4767] = 2,
    ACTIONS(226),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(224),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [4800] = 10,
    ACTIONS(228),
    1,
    anon_sym_LPAREN,
    ACTIONS(232),
    1,
    anon_sym_List,
    ACTIONS(234),
    1,
    aux_sym_type_identifier_token1,
    STATE(175),
    1,
    sym_type_identifier,
    STATE(275),
    1,
    aux_sym_qualified_fn_name_repeat1,
    STATE(309),
    1,
    sym_module_identifier,
    STATE(322),
    1,
    sym_type_reference,
    STATE(176),
    2,
    sym_builtin_type,
    sym_qualified_type_name,
    STATE(207),
    2,
    sym_list_type_reference,
    sym_tuple_type_reference,
    ACTIONS(230),
    17,
    aux_sym_builtin_type_token1,
    aux_sym_builtin_type_token2,
    aux_sym_builtin_type_token3,
    aux_sym_builtin_type_token4,
    aux_sym_builtin_type_token5,
    aux_sym_builtin_type_token6,
    aux_sym_builtin_type_token7,
    aux_sym_builtin_type_token8,
    aux_sym_builtin_type_token9,
    aux_sym_builtin_type_token10,
    aux_sym_builtin_type_token11,
    aux_sym_builtin_type_token12,
    aux_sym_builtin_type_token13,
    aux_sym_builtin_type_token14,
    aux_sym_builtin_type_token15,
    aux_sym_builtin_type_token16,
    aux_sym_builtin_type_token17,
    [4849] = 2,
    ACTIONS(238),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(236),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [4882] = 2,
    ACTIONS(242),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(240),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [4915] = 2,
    ACTIONS(246),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(244),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [4948] = 10,
    ACTIONS(228),
    1,
    anon_sym_LPAREN,
    ACTIONS(232),
    1,
    anon_sym_List,
    ACTIONS(234),
    1,
    aux_sym_type_identifier_token1,
    STATE(175),
    1,
    sym_type_identifier,
    STATE(275),
    1,
    aux_sym_qualified_fn_name_repeat1,
    STATE(309),
    1,
    sym_module_identifier,
    STATE(329),
    1,
    sym_type_reference,
    STATE(176),
    2,
    sym_builtin_type,
    sym_qualified_type_name,
    STATE(207),
    2,
    sym_list_type_reference,
    sym_tuple_type_reference,
    ACTIONS(230),
    17,
    aux_sym_builtin_type_token1,
    aux_sym_builtin_type_token2,
    aux_sym_builtin_type_token3,
    aux_sym_builtin_type_token4,
    aux_sym_builtin_type_token5,
    aux_sym_builtin_type_token6,
    aux_sym_builtin_type_token7,
    aux_sym_builtin_type_token8,
    aux_sym_builtin_type_token9,
    aux_sym_builtin_type_token10,
    aux_sym_builtin_type_token11,
    aux_sym_builtin_type_token12,
    aux_sym_builtin_type_token13,
    aux_sym_builtin_type_token14,
    aux_sym_builtin_type_token15,
    aux_sym_builtin_type_token16,
    aux_sym_builtin_type_token17,
    [4997] = 2,
    ACTIONS(250),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(248),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5030] = 10,
    ACTIONS(228),
    1,
    anon_sym_LPAREN,
    ACTIONS(232),
    1,
    anon_sym_List,
    ACTIONS(234),
    1,
    aux_sym_type_identifier_token1,
    STATE(175),
    1,
    sym_type_identifier,
    STATE(250),
    1,
    sym_type_reference,
    STATE(275),
    1,
    aux_sym_qualified_fn_name_repeat1,
    STATE(309),
    1,
    sym_module_identifier,
    STATE(176),
    2,
    sym_builtin_type,
    sym_qualified_type_name,
    STATE(207),
    2,
    sym_list_type_reference,
    sym_tuple_type_reference,
    ACTIONS(230),
    17,
    aux_sym_builtin_type_token1,
    aux_sym_builtin_type_token2,
    aux_sym_builtin_type_token3,
    aux_sym_builtin_type_token4,
    aux_sym_builtin_type_token5,
    aux_sym_builtin_type_token6,
    aux_sym_builtin_type_token7,
    aux_sym_builtin_type_token8,
    aux_sym_builtin_type_token9,
    aux_sym_builtin_type_token10,
    aux_sym_builtin_type_token11,
    aux_sym_builtin_type_token12,
    aux_sym_builtin_type_token13,
    aux_sym_builtin_type_token14,
    aux_sym_builtin_type_token15,
    aux_sym_builtin_type_token16,
    aux_sym_builtin_type_token17,
    [5079] = 2,
    ACTIONS(254),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(252),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5112] = 2,
    ACTIONS(258),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(256),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5145] = 2,
    ACTIONS(262),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(260),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5178] = 2,
    ACTIONS(266),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(264),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5211] = 2,
    ACTIONS(270),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(268),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5244] = 2,
    ACTIONS(274),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(272),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5277] = 2,
    ACTIONS(278),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(276),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5310] = 2,
    ACTIONS(282),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(280),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5343] = 3,
    ACTIONS(288),
    1,
    anon_sym_CARET,
    ACTIONS(286),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(284),
    15,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5378] = 10,
    ACTIONS(228),
    1,
    anon_sym_LPAREN,
    ACTIONS(232),
    1,
    anon_sym_List,
    ACTIONS(234),
    1,
    aux_sym_type_identifier_token1,
    STATE(175),
    1,
    sym_type_identifier,
    STATE(275),
    1,
    aux_sym_qualified_fn_name_repeat1,
    STATE(309),
    1,
    sym_module_identifier,
    STATE(313),
    1,
    sym_type_reference,
    STATE(176),
    2,
    sym_builtin_type,
    sym_qualified_type_name,
    STATE(207),
    2,
    sym_list_type_reference,
    sym_tuple_type_reference,
    ACTIONS(230),
    17,
    aux_sym_builtin_type_token1,
    aux_sym_builtin_type_token2,
    aux_sym_builtin_type_token3,
    aux_sym_builtin_type_token4,
    aux_sym_builtin_type_token5,
    aux_sym_builtin_type_token6,
    aux_sym_builtin_type_token7,
    aux_sym_builtin_type_token8,
    aux_sym_builtin_type_token9,
    aux_sym_builtin_type_token10,
    aux_sym_builtin_type_token11,
    aux_sym_builtin_type_token12,
    aux_sym_builtin_type_token13,
    aux_sym_builtin_type_token14,
    aux_sym_builtin_type_token15,
    aux_sym_builtin_type_token16,
    aux_sym_builtin_type_token17,
    [5427] = 3,
    ACTIONS(288),
    1,
    anon_sym_CARET,
    ACTIONS(286),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(284),
    15,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5462] = 4,
    ACTIONS(288),
    1,
    anon_sym_CARET,
    ACTIONS(290),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(284),
    12,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    ACTIONS(286),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    [5499] = 5,
    ACTIONS(288),
    1,
    anon_sym_CARET,
    ACTIONS(292),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(290),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(286),
    10,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(284),
    12,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5538] = 7,
    ACTIONS(288),
    1,
    anon_sym_CARET,
    ACTIONS(292),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(296),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(290),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(294),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    ACTIONS(284),
    8,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    ACTIONS(286),
    8,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    [5581] = 2,
    ACTIONS(300),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(298),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5614] = 2,
    ACTIONS(304),
    12,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(302),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5647] = 10,
    ACTIONS(228),
    1,
    anon_sym_LPAREN,
    ACTIONS(232),
    1,
    anon_sym_List,
    ACTIONS(234),
    1,
    aux_sym_type_identifier_token1,
    STATE(175),
    1,
    sym_type_identifier,
    STATE(275),
    1,
    aux_sym_qualified_fn_name_repeat1,
    STATE(309),
    1,
    sym_module_identifier,
    STATE(325),
    1,
    sym_type_reference,
    STATE(176),
    2,
    sym_builtin_type,
    sym_qualified_type_name,
    STATE(207),
    2,
    sym_list_type_reference,
    sym_tuple_type_reference,
    ACTIONS(230),
    17,
    aux_sym_builtin_type_token1,
    aux_sym_builtin_type_token2,
    aux_sym_builtin_type_token3,
    aux_sym_builtin_type_token4,
    aux_sym_builtin_type_token5,
    aux_sym_builtin_type_token6,
    aux_sym_builtin_type_token7,
    aux_sym_builtin_type_token8,
    aux_sym_builtin_type_token9,
    aux_sym_builtin_type_token10,
    aux_sym_builtin_type_token11,
    aux_sym_builtin_type_token12,
    aux_sym_builtin_type_token13,
    aux_sym_builtin_type_token14,
    aux_sym_builtin_type_token15,
    aux_sym_builtin_type_token16,
    aux_sym_builtin_type_token17,
    [5696] = 10,
    ACTIONS(228),
    1,
    anon_sym_LPAREN,
    ACTIONS(232),
    1,
    anon_sym_List,
    ACTIONS(234),
    1,
    aux_sym_type_identifier_token1,
    STATE(175),
    1,
    sym_type_identifier,
    STATE(275),
    1,
    aux_sym_qualified_fn_name_repeat1,
    STATE(277),
    1,
    sym_type_reference,
    STATE(309),
    1,
    sym_module_identifier,
    STATE(176),
    2,
    sym_builtin_type,
    sym_qualified_type_name,
    STATE(207),
    2,
    sym_list_type_reference,
    sym_tuple_type_reference,
    ACTIONS(230),
    17,
    aux_sym_builtin_type_token1,
    aux_sym_builtin_type_token2,
    aux_sym_builtin_type_token3,
    aux_sym_builtin_type_token4,
    aux_sym_builtin_type_token5,
    aux_sym_builtin_type_token6,
    aux_sym_builtin_type_token7,
    aux_sym_builtin_type_token8,
    aux_sym_builtin_type_token9,
    aux_sym_builtin_type_token10,
    aux_sym_builtin_type_token11,
    aux_sym_builtin_type_token12,
    aux_sym_builtin_type_token13,
    aux_sym_builtin_type_token14,
    aux_sym_builtin_type_token15,
    aux_sym_builtin_type_token16,
    aux_sym_builtin_type_token17,
    [5745] = 8,
    ACTIONS(288),
    1,
    anon_sym_CARET,
    ACTIONS(292),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(296),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(310),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(290),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(294),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    ACTIONS(306),
    6,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    ACTIONS(308),
    8,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    [5790] = 8,
    ACTIONS(288),
    1,
    anon_sym_CARET,
    ACTIONS(292),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(296),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(310),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(290),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(294),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    ACTIONS(312),
    6,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    ACTIONS(314),
    8,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    [5835] = 10,
    ACTIONS(228),
    1,
    anon_sym_LPAREN,
    ACTIONS(232),
    1,
    anon_sym_List,
    ACTIONS(234),
    1,
    aux_sym_type_identifier_token1,
    STATE(175),
    1,
    sym_type_identifier,
    STATE(275),
    1,
    aux_sym_qualified_fn_name_repeat1,
    STATE(299),
    1,
    sym_type_reference,
    STATE(309),
    1,
    sym_module_identifier,
    STATE(176),
    2,
    sym_builtin_type,
    sym_qualified_type_name,
    STATE(207),
    2,
    sym_list_type_reference,
    sym_tuple_type_reference,
    ACTIONS(230),
    17,
    aux_sym_builtin_type_token1,
    aux_sym_builtin_type_token2,
    aux_sym_builtin_type_token3,
    aux_sym_builtin_type_token4,
    aux_sym_builtin_type_token5,
    aux_sym_builtin_type_token6,
    aux_sym_builtin_type_token7,
    aux_sym_builtin_type_token8,
    aux_sym_builtin_type_token9,
    aux_sym_builtin_type_token10,
    aux_sym_builtin_type_token11,
    aux_sym_builtin_type_token12,
    aux_sym_builtin_type_token13,
    aux_sym_builtin_type_token14,
    aux_sym_builtin_type_token15,
    aux_sym_builtin_type_token16,
    aux_sym_builtin_type_token17,
    [5884] = 2,
    ACTIONS(300),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(298),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5916] = 4,
    ACTIONS(316),
    1,
    anon_sym_CARET,
    ACTIONS(318),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(286),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(284),
    12,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [5952] = 8,
    ACTIONS(320),
    1,
    anon_sym_CARET,
    ACTIONS(324),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(328),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(330),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(322),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(326),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    ACTIONS(306),
    6,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    ACTIONS(308),
    7,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    [5996] = 2,
    ACTIONS(214),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(212),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6028] = 7,
    ACTIONS(320),
    1,
    anon_sym_CARET,
    ACTIONS(324),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(328),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(322),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(326),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    ACTIONS(286),
    7,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(284),
    8,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6070] = 2,
    ACTIONS(242),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(240),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6102] = 2,
    ACTIONS(210),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(208),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6134] = 8,
    ACTIONS(320),
    1,
    anon_sym_CARET,
    ACTIONS(324),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(328),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(330),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(322),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(326),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    ACTIONS(332),
    6,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    ACTIONS(334),
    7,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    [6178] = 5,
    ACTIONS(320),
    1,
    anon_sym_CARET,
    ACTIONS(324),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(322),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(286),
    9,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(284),
    12,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6216] = 4,
    ACTIONS(320),
    1,
    anon_sym_CARET,
    ACTIONS(322),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(286),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(284),
    12,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6252] = 3,
    ACTIONS(320),
    1,
    anon_sym_CARET,
    ACTIONS(286),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(284),
    15,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6286] = 3,
    ACTIONS(320),
    1,
    anon_sym_CARET,
    ACTIONS(286),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(284),
    15,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6320] = 2,
    ACTIONS(282),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(280),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6352] = 2,
    ACTIONS(278),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(276),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6384] = 2,
    ACTIONS(202),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(200),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6416] = 2,
    ACTIONS(274),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(272),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6448] = 2,
    ACTIONS(270),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(268),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6480] = 2,
    ACTIONS(266),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(264),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6512] = 2,
    ACTIONS(262),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(260),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6544] = 2,
    ACTIONS(258),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(256),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6576] = 8,
    ACTIONS(316),
    1,
    anon_sym_CARET,
    ACTIONS(336),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(340),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(342),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(318),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(338),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    ACTIONS(332),
    6,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    ACTIONS(334),
    7,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    [6620] = 2,
    ACTIONS(254),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(252),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6652] = 2,
    ACTIONS(250),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(248),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6684] = 2,
    ACTIONS(246),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(244),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6716] = 2,
    ACTIONS(242),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(240),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6748] = 8,
    ACTIONS(316),
    1,
    anon_sym_CARET,
    ACTIONS(336),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(340),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(342),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(318),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(338),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    ACTIONS(306),
    6,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    ACTIONS(308),
    7,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    [6792] = 2,
    ACTIONS(304),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(302),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6824] = 2,
    ACTIONS(300),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(298),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6856] = 2,
    ACTIONS(206),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(204),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6888] = 7,
    ACTIONS(316),
    1,
    anon_sym_CARET,
    ACTIONS(336),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(340),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(318),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(338),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    ACTIONS(286),
    7,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(284),
    8,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6930] = 5,
    ACTIONS(316),
    1,
    anon_sym_CARET,
    ACTIONS(336),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(318),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(286),
    9,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(284),
    12,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [6968] = 3,
    ACTIONS(316),
    1,
    anon_sym_CARET,
    ACTIONS(286),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(284),
    15,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7002] = 2,
    ACTIONS(304),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(302),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7034] = 3,
    ACTIONS(316),
    1,
    anon_sym_CARET,
    ACTIONS(286),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(284),
    15,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7068] = 2,
    ACTIONS(282),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(280),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7100] = 2,
    ACTIONS(278),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(276),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7132] = 2,
    ACTIONS(202),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(200),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7164] = 2,
    ACTIONS(274),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(272),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7196] = 2,
    ACTIONS(238),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(236),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7228] = 2,
    ACTIONS(270),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(268),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7260] = 2,
    ACTIONS(266),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(264),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7292] = 2,
    ACTIONS(262),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(260),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7324] = 2,
    ACTIONS(258),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(256),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7356] = 2,
    ACTIONS(254),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(252),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7388] = 2,
    ACTIONS(218),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(216),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7420] = 2,
    ACTIONS(222),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(220),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7452] = 2,
    ACTIONS(250),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(248),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7484] = 2,
    ACTIONS(246),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(244),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7516] = 2,
    ACTIONS(218),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(216),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7548] = 2,
    ACTIONS(226),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(224),
    16,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7580] = 2,
    ACTIONS(222),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(220),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7612] = 2,
    ACTIONS(226),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(224),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7644] = 2,
    ACTIONS(238),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(236),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7676] = 2,
    ACTIONS(214),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(212),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7708] = 2,
    ACTIONS(210),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(208),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7740] = 2,
    ACTIONS(194),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(192),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7772] = 2,
    ACTIONS(206),
    11,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_LT,
    anon_sym_GT,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(204),
    16,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7804] = 3,
    ACTIONS(348),
    1,
    anon_sym_DOT,
    ACTIONS(346),
    8,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(344),
    10,
    ts_builtin_sym_end,
    anon_sym_EQ,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_STAR,
    anon_sym_GT,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [7830] = 2,
    ACTIONS(250),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(248),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [7853] = 2,
    ACTIONS(254),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(252),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [7876] = 2,
    ACTIONS(300),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(298),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [7899] = 6,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(358),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(356),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    ACTIONS(284),
    6,
    anon_sym_RPAREN,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [7930] = 5,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(286),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(284),
    10,
    anon_sym_RPAREN,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [7959] = 10,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(360),
    1,
    anon_sym_RPAREN,
    ACTIONS(364),
    1,
    anon_sym_COMMA,
    STATE(280),
    1,
    aux_sym_tuple_literal_the_rest_repeat1,
    STATE(312),
    1,
    sym_tuple_literal_the_rest,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(358),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(362),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(356),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    [7998] = 2,
    ACTIONS(242),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(240),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8021] = 2,
    ACTIONS(368),
    8,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(366),
    10,
    ts_builtin_sym_end,
    anon_sym_EQ,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_STAR,
    anon_sym_GT,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [8044] = 2,
    ACTIONS(372),
    8,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(370),
    10,
    ts_builtin_sym_end,
    anon_sym_EQ,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_STAR,
    anon_sym_GT,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [8067] = 3,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(286),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(284),
    15,
    anon_sym_RPAREN,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8092] = 3,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(286),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(284),
    15,
    anon_sym_RPAREN,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8117] = 2,
    ACTIONS(282),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(280),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8140] = 2,
    ACTIONS(278),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(276),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8163] = 2,
    ACTIONS(202),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(200),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8186] = 2,
    ACTIONS(274),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(272),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8209] = 2,
    ACTIONS(270),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(268),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8232] = 2,
    ACTIONS(266),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(264),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8255] = 2,
    ACTIONS(262),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(260),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8278] = 2,
    ACTIONS(258),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(256),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8301] = 10,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(364),
    1,
    anon_sym_COMMA,
    ACTIONS(374),
    1,
    anon_sym_RPAREN,
    STATE(280),
    1,
    aux_sym_tuple_literal_the_rest_repeat1,
    STATE(315),
    1,
    sym_tuple_literal_the_rest,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(358),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(362),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(356),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    [8340] = 2,
    ACTIONS(378),
    8,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(376),
    10,
    ts_builtin_sym_end,
    anon_sym_EQ,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_STAR,
    anon_sym_GT,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [8363] = 2,
    ACTIONS(382),
    8,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(380),
    10,
    ts_builtin_sym_end,
    anon_sym_EQ,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_STAR,
    anon_sym_GT,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [8386] = 10,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(364),
    1,
    anon_sym_COMMA,
    ACTIONS(384),
    1,
    anon_sym_RPAREN,
    STATE(280),
    1,
    aux_sym_tuple_literal_the_rest_repeat1,
    STATE(314),
    1,
    sym_tuple_literal_the_rest,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(358),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(362),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(356),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    [8425] = 2,
    ACTIONS(388),
    8,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(386),
    10,
    ts_builtin_sym_end,
    anon_sym_EQ,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_STAR,
    anon_sym_GT,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [8448] = 4,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(286),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(284),
    12,
    anon_sym_RPAREN,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8475] = 10,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(364),
    1,
    anon_sym_COMMA,
    ACTIONS(390),
    1,
    anon_sym_RPAREN,
    STATE(280),
    1,
    aux_sym_tuple_literal_the_rest_repeat1,
    STATE(331),
    1,
    sym_tuple_literal_the_rest,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(358),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(362),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(356),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    [8514] = 2,
    ACTIONS(238),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(236),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8537] = 2,
    ACTIONS(218),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(216),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8560] = 2,
    ACTIONS(222),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(220),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8583] = 2,
    ACTIONS(226),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(224),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8606] = 2,
    ACTIONS(246),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(244),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8629] = 2,
    ACTIONS(206),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(204),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8652] = 2,
    ACTIONS(194),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(192),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8675] = 2,
    ACTIONS(210),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(208),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8698] = 2,
    ACTIONS(214),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(212),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8721] = 10,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(364),
    1,
    anon_sym_COMMA,
    ACTIONS(392),
    1,
    anon_sym_RPAREN,
    STATE(280),
    1,
    aux_sym_tuple_literal_the_rest_repeat1,
    STATE(302),
    1,
    sym_tuple_literal_the_rest,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(358),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(362),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(356),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    [8760] = 2,
    ACTIONS(304),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(302),
    16,
    anon_sym_RPAREN,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [8783] = 2,
    ACTIONS(396),
    8,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(394),
    10,
    ts_builtin_sym_end,
    anon_sym_EQ,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_STAR,
    anon_sym_GT,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [8806] = 7,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(358),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(362),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(306),
    4,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    anon_sym_COMMA,
    ACTIONS(356),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    [8839] = 2,
    ACTIONS(400),
    8,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    ACTIONS(398),
    10,
    ts_builtin_sym_end,
    anon_sym_EQ,
    anon_sym_RPAREN,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_STAR,
    anon_sym_GT,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    [8862] = 5,
    ACTIONS(404),
    1,
    anon_sym_let,
    ACTIONS(409),
    1,
    anon_sym_type,
    STATE(208),
    3,
    sym_fn_decl,
    sym_type_decl,
    aux_sym_source_file_repeat1,
    ACTIONS(402),
    6,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    ACTIONS(407),
    6,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    [8890] = 9,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(412),
    1,
    anon_sym_RBRACK,
    ACTIONS(414),
    1,
    anon_sym_SEMI,
    STATE(281),
    1,
    aux_sym_list_content_repeat1,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(358),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(362),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(356),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    [8926] = 7,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(358),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(362),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(416),
    2,
    anon_sym_RBRACK,
    anon_sym_SEMI,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(356),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    [8957] = 8,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(418),
    1,
    anon_sym_RPAREN,
    ACTIONS(420),
    1,
    anon_sym_COMMA,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(358),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(362),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(356),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    [8990] = 8,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(422),
    1,
    anon_sym_RPAREN,
    ACTIONS(424),
    1,
    anon_sym_COMMA,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(358),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(362),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(356),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    [9023] = 8,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(426),
    1,
    anon_sym_RPAREN,
    ACTIONS(428),
    1,
    anon_sym_COMMA,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(358),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(362),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(356),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    [9056] = 8,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(430),
    1,
    anon_sym_RPAREN,
    ACTIONS(432),
    1,
    anon_sym_COMMA,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(358),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(362),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(356),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    [9089] = 8,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(434),
    1,
    anon_sym_RPAREN,
    ACTIONS(436),
    1,
    anon_sym_COMMA,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(358),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(362),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(356),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    [9122] = 7,
    ACTIONS(350),
    1,
    anon_sym_CARET,
    ACTIONS(354),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(358),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(362),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(438),
    2,
    anon_sym_RPAREN,
    anon_sym_COMMA,
    ACTIONS(352),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(356),
    4,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT_EQ,
    anon_sym_GT_EQ,
    [9153] = 6,
    ACTIONS(284),
    1,
    anon_sym_LF,
    ACTIONS(440),
    1,
    anon_sym_CARET,
    ACTIONS(286),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(444),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(442),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(446),
    6,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    [9181] = 2,
    ACTIONS(224),
    1,
    anon_sym_LF,
    ACTIONS(226),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9201] = 2,
    ACTIONS(276),
    1,
    anon_sym_LF,
    ACTIONS(278),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9221] = 2,
    ACTIONS(280),
    1,
    anon_sym_LF,
    ACTIONS(282),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9241] = 3,
    ACTIONS(284),
    1,
    anon_sym_LF,
    ACTIONS(440),
    1,
    anon_sym_CARET,
    ACTIONS(286),
    13,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9263] = 3,
    ACTIONS(284),
    1,
    anon_sym_LF,
    ACTIONS(440),
    1,
    anon_sym_CARET,
    ACTIONS(286),
    13,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9285] = 4,
    ACTIONS(284),
    1,
    anon_sym_LF,
    ACTIONS(440),
    1,
    anon_sym_CARET,
    ACTIONS(442),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(286),
    10,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9309] = 5,
    ACTIONS(284),
    1,
    anon_sym_LF,
    ACTIONS(440),
    1,
    anon_sym_CARET,
    ACTIONS(444),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(442),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(286),
    8,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9335] = 6,
    ACTIONS(440),
    1,
    anon_sym_CARET,
    ACTIONS(448),
    1,
    anon_sym_LF,
    ACTIONS(444),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(450),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(442),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(446),
    6,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    [9363] = 2,
    ACTIONS(298),
    1,
    anon_sym_LF,
    ACTIONS(300),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9383] = 6,
    ACTIONS(440),
    1,
    anon_sym_CARET,
    ACTIONS(452),
    1,
    anon_sym_LF,
    ACTIONS(444),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(450),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(442),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(446),
    6,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    [9411] = 2,
    ACTIONS(302),
    1,
    anon_sym_LF,
    ACTIONS(304),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9431] = 6,
    ACTIONS(306),
    1,
    anon_sym_LF,
    ACTIONS(440),
    1,
    anon_sym_CARET,
    ACTIONS(444),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(450),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(442),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(446),
    6,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    [9459] = 2,
    ACTIONS(240),
    1,
    anon_sym_LF,
    ACTIONS(242),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9479] = 6,
    ACTIONS(440),
    1,
    anon_sym_CARET,
    ACTIONS(454),
    1,
    anon_sym_LF,
    ACTIONS(444),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(450),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(442),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(446),
    6,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    [9507] = 2,
    ACTIONS(272),
    1,
    anon_sym_LF,
    ACTIONS(274),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9527] = 2,
    ACTIONS(268),
    1,
    anon_sym_LF,
    ACTIONS(270),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9547] = 2,
    ACTIONS(264),
    1,
    anon_sym_LF,
    ACTIONS(266),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9567] = 2,
    ACTIONS(260),
    1,
    anon_sym_LF,
    ACTIONS(262),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9587] = 2,
    ACTIONS(256),
    1,
    anon_sym_LF,
    ACTIONS(258),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9607] = 2,
    ACTIONS(252),
    1,
    anon_sym_LF,
    ACTIONS(254),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9627] = 6,
    ACTIONS(440),
    1,
    anon_sym_CARET,
    ACTIONS(456),
    1,
    anon_sym_LF,
    ACTIONS(444),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(450),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(442),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(446),
    6,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    [9655] = 2,
    ACTIONS(212),
    1,
    anon_sym_LF,
    ACTIONS(214),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9675] = 2,
    ACTIONS(208),
    1,
    anon_sym_LF,
    ACTIONS(210),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9695] = 2,
    ACTIONS(192),
    1,
    anon_sym_LF,
    ACTIONS(194),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9715] = 2,
    ACTIONS(204),
    1,
    anon_sym_LF,
    ACTIONS(206),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9735] = 2,
    ACTIONS(236),
    1,
    anon_sym_LF,
    ACTIONS(238),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9755] = 6,
    ACTIONS(440),
    1,
    anon_sym_CARET,
    ACTIONS(458),
    1,
    anon_sym_LF,
    ACTIONS(444),
    2,
    anon_sym_PLUS,
    anon_sym_DASH,
    ACTIONS(450),
    2,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    ACTIONS(442),
    3,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    ACTIONS(446),
    6,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    [9783] = 2,
    ACTIONS(220),
    1,
    anon_sym_LF,
    ACTIONS(222),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9803] = 2,
    ACTIONS(216),
    1,
    anon_sym_LF,
    ACTIONS(218),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9823] = 2,
    ACTIONS(244),
    1,
    anon_sym_LF,
    ACTIONS(246),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9843] = 2,
    ACTIONS(248),
    1,
    anon_sym_LF,
    ACTIONS(250),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9863] = 2,
    ACTIONS(200),
    1,
    anon_sym_LF,
    ACTIONS(202),
    14,
    anon_sym_CARET,
    anon_sym_STAR,
    anon_sym_SLASH,
    anon_sym_PERCENT,
    anon_sym_PLUS,
    anon_sym_DASH,
    anon_sym_EQ_EQ,
    anon_sym_BANG_EQ,
    anon_sym_LT,
    anon_sym_LT_EQ,
    anon_sym_GT,
    anon_sym_GT_EQ,
    anon_sym_AMP_AMP,
    anon_sym_PIPE_PIPE,
    [9883] = 2,
    ACTIONS(460),
    6,
    ts_builtin_sym_end,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    ACTIONS(462),
    8,
    anon_sym_let,
    anon_sym_LPAREN,
    anon_sym_type,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    [9902] = 2,
    ACTIONS(466),
    5,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    ACTIONS(464),
    7,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    [9919] = 2,
    ACTIONS(198),
    5,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    ACTIONS(196),
    7,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    [9936] = 2,
    ACTIONS(470),
    5,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_float_literal,
    anon_sym_LBRACK,
    sym_unit,
    ACTIONS(468),
    7,
    anon_sym_let,
    anon_sym_LPAREN,
    aux_sym_bool_literal_token1,
    aux_sym_bool_literal_token2,
    sym_negative_digits,
    sym_positive_digits,
    aux_sym_variable_identifier_token1,
    [9953] = 6,
    ACTIONS(474),
    1,
    anon_sym_uy,
    ACTIONS(476),
    1,
    anon_sym_us,
    ACTIONS(478),
    1,
    anon_sym_ul,
    ACTIONS(480),
    1,
    anon_sym_UL,
    ACTIONS(482),
    1,
    anon_sym_Z,
    ACTIONS(472),
    5,
    anon_sym_y,
    anon_sym_s,
    anon_sym_l,
    anon_sym_L,
    anon_sym_Q,
    [9976] = 6,
    ACTIONS(484),
    1,
    anon_sym_uy,
    ACTIONS(486),
    1,
    anon_sym_us,
    ACTIONS(488),
    1,
    anon_sym_ul,
    ACTIONS(490),
    1,
    anon_sym_UL,
    ACTIONS(492),
    1,
    anon_sym_Z,
    ACTIONS(472),
    5,
    anon_sym_y,
    anon_sym_s,
    anon_sym_l,
    anon_sym_L,
    anon_sym_Q,
    [9999] = 6,
    ACTIONS(494),
    1,
    anon_sym_uy,
    ACTIONS(496),
    1,
    anon_sym_us,
    ACTIONS(498),
    1,
    anon_sym_ul,
    ACTIONS(500),
    1,
    anon_sym_UL,
    ACTIONS(502),
    1,
    anon_sym_Z,
    ACTIONS(472),
    5,
    anon_sym_y,
    anon_sym_s,
    anon_sym_l,
    anon_sym_L,
    anon_sym_Q,
    [10022] = 6,
    ACTIONS(504),
    1,
    anon_sym_uy,
    ACTIONS(506),
    1,
    anon_sym_us,
    ACTIONS(508),
    1,
    anon_sym_ul,
    ACTIONS(510),
    1,
    anon_sym_UL,
    ACTIONS(512),
    1,
    anon_sym_Z,
    ACTIONS(472),
    5,
    anon_sym_y,
    anon_sym_s,
    anon_sym_l,
    anon_sym_L,
    anon_sym_Q,
    [10045] = 6,
    ACTIONS(514),
    1,
    anon_sym_uy,
    ACTIONS(516),
    1,
    anon_sym_us,
    ACTIONS(518),
    1,
    anon_sym_ul,
    ACTIONS(520),
    1,
    anon_sym_UL,
    ACTIONS(522),
    1,
    anon_sym_Z,
    ACTIONS(472),
    5,
    anon_sym_y,
    anon_sym_s,
    anon_sym_l,
    anon_sym_L,
    anon_sym_Q,
    [10068] = 4,
    ACTIONS(524),
    1,
    anon_sym_LPAREN,
    ACTIONS(526),
    1,
    sym_unit,
    STATE(328),
    1,
    sym_fn_decl_params,
    STATE(264),
    2,
    sym_fn_decl_param,
    aux_sym_fn_decl_params_repeat1,
    [10082] = 4,
    ACTIONS(528),
    1,
    anon_sym_COLON,
    ACTIONS(530),
    1,
    anon_sym_LPAREN,
    ACTIONS(533),
    1,
    sym_unit,
    STATE(260),
    2,
    sym_fn_decl_param,
    aux_sym_fn_decl_params_repeat1,
    [10096] = 4,
    ACTIONS(536),
    1,
    anon_sym_DQUOTE,
    STATE(278),
    1,
    aux_sym_string_content_repeat1,
    STATE(306),
    1,
    sym_string_content,
    ACTIONS(538),
    2,
    aux_sym_string_content_token1,
    sym_char_or_string_escape_sequence,
    [10110] = 5,
    ACTIONS(540),
    1,
    anon_sym_y,
    ACTIONS(542),
    1,
    anon_sym_s,
    ACTIONS(544),
    1,
    anon_sym_l,
    ACTIONS(546),
    1,
    anon_sym_L,
    ACTIONS(548),
    1,
    anon_sym_Q,
    [10126] = 4,
    ACTIONS(550),
    1,
    anon_sym_DQUOTE,
    STATE(278),
    1,
    aux_sym_string_content_repeat1,
    STATE(321),
    1,
    sym_string_content,
    ACTIONS(538),
    2,
    aux_sym_string_content_token1,
    sym_char_or_string_escape_sequence,
    [10140] = 4,
    ACTIONS(524),
    1,
    anon_sym_LPAREN,
    ACTIONS(552),
    1,
    anon_sym_COLON,
    ACTIONS(554),
    1,
    sym_unit,
    STATE(260),
    2,
    sym_fn_decl_param,
    aux_sym_fn_decl_params_repeat1,
    [10154] = 5,
    ACTIONS(556),
    1,
    anon_sym_y,
    ACTIONS(558),
    1,
    anon_sym_s,
    ACTIONS(560),
    1,
    anon_sym_l,
    ACTIONS(562),
    1,
    anon_sym_L,
    ACTIONS(564),
    1,
    anon_sym_Q,
    [10170] = 5,
    ACTIONS(566),
    1,
    anon_sym_y,
    ACTIONS(568),
    1,
    anon_sym_s,
    ACTIONS(570),
    1,
    anon_sym_l,
    ACTIONS(572),
    1,
    anon_sym_L,
    ACTIONS(574),
    1,
    anon_sym_Q,
    [10186] = 1,
    ACTIONS(472),
    5,
    anon_sym_y,
    anon_sym_s,
    anon_sym_l,
    anon_sym_L,
    anon_sym_Q,
    [10194] = 4,
    ACTIONS(576),
    1,
    anon_sym_DQUOTE,
    STATE(278),
    1,
    aux_sym_string_content_repeat1,
    STATE(340),
    1,
    sym_string_content,
    ACTIONS(538),
    2,
    aux_sym_string_content_token1,
    sym_char_or_string_escape_sequence,
    [10208] = 5,
    ACTIONS(47),
    1,
    aux_sym_type_identifier_token1,
    ACTIONS(578),
    1,
    aux_sym_variable_identifier_token1,
    STATE(253),
    1,
    sym_fn_identifier,
    STATE(276),
    1,
    aux_sym_qualified_fn_name_repeat1,
    STATE(309),
    1,
    sym_module_identifier,
    [10224] = 4,
    ACTIONS(580),
    1,
    anon_sym_DQUOTE,
    STATE(278),
    1,
    aux_sym_string_content_repeat1,
    STATE(303),
    1,
    sym_string_content,
    ACTIONS(538),
    2,
    aux_sym_string_content_token1,
    sym_char_or_string_escape_sequence,
    [10238] = 5,
    ACTIONS(582),
    1,
    anon_sym_y,
    ACTIONS(584),
    1,
    anon_sym_s,
    ACTIONS(586),
    1,
    anon_sym_l,
    ACTIONS(588),
    1,
    anon_sym_L,
    ACTIONS(590),
    1,
    anon_sym_Q,
    [10254] = 5,
    ACTIONS(592),
    1,
    anon_sym_y,
    ACTIONS(594),
    1,
    anon_sym_s,
    ACTIONS(596),
    1,
    anon_sym_l,
    ACTIONS(598),
    1,
    anon_sym_L,
    ACTIONS(600),
    1,
    anon_sym_Q,
    [10270] = 4,
    ACTIONS(602),
    1,
    anon_sym_DQUOTE,
    STATE(278),
    1,
    aux_sym_string_content_repeat1,
    STATE(332),
    1,
    sym_string_content,
    ACTIONS(538),
    2,
    aux_sym_string_content_token1,
    sym_char_or_string_escape_sequence,
    [10284] = 3,
    ACTIONS(604),
    1,
    anon_sym_DQUOTE,
    STATE(274),
    1,
    aux_sym_string_content_repeat1,
    ACTIONS(606),
    2,
    aux_sym_string_content_token1,
    sym_char_or_string_escape_sequence,
    [10295] = 4,
    ACTIONS(609),
    1,
    aux_sym_type_identifier_token1,
    STATE(189),
    1,
    sym_type_identifier,
    STATE(276),
    1,
    aux_sym_qualified_fn_name_repeat1,
    STATE(309),
    1,
    sym_module_identifier,
    [10308] = 4,
    ACTIONS(611),
    1,
    aux_sym_variable_identifier_token1,
    ACTIONS(613),
    1,
    aux_sym_type_identifier_token1,
    STATE(276),
    1,
    aux_sym_qualified_fn_name_repeat1,
    STATE(309),
    1,
    sym_module_identifier,
    [10321] = 4,
    ACTIONS(616),
    1,
    anon_sym_RPAREN,
    ACTIONS(618),
    1,
    anon_sym_STAR,
    STATE(291),
    1,
    aux_sym_type_reference_tuple_the_rest_repeat1,
    STATE(311),
    1,
    sym_type_reference_tuple_the_rest,
    [10334] = 3,
    ACTIONS(620),
    1,
    anon_sym_DQUOTE,
    STATE(274),
    1,
    aux_sym_string_content_repeat1,
    ACTIONS(622),
    2,
    aux_sym_string_content_token1,
    sym_char_or_string_escape_sequence,
    [10345] = 2,
    STATE(304),
    1,
    sym_character,
    ACTIONS(624),
    2,
    aux_sym_character_token1,
    sym_char_or_string_escape_sequence,
    [10353] = 3,
    ACTIONS(364),
    1,
    anon_sym_COMMA,
    ACTIONS(626),
    1,
    anon_sym_RPAREN,
    STATE(286),
    1,
    aux_sym_tuple_literal_the_rest_repeat1,
    [10363] = 3,
    ACTIONS(628),
    1,
    anon_sym_RBRACK,
    ACTIONS(630),
    1,
    anon_sym_SEMI,
    STATE(288),
    1,
    aux_sym_list_content_repeat1,
    [10373] = 2,
    STATE(319),
    1,
    sym_character,
    ACTIONS(624),
    2,
    aux_sym_character_token1,
    sym_char_or_string_escape_sequence,
    [10381] = 3,
    ACTIONS(632),
    1,
    aux_sym_variable_identifier_token1,
    STATE(259),
    1,
    sym_fn_identifier,
    STATE(320),
    1,
    sym_variable_identifier,
    [10391] = 2,
    ACTIONS(636),
    1,
    anon_sym_LPAREN,
    ACTIONS(634),
    2,
    anon_sym_COLON,
    sym_unit,
    [10399] = 3,
    ACTIONS(638),
    1,
    anon_sym_RPAREN,
    ACTIONS(640),
    1,
    anon_sym_STAR,
    STATE(285),
    1,
    aux_sym_type_reference_tuple_the_rest_repeat1,
    [10409] = 3,
    ACTIONS(643),
    1,
    anon_sym_RPAREN,
    ACTIONS(645),
    1,
    anon_sym_COMMA,
    STATE(286),
    1,
    aux_sym_tuple_literal_the_rest_repeat1,
    [10419] = 2,
    STATE(339),
    1,
    sym_character,
    ACTIONS(624),
    2,
    aux_sym_character_token1,
    sym_char_or_string_escape_sequence,
    [10427] = 3,
    ACTIONS(648),
    1,
    anon_sym_RBRACK,
    ACTIONS(650),
    1,
    anon_sym_SEMI,
    STATE(288),
    1,
    aux_sym_list_content_repeat1,
    [10437] = 2,
    STATE(307),
    1,
    sym_character,
    ACTIONS(624),
    2,
    aux_sym_character_token1,
    sym_char_or_string_escape_sequence,
    [10445] = 2,
    STATE(326),
    1,
    sym_character,
    ACTIONS(624),
    2,
    aux_sym_character_token1,
    sym_char_or_string_escape_sequence,
    [10453] = 3,
    ACTIONS(618),
    1,
    anon_sym_STAR,
    ACTIONS(653),
    1,
    anon_sym_RPAREN,
    STATE(285),
    1,
    aux_sym_type_reference_tuple_the_rest_repeat1,
    [10463] = 2,
    ACTIONS(655),
    1,
    aux_sym_variable_identifier_token1,
    STATE(330),
    1,
    sym_variable_identifier,
    [10470] = 1,
    ACTIONS(611),
    2,
    aux_sym_variable_identifier_token1,
    aux_sym_type_identifier_token1,
    [10475] = 2,
    ACTIONS(655),
    1,
    aux_sym_variable_identifier_token1,
    STATE(337),
    1,
    sym_variable_identifier,
    [10482] = 2,
    ACTIONS(655),
    1,
    aux_sym_variable_identifier_token1,
    STATE(336),
    1,
    sym_variable_identifier,
    [10489] = 2,
    ACTIONS(655),
    1,
    aux_sym_variable_identifier_token1,
    STATE(335),
    1,
    sym_variable_identifier,
    [10496] = 2,
    ACTIONS(655),
    1,
    aux_sym_variable_identifier_token1,
    STATE(320),
    1,
    sym_variable_identifier,
    [10503] = 2,
    ACTIONS(657),
    1,
    aux_sym_type_identifier_token1,
    STATE(305),
    1,
    sym_type_identifier,
    [10510] = 1,
    ACTIONS(659),
    2,
    anon_sym_RPAREN,
    anon_sym_STAR,
    [10515] = 2,
    ACTIONS(578),
    1,
    aux_sym_variable_identifier_token1,
    STATE(259),
    1,
    sym_fn_identifier,
    [10522] = 2,
    ACTIONS(655),
    1,
    aux_sym_variable_identifier_token1,
    STATE(333),
    1,
    sym_variable_identifier,
    [10529] = 1,
    ACTIONS(661),
    1,
    anon_sym_RPAREN,
    [10533] = 1,
    ACTIONS(663),
    1,
    anon_sym_DQUOTE,
    [10537] = 1,
    ACTIONS(665),
    1,
    anon_sym_SQUOTE,
    [10541] = 1,
    ACTIONS(667),
    1,
    anon_sym_EQ,
    [10545] = 1,
    ACTIONS(669),
    1,
    anon_sym_DQUOTE,
    [10549] = 1,
    ACTIONS(671),
    1,
    anon_sym_SQUOTE,
    [10553] = 1,
    ACTIONS(673),
    1,
    anon_sym_RBRACK,
    [10557] = 1,
    ACTIONS(675),
    1,
    anon_sym_DOT,
    [10561] = 1,
    ACTIONS(677),
    1,
    anon_sym_RBRACK,
    [10565] = 1,
    ACTIONS(679),
    1,
    anon_sym_RPAREN,
    [10569] = 1,
    ACTIONS(681),
    1,
    anon_sym_RPAREN,
    [10573] = 1,
    ACTIONS(683),
    1,
    anon_sym_RPAREN,
    [10577] = 1,
    ACTIONS(685),
    1,
    anon_sym_RPAREN,
    [10581] = 1,
    ACTIONS(687),
    1,
    anon_sym_RPAREN,
    [10585] = 1,
    ACTIONS(348),
    1,
    anon_sym_DOT,
    [10589] = 1,
    ACTIONS(689),
    1,
    anon_sym_LT,
    [10593] = 1,
    ACTIONS(691),
    1,
    anon_sym_RBRACK,
    [10597] = 1,
    ACTIONS(693),
    1,
    anon_sym_SQUOTE,
    [10601] = 1,
    ACTIONS(695),
    1,
    anon_sym_EQ,
    [10605] = 1,
    ACTIONS(697),
    1,
    anon_sym_DQUOTE,
    [10609] = 1,
    ACTIONS(699),
    1,
    anon_sym_EQ,
    [10613] = 1,
    ACTIONS(701),
    1,
    ts_builtin_sym_end,
    [10617] = 1,
    ACTIONS(703),
    1,
    anon_sym_SQUOTE,
    [10621] = 1,
    ACTIONS(705),
    1,
    anon_sym_GT,
    [10625] = 1,
    ACTIONS(707),
    1,
    anon_sym_SQUOTE,
    [10629] = 1,
    ACTIONS(709),
    1,
    anon_sym_RBRACK,
    [10633] = 1,
    ACTIONS(711),
    1,
    anon_sym_COLON,
    [10637] = 1,
    ACTIONS(713),
    1,
    anon_sym_STAR,
    [10641] = 1,
    ACTIONS(715),
    1,
    anon_sym_COLON,
    [10645] = 1,
    ACTIONS(717),
    1,
    anon_sym_RPAREN,
    [10649] = 1,
    ACTIONS(719),
    1,
    anon_sym_DQUOTE,
    [10653] = 1,
    ACTIONS(721),
    1,
    anon_sym_EQ,
    [10657] = 1,
    ACTIONS(344),
    1,
    anon_sym_EQ,
    [10661] = 1,
    ACTIONS(723),
    1,
    anon_sym_EQ,
    [10665] = 1,
    ACTIONS(725),
    1,
    anon_sym_EQ,
    [10669] = 1,
    ACTIONS(727),
    1,
    anon_sym_EQ,
    [10673] = 1,
    ACTIONS(729),
    1,
    anon_sym_RBRACK,
    [10677] = 1,
    ACTIONS(731),
    1,
    anon_sym_SQUOTE,
    [10681] = 1,
    ACTIONS(733),
    1,
    anon_sym_DQUOTE,
};

static const uint32_t ts_small_parse_table_map[] = {
    [SMALL_STATE(2)] = 0,
    [SMALL_STATE(3)] = 75,
    [SMALL_STATE(4)] = 151,
    [SMALL_STATE(5)] = 227,
    [SMALL_STATE(6)] = 303,
    [SMALL_STATE(7)] = 379,
    [SMALL_STATE(8)] = 455,
    [SMALL_STATE(9)] = 522,
    [SMALL_STATE(10)] = 589,
    [SMALL_STATE(11)] = 656,
    [SMALL_STATE(12)] = 723,
    [SMALL_STATE(13)] = 790,
    [SMALL_STATE(14)] = 857,
    [SMALL_STATE(15)] = 924,
    [SMALL_STATE(16)] = 991,
    [SMALL_STATE(17)] = 1058,
    [SMALL_STATE(18)] = 1125,
    [SMALL_STATE(19)] = 1192,
    [SMALL_STATE(20)] = 1259,
    [SMALL_STATE(21)] = 1326,
    [SMALL_STATE(22)] = 1393,
    [SMALL_STATE(23)] = 1457,
    [SMALL_STATE(24)] = 1521,
    [SMALL_STATE(25)] = 1585,
    [SMALL_STATE(26)] = 1649,
    [SMALL_STATE(27)] = 1713,
    [SMALL_STATE(28)] = 1777,
    [SMALL_STATE(29)] = 1841,
    [SMALL_STATE(30)] = 1902,
    [SMALL_STATE(31)] = 1963,
    [SMALL_STATE(32)] = 2024,
    [SMALL_STATE(33)] = 2085,
    [SMALL_STATE(34)] = 2146,
    [SMALL_STATE(35)] = 2207,
    [SMALL_STATE(36)] = 2268,
    [SMALL_STATE(37)] = 2329,
    [SMALL_STATE(38)] = 2390,
    [SMALL_STATE(39)] = 2451,
    [SMALL_STATE(40)] = 2512,
    [SMALL_STATE(41)] = 2573,
    [SMALL_STATE(42)] = 2634,
    [SMALL_STATE(43)] = 2695,
    [SMALL_STATE(44)] = 2756,
    [SMALL_STATE(45)] = 2817,
    [SMALL_STATE(46)] = 2878,
    [SMALL_STATE(47)] = 2939,
    [SMALL_STATE(48)] = 3000,
    [SMALL_STATE(49)] = 3061,
    [SMALL_STATE(50)] = 3122,
    [SMALL_STATE(51)] = 3183,
    [SMALL_STATE(52)] = 3244,
    [SMALL_STATE(53)] = 3305,
    [SMALL_STATE(54)] = 3366,
    [SMALL_STATE(55)] = 3427,
    [SMALL_STATE(56)] = 3488,
    [SMALL_STATE(57)] = 3549,
    [SMALL_STATE(58)] = 3610,
    [SMALL_STATE(59)] = 3671,
    [SMALL_STATE(60)] = 3732,
    [SMALL_STATE(61)] = 3793,
    [SMALL_STATE(62)] = 3854,
    [SMALL_STATE(63)] = 3915,
    [SMALL_STATE(64)] = 3976,
    [SMALL_STATE(65)] = 4037,
    [SMALL_STATE(66)] = 4098,
    [SMALL_STATE(67)] = 4159,
    [SMALL_STATE(68)] = 4220,
    [SMALL_STATE(69)] = 4281,
    [SMALL_STATE(70)] = 4342,
    [SMALL_STATE(71)] = 4403,
    [SMALL_STATE(72)] = 4464,
    [SMALL_STATE(73)] = 4498,
    [SMALL_STATE(74)] = 4536,
    [SMALL_STATE(75)] = 4569,
    [SMALL_STATE(76)] = 4602,
    [SMALL_STATE(77)] = 4635,
    [SMALL_STATE(78)] = 4668,
    [SMALL_STATE(79)] = 4701,
    [SMALL_STATE(80)] = 4734,
    [SMALL_STATE(81)] = 4767,
    [SMALL_STATE(82)] = 4800,
    [SMALL_STATE(83)] = 4849,
    [SMALL_STATE(84)] = 4882,
    [SMALL_STATE(85)] = 4915,
    [SMALL_STATE(86)] = 4948,
    [SMALL_STATE(87)] = 4997,
    [SMALL_STATE(88)] = 5030,
    [SMALL_STATE(89)] = 5079,
    [SMALL_STATE(90)] = 5112,
    [SMALL_STATE(91)] = 5145,
    [SMALL_STATE(92)] = 5178,
    [SMALL_STATE(93)] = 5211,
    [SMALL_STATE(94)] = 5244,
    [SMALL_STATE(95)] = 5277,
    [SMALL_STATE(96)] = 5310,
    [SMALL_STATE(97)] = 5343,
    [SMALL_STATE(98)] = 5378,
    [SMALL_STATE(99)] = 5427,
    [SMALL_STATE(100)] = 5462,
    [SMALL_STATE(101)] = 5499,
    [SMALL_STATE(102)] = 5538,
    [SMALL_STATE(103)] = 5581,
    [SMALL_STATE(104)] = 5614,
    [SMALL_STATE(105)] = 5647,
    [SMALL_STATE(106)] = 5696,
    [SMALL_STATE(107)] = 5745,
    [SMALL_STATE(108)] = 5790,
    [SMALL_STATE(109)] = 5835,
    [SMALL_STATE(110)] = 5884,
    [SMALL_STATE(111)] = 5916,
    [SMALL_STATE(112)] = 5952,
    [SMALL_STATE(113)] = 5996,
    [SMALL_STATE(114)] = 6028,
    [SMALL_STATE(115)] = 6070,
    [SMALL_STATE(116)] = 6102,
    [SMALL_STATE(117)] = 6134,
    [SMALL_STATE(118)] = 6178,
    [SMALL_STATE(119)] = 6216,
    [SMALL_STATE(120)] = 6252,
    [SMALL_STATE(121)] = 6286,
    [SMALL_STATE(122)] = 6320,
    [SMALL_STATE(123)] = 6352,
    [SMALL_STATE(124)] = 6384,
    [SMALL_STATE(125)] = 6416,
    [SMALL_STATE(126)] = 6448,
    [SMALL_STATE(127)] = 6480,
    [SMALL_STATE(128)] = 6512,
    [SMALL_STATE(129)] = 6544,
    [SMALL_STATE(130)] = 6576,
    [SMALL_STATE(131)] = 6620,
    [SMALL_STATE(132)] = 6652,
    [SMALL_STATE(133)] = 6684,
    [SMALL_STATE(134)] = 6716,
    [SMALL_STATE(135)] = 6748,
    [SMALL_STATE(136)] = 6792,
    [SMALL_STATE(137)] = 6824,
    [SMALL_STATE(138)] = 6856,
    [SMALL_STATE(139)] = 6888,
    [SMALL_STATE(140)] = 6930,
    [SMALL_STATE(141)] = 6968,
    [SMALL_STATE(142)] = 7002,
    [SMALL_STATE(143)] = 7034,
    [SMALL_STATE(144)] = 7068,
    [SMALL_STATE(145)] = 7100,
    [SMALL_STATE(146)] = 7132,
    [SMALL_STATE(147)] = 7164,
    [SMALL_STATE(148)] = 7196,
    [SMALL_STATE(149)] = 7228,
    [SMALL_STATE(150)] = 7260,
    [SMALL_STATE(151)] = 7292,
    [SMALL_STATE(152)] = 7324,
    [SMALL_STATE(153)] = 7356,
    [SMALL_STATE(154)] = 7388,
    [SMALL_STATE(155)] = 7420,
    [SMALL_STATE(156)] = 7452,
    [SMALL_STATE(157)] = 7484,
    [SMALL_STATE(158)] = 7516,
    [SMALL_STATE(159)] = 7548,
    [SMALL_STATE(160)] = 7580,
    [SMALL_STATE(161)] = 7612,
    [SMALL_STATE(162)] = 7644,
    [SMALL_STATE(163)] = 7676,
    [SMALL_STATE(164)] = 7708,
    [SMALL_STATE(165)] = 7740,
    [SMALL_STATE(166)] = 7772,
    [SMALL_STATE(167)] = 7804,
    [SMALL_STATE(168)] = 7830,
    [SMALL_STATE(169)] = 7853,
    [SMALL_STATE(170)] = 7876,
    [SMALL_STATE(171)] = 7899,
    [SMALL_STATE(172)] = 7930,
    [SMALL_STATE(173)] = 7959,
    [SMALL_STATE(174)] = 7998,
    [SMALL_STATE(175)] = 8021,
    [SMALL_STATE(176)] = 8044,
    [SMALL_STATE(177)] = 8067,
    [SMALL_STATE(178)] = 8092,
    [SMALL_STATE(179)] = 8117,
    [SMALL_STATE(180)] = 8140,
    [SMALL_STATE(181)] = 8163,
    [SMALL_STATE(182)] = 8186,
    [SMALL_STATE(183)] = 8209,
    [SMALL_STATE(184)] = 8232,
    [SMALL_STATE(185)] = 8255,
    [SMALL_STATE(186)] = 8278,
    [SMALL_STATE(187)] = 8301,
    [SMALL_STATE(188)] = 8340,
    [SMALL_STATE(189)] = 8363,
    [SMALL_STATE(190)] = 8386,
    [SMALL_STATE(191)] = 8425,
    [SMALL_STATE(192)] = 8448,
    [SMALL_STATE(193)] = 8475,
    [SMALL_STATE(194)] = 8514,
    [SMALL_STATE(195)] = 8537,
    [SMALL_STATE(196)] = 8560,
    [SMALL_STATE(197)] = 8583,
    [SMALL_STATE(198)] = 8606,
    [SMALL_STATE(199)] = 8629,
    [SMALL_STATE(200)] = 8652,
    [SMALL_STATE(201)] = 8675,
    [SMALL_STATE(202)] = 8698,
    [SMALL_STATE(203)] = 8721,
    [SMALL_STATE(204)] = 8760,
    [SMALL_STATE(205)] = 8783,
    [SMALL_STATE(206)] = 8806,
    [SMALL_STATE(207)] = 8839,
    [SMALL_STATE(208)] = 8862,
    [SMALL_STATE(209)] = 8890,
    [SMALL_STATE(210)] = 8926,
    [SMALL_STATE(211)] = 8957,
    [SMALL_STATE(212)] = 8990,
    [SMALL_STATE(213)] = 9023,
    [SMALL_STATE(214)] = 9056,
    [SMALL_STATE(215)] = 9089,
    [SMALL_STATE(216)] = 9122,
    [SMALL_STATE(217)] = 9153,
    [SMALL_STATE(218)] = 9181,
    [SMALL_STATE(219)] = 9201,
    [SMALL_STATE(220)] = 9221,
    [SMALL_STATE(221)] = 9241,
    [SMALL_STATE(222)] = 9263,
    [SMALL_STATE(223)] = 9285,
    [SMALL_STATE(224)] = 9309,
    [SMALL_STATE(225)] = 9335,
    [SMALL_STATE(226)] = 9363,
    [SMALL_STATE(227)] = 9383,
    [SMALL_STATE(228)] = 9411,
    [SMALL_STATE(229)] = 9431,
    [SMALL_STATE(230)] = 9459,
    [SMALL_STATE(231)] = 9479,
    [SMALL_STATE(232)] = 9507,
    [SMALL_STATE(233)] = 9527,
    [SMALL_STATE(234)] = 9547,
    [SMALL_STATE(235)] = 9567,
    [SMALL_STATE(236)] = 9587,
    [SMALL_STATE(237)] = 9607,
    [SMALL_STATE(238)] = 9627,
    [SMALL_STATE(239)] = 9655,
    [SMALL_STATE(240)] = 9675,
    [SMALL_STATE(241)] = 9695,
    [SMALL_STATE(242)] = 9715,
    [SMALL_STATE(243)] = 9735,
    [SMALL_STATE(244)] = 9755,
    [SMALL_STATE(245)] = 9783,
    [SMALL_STATE(246)] = 9803,
    [SMALL_STATE(247)] = 9823,
    [SMALL_STATE(248)] = 9843,
    [SMALL_STATE(249)] = 9863,
    [SMALL_STATE(250)] = 9883,
    [SMALL_STATE(251)] = 9902,
    [SMALL_STATE(252)] = 9919,
    [SMALL_STATE(253)] = 9936,
    [SMALL_STATE(254)] = 9953,
    [SMALL_STATE(255)] = 9976,
    [SMALL_STATE(256)] = 9999,
    [SMALL_STATE(257)] = 10022,
    [SMALL_STATE(258)] = 10045,
    [SMALL_STATE(259)] = 10068,
    [SMALL_STATE(260)] = 10082,
    [SMALL_STATE(261)] = 10096,
    [SMALL_STATE(262)] = 10110,
    [SMALL_STATE(263)] = 10126,
    [SMALL_STATE(264)] = 10140,
    [SMALL_STATE(265)] = 10154,
    [SMALL_STATE(266)] = 10170,
    [SMALL_STATE(267)] = 10186,
    [SMALL_STATE(268)] = 10194,
    [SMALL_STATE(269)] = 10208,
    [SMALL_STATE(270)] = 10224,
    [SMALL_STATE(271)] = 10238,
    [SMALL_STATE(272)] = 10254,
    [SMALL_STATE(273)] = 10270,
    [SMALL_STATE(274)] = 10284,
    [SMALL_STATE(275)] = 10295,
    [SMALL_STATE(276)] = 10308,
    [SMALL_STATE(277)] = 10321,
    [SMALL_STATE(278)] = 10334,
    [SMALL_STATE(279)] = 10345,
    [SMALL_STATE(280)] = 10353,
    [SMALL_STATE(281)] = 10363,
    [SMALL_STATE(282)] = 10373,
    [SMALL_STATE(283)] = 10381,
    [SMALL_STATE(284)] = 10391,
    [SMALL_STATE(285)] = 10399,
    [SMALL_STATE(286)] = 10409,
    [SMALL_STATE(287)] = 10419,
    [SMALL_STATE(288)] = 10427,
    [SMALL_STATE(289)] = 10437,
    [SMALL_STATE(290)] = 10445,
    [SMALL_STATE(291)] = 10453,
    [SMALL_STATE(292)] = 10463,
    [SMALL_STATE(293)] = 10470,
    [SMALL_STATE(294)] = 10475,
    [SMALL_STATE(295)] = 10482,
    [SMALL_STATE(296)] = 10489,
    [SMALL_STATE(297)] = 10496,
    [SMALL_STATE(298)] = 10503,
    [SMALL_STATE(299)] = 10510,
    [SMALL_STATE(300)] = 10515,
    [SMALL_STATE(301)] = 10522,
    [SMALL_STATE(302)] = 10529,
    [SMALL_STATE(303)] = 10533,
    [SMALL_STATE(304)] = 10537,
    [SMALL_STATE(305)] = 10541,
    [SMALL_STATE(306)] = 10545,
    [SMALL_STATE(307)] = 10549,
    [SMALL_STATE(308)] = 10553,
    [SMALL_STATE(309)] = 10557,
    [SMALL_STATE(310)] = 10561,
    [SMALL_STATE(311)] = 10565,
    [SMALL_STATE(312)] = 10569,
    [SMALL_STATE(313)] = 10573,
    [SMALL_STATE(314)] = 10577,
    [SMALL_STATE(315)] = 10581,
    [SMALL_STATE(316)] = 10585,
    [SMALL_STATE(317)] = 10589,
    [SMALL_STATE(318)] = 10593,
    [SMALL_STATE(319)] = 10597,
    [SMALL_STATE(320)] = 10601,
    [SMALL_STATE(321)] = 10605,
    [SMALL_STATE(322)] = 10609,
    [SMALL_STATE(323)] = 10613,
    [SMALL_STATE(324)] = 10617,
    [SMALL_STATE(325)] = 10621,
    [SMALL_STATE(326)] = 10625,
    [SMALL_STATE(327)] = 10629,
    [SMALL_STATE(328)] = 10633,
    [SMALL_STATE(329)] = 10637,
    [SMALL_STATE(330)] = 10641,
    [SMALL_STATE(331)] = 10645,
    [SMALL_STATE(332)] = 10649,
    [SMALL_STATE(333)] = 10653,
    [SMALL_STATE(334)] = 10657,
    [SMALL_STATE(335)] = 10661,
    [SMALL_STATE(336)] = 10665,
    [SMALL_STATE(337)] = 10669,
    [SMALL_STATE(338)] = 10673,
    [SMALL_STATE(339)] = 10677,
    [SMALL_STATE(340)] = 10681,
};

static const TSParseActionEntry ts_parse_actions[] = {
    [0] = {.entry = {.count = 0, .reusable = false}},
    [1] = {.entry = {.count = 1, .reusable = false}},
    RECOVER(),
    [3] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_source_file, 0),
    [5] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(283),
    [7] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(4),
    [9] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(298),
    [11] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(113),
    [13] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(273),
    [15] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(290),
    [17] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(267),
    [19] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(256),
    [21] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(116),
    [23] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(12),
    [25] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(72),
    [27] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_source_file, 1),
    [29] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(301),
    [31] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(3),
    [33] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(202),
    [35] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(263),
    [37] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(282),
    [39] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(257),
    [41] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(201),
    [43] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(17),
    [45] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(73),
    [47] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(316),
    [49] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    [51] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(297),
    [54] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(4),
    [57] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(113),
    [60] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(273),
    [63] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(290),
    [66] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(267),
    [69] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(256),
    [72] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(116),
    [75] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(12),
    [78] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(72),
    [81] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(296),
    [83] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(7),
    [85] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(137),
    [87] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(163),
    [89] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(270),
    [91] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(279),
    [93] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(254),
    [95] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(164),
    [97] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(15),
    [99] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(165),
    [101] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(296),
    [104] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(7),
    [107] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(163),
    [110] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(270),
    [113] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(279),
    [116] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(254),
    [119] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(164),
    [122] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(15),
    [125] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_source_file_repeat2, 2),
    SHIFT_REPEAT(165),
    [128] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(110),
    [130] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(132),
    [132] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(200),
    [134] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(297),
    [136] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_source_file, 2),
    [138] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(156),
    [140] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(170),
    [142] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(168),
    [144] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(103),
    [146] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(87),
    [148] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(226),
    [150] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(248),
    [152] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_list_content, 3, .production_id = 7),
    [154] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_list_content, 2),
    [156] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(295),
    [158] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(5),
    [160] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(239),
    [162] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(268),
    [164] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(287),
    [166] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(255),
    [168] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(240),
    [170] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(21),
    [172] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(241),
    [174] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(294),
    [176] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(6),
    [178] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(78),
    [180] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(261),
    [182] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(289),
    [184] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(258),
    [186] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(77),
    [188] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(19),
    [190] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(76),
    [192] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_variable_identifier, 1),
    [194] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_variable_identifier, 1),
    [196] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_fn_identifier, 1),
    [198] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_fn_identifier, 1),
    [200] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_string_literal, 3, .production_id = 5),
    [202] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_string_literal, 3, .production_id = 5),
    [204] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_string_literal, 2, .production_id = 1),
    [206] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_string_literal, 2, .production_id = 1),
    [208] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_expression, 1),
    [210] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_expression, 1),
    [212] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_bool_literal, 1),
    [214] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_bool_literal, 1),
    [216] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_uint64_literal, 2, .production_id = 2),
    [218] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_uint64_literal, 2, .production_id = 2),
    [220] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_uint32_literal, 2, .production_id = 2),
    [222] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_uint32_literal, 2, .production_id = 2),
    [224] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_uint16_literal, 2, .production_id = 2),
    [226] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_uint16_literal, 2, .production_id = 2),
    [228] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(86),
    [230] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(207),
    [232] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(317),
    [234] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(167),
    [236] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_uint8_literal, 2, .production_id = 2),
    [238] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_uint8_literal, 2, .production_id = 2),
    [240] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_tuple_literal, 6, .production_id = 18),
    [242] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_tuple_literal, 6, .production_id = 18),
    [244] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_uint128_literal, 2, .production_id = 2),
    [246] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_uint128_literal, 2, .production_id = 2),
    [248] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_list_literal, 2, .production_id = 3),
    [250] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_list_literal, 2, .production_id = 3),
    [252] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_int8_literal, 2, .production_id = 2),
    [254] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_int8_literal, 2, .production_id = 2),
    [256] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_int16_literal, 2, .production_id = 2),
    [258] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_int16_literal, 2, .production_id = 2),
    [260] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_int32_literal, 2, .production_id = 2),
    [262] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_int32_literal, 2, .production_id = 2),
    [264] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_int64_literal, 2, .production_id = 2),
    [266] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_int64_literal, 2, .production_id = 2),
    [268] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_int128_literal, 2, .production_id = 2),
    [270] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_int128_literal, 2, .production_id = 2),
    [272] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_paren_expression, 3, .production_id = 4),
    [274] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_paren_expression, 3, .production_id = 4),
    [276] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_char_literal, 3, .production_id = 6),
    [278] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_char_literal, 3, .production_id = 6),
    [280] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_list_literal, 3, .production_id = 8),
    [282] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_list_literal, 3, .production_id = 8),
    [284] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_infix_operation, 3, .production_id = 9),
    [286] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_infix_operation, 3, .production_id = 9),
    [288] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(66),
    [290] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(65),
    [292] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(64),
    [294] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(59),
    [296] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(59),
    [298] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_function_call, 4, .production_id = 10),
    [300] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_function_call, 4, .production_id = 10),
    [302] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_tuple_literal, 5, .production_id = 14),
    [304] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_tuple_literal, 5, .production_id = 14),
    [306] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_let_expression, 6, .production_id = 16),
    [308] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_let_expression, 6, .production_id = 16),
    [310] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(48),
    [312] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_fn_decl, 7, .production_id = 21),
    [314] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_fn_decl, 7, .production_id = 21),
    [316] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(51),
    [318] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(30),
    [320] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(43),
    [322] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(44),
    [324] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(45),
    [326] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(46),
    [328] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(46),
    [330] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(47),
    [332] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym_source_file_repeat2, 1),
    [334] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym_source_file_repeat2, 1),
    [336] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(53),
    [338] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(57),
    [340] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(57),
    [342] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(58),
    [344] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_type_identifier, 1),
    [346] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_type_identifier, 1),
    [348] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_module_identifier, 1),
    [350] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(34),
    [352] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(35),
    [354] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(37),
    [356] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(39),
    [358] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(39),
    [360] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(104),
    [362] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(40),
    [364] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(38),
    [366] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_qualified_type_name, 1),
    [368] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_qualified_type_name, 1),
    [370] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_type_reference, 1),
    [372] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_type_reference, 1),
    [374] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(142),
    [376] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_list_type_reference, 4, .production_id = 22),
    [378] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_list_type_reference, 4, .production_id = 22),
    [380] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_qualified_type_name, 2),
    [382] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_qualified_type_name, 2),
    [384] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(204),
    [386] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_tuple_type_reference, 5, .production_id = 23),
    [388] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_tuple_type_reference, 5, .production_id = 23),
    [390] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(228),
    [392] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(136),
    [394] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_tuple_type_reference, 6, .production_id = 26),
    [396] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_tuple_type_reference, 6, .production_id = 26),
    [398] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_builtin_type, 1),
    [400] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_builtin_type, 1),
    [402] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym_source_file_repeat1, 2),
    [404] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_source_file_repeat1, 2),
    SHIFT_REPEAT(300),
    [407] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym_source_file_repeat1, 2),
    [409] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_source_file_repeat1, 2),
    SHIFT_REPEAT(298),
    [412] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_list_content, 1),
    [414] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(25),
    [416] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym_list_content_repeat1, 2, .production_id = 12),
    [418] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(94),
    [420] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(68),
    [422] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(232),
    [424] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(54),
    [426] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(147),
    [428] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(62),
    [430] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(182),
    [432] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(31),
    [434] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(125),
    [436] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(61),
    [438] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym_tuple_literal_the_rest_repeat1, 2, .production_id = 17),
    [440] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(67),
    [442] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(29),
    [444] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(52),
    [446] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(50),
    [448] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(71),
    [450] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(70),
    [452] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(41),
    [454] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(63),
    [456] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(49),
    [458] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(36),
    [460] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_type_decl, 4, .production_id = 11),
    [462] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_type_decl, 4, .production_id = 11),
    [464] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_qualified_fn_name, 1),
    [466] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_qualified_fn_name, 1),
    [468] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_qualified_fn_name, 2),
    [470] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_qualified_fn_name, 2),
    [472] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_digits, 1),
    [474] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(162),
    [476] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(161),
    [478] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(160),
    [480] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(158),
    [482] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(157),
    [484] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(243),
    [486] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(218),
    [488] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(245),
    [490] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(246),
    [492] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(247),
    [494] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(148),
    [496] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(159),
    [498] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(155),
    [500] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(154),
    [502] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(133),
    [504] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(194),
    [506] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(197),
    [508] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(196),
    [510] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(195),
    [512] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(198),
    [514] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(83),
    [516] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(81),
    [518] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(80),
    [520] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(79),
    [522] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(85),
    [524] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(292),
    [526] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(264),
    [528] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym_fn_decl_params_repeat1, 2),
    [530] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_fn_decl_params_repeat1, 2),
    SHIFT_REPEAT(292),
    [533] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_fn_decl_params_repeat1, 2),
    SHIFT_REPEAT(260),
    [536] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(75),
    [538] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(278),
    [540] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(237),
    [542] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(236),
    [544] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(235),
    [546] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(234),
    [548] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(233),
    [550] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(199),
    [552] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_fn_decl_params, 1),
    [554] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(260),
    [556] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(169),
    [558] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(186),
    [560] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(185),
    [562] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(184),
    [564] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(183),
    [566] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(131),
    [568] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(129),
    [570] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(128),
    [572] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(127),
    [574] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(126),
    [576] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(242),
    [578] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(252),
    [580] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(166),
    [582] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(89),
    [584] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(90),
    [586] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(91),
    [588] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(92),
    [590] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(93),
    [592] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(153),
    [594] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(152),
    [596] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(151),
    [598] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(150),
    [600] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(149),
    [602] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(138),
    [604] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym_string_content_repeat1, 2),
    [606] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_string_content_repeat1, 2),
    SHIFT_REPEAT(274),
    [609] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(167),
    [611] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym_qualified_fn_name_repeat1, 2),
    [613] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_qualified_fn_name_repeat1, 2),
    SHIFT_REPEAT(316),
    [616] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(191),
    [618] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(109),
    [620] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_string_content, 1),
    [622] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(274),
    [624] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(324),
    [626] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_tuple_literal_the_rest, 1, .production_id = 15),
    [628] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_list_content, 2, .production_id = 7),
    [630] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(23),
    [632] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(73),
    [634] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_fn_decl_param, 5, .production_id = 20),
    [636] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_fn_decl_param, 5, .production_id = 20),
    [638] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym_type_reference_tuple_the_rest_repeat1, 2, .production_id = 27),
    [640] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_type_reference_tuple_the_rest_repeat1, 2, .production_id = 27),
    SHIFT_REPEAT(109),
    [643] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym_tuple_literal_the_rest_repeat1, 2, .production_id = 19),
    [645] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_tuple_literal_the_rest_repeat1, 2, .production_id = 19),
    SHIFT_REPEAT(38),
    [648] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym_list_content_repeat1, 2, .production_id = 13),
    [650] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_list_content_repeat1, 2, .production_id = 13),
    SHIFT_REPEAT(33),
    [653] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_type_reference_tuple_the_rest, 1, .production_id = 24),
    [655] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(72),
    [657] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(334),
    [659] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym_type_reference_tuple_the_rest_repeat1, 2, .production_id = 25),
    [661] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(134),
    [663] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(146),
    [665] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(145),
    [667] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(88),
    [669] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(74),
    [671] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(95),
    [673] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(96),
    [675] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(293),
    [677] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(144),
    [679] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(205),
    [681] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(84),
    [683] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(284),
    [685] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(174),
    [687] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(115),
    [689] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(105),
    [691] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(179),
    [693] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(180),
    [695] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(56),
    [697] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(181),
    [699] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(55),
    [701] = {.entry = {.count = 1, .reusable = true}},
    ACCEPT_INPUT(),
    [703] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_character, 1),
    [705] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(188),
    [707] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(123),
    [709] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(122),
    [711] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(82),
    [713] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(106),
    [715] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(98),
    [717] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(230),
    [719] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(124),
    [721] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(32),
    [723] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(60),
    [725] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(42),
    [727] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(69),
    [729] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(220),
    [731] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(219),
    [733] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(249),
};

#ifdef __cplusplus
extern "C"
{
#endif
#ifdef _WIN32
#define extern __declspec(dllexport)
#endif

  extern const TSLanguage *tree_sitter_darklang(void)
  {
    static const TSLanguage language = {
        .version = LANGUAGE_VERSION,
        .symbol_count = SYMBOL_COUNT,
        .alias_count = ALIAS_COUNT,
        .token_count = TOKEN_COUNT,
        .external_token_count = EXTERNAL_TOKEN_COUNT,
        .state_count = STATE_COUNT,
        .large_state_count = LARGE_STATE_COUNT,
        .production_id_count = PRODUCTION_ID_COUNT,
        .field_count = FIELD_COUNT,
        .max_alias_sequence_length = MAX_ALIAS_SEQUENCE_LENGTH,
        .parse_table = &ts_parse_table[0][0],
        .small_parse_table = ts_small_parse_table,
        .small_parse_table_map = ts_small_parse_table_map,
        .parse_actions = ts_parse_actions,
        .symbol_names = ts_symbol_names,
        .field_names = ts_field_names,
        .field_map_slices = ts_field_map_slices,
        .field_map_entries = ts_field_map_entries,
        .symbol_metadata = ts_symbol_metadata,
        .public_symbol_map = ts_symbol_map,
        .alias_map = ts_non_terminal_alias_map,
        .alias_sequences = &ts_alias_sequences[0][0],
        .lex_modes = ts_lex_modes,
        .lex_fn = ts_lex,
        .primary_state_ids = ts_primary_state_ids,
    };
    return &language;
  }
#ifdef __cplusplus
}
#endif
