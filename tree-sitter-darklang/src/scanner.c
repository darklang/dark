...
//   // If the scanner is in the middle of processing a dedentation sequence.
...
//   // If the next character is a newline, process the new line and any following indentation
//   if (lexer->lookahead == '\n')
//   {
//     lexer->advance(lexer, true); // Skip the newline
//     // Skip any following whitespace on the new line to count indentation
//     while (iswspace(lexer->lookahead) && lexer->lookahead != '\n')
//     {
//       lexer->advance(lexer, true); // Move lexer forward over whitespace
//     }

//     unsigned int new_indent_length = lexer->get_column(lexer); // Get the indentation level at the new line.
//     if (lexer->lookahead != 0)                                 // If we're not at the end of the file.
//     {
//       // If the new indentation is greater than the last one, push it onto the stack and emit an INDENT token.
...
//       // If the new indentation is less, start dedent processing and emit a DEDENT token.
//       else if (new_indent_length < (unsigned int)scanner->indent_stack[scanner->stack_size])
//       {
//         pop(scanner);
