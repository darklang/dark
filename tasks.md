...
OK let's now go back to working on the 'main' CLI --
(the UX someone has if they just run ./scripts/run-cli and hit the code in the root packages/darklang/cli stuff)

I've created a new document tasks.md in the root directory here.
Please read it, do appropriate research, and mark them as complete one by one. Eventually, it'd be great to have all tasks complete, roughly in order but you can jump around a bit if useful.
As you complete each item, check in with me, ask me to test it (and remind me how to, briefly), and then commit.

Before starting, it'd be worth reviewing the codebase in there, ignoring anything in the experiments subdirectory.
