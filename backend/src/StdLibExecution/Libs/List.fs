...
let fns : List<BuiltInFn> =
  [ { name = fn "singleton" 0
      ...
      fn =
        (function
        | _, _, [ v ] ->
          // VTTODO we could get a Known type here - but probably not needed?
          Ply(Dval.list Unknown [ v ])
        | _ -> incorrectArgs ())
    ...
    { name = fn "range" 0
      ...
      fn =
        ...
        | _, _, [ DInt start; DInt stop ] ->
          [ start..stop ] |> List.map DInt |> Dval.list (Known KTInt) |> Ply
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
      ...
      fn =
        ...
        | state, _, [ DList(_, l); init; DFnVal b ] ->
          ...
          uply {
            ...
            // VTTODO it seems possible to get a Known type here
            // in case of an empty resultant list
    ...
    { name = fn "flatten" 0
      ...
      fn =
        ...
        | _, _, [ DList(_, l) ] ->
          ...
          // VTTODO we could get a Known type here
          List.fold f [] l |> Dval.list Unknown |> Ply
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
        ...
        | state, _, [ DList(typ, list); DFnVal f ] ->
          ...
          uply {
            try
              ...
              return array |> Array.toList |> Dval.list typ |> Dval.resultOk
            with e ->
    ...
    { name = fn "append" 0
      ...
      fn =
        (function
        | _, _, [ DList(typ1, l1); DList(typ2, l2) ] ->
          // merge the 2
          match Dval.mergeValueTypes typ1 typ2 with
          | Ok typ -> Ply(DList(typ, List.append l1 l2)) // no checking for DError required
          | Error _ -> incorrectArgs () // VTTODO is this right?
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
      ...
      fn =
        ...
        | state, _, [ DList(_, l); DFnVal b ] ->
          uply {
            ...
            match abortReason.Value with
            | None -> return Dval.list Unknown result // VTTODO: consider using output type of passed-in fn
            | Some v -> return v
          }
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
    ...
    { name = fn "drop" 0
      ...
      fn =
        ...
        (function
        | _, _, [ DList(typ, l); DInt c ] ->
          if c < 0L then Ply(DList(typ, l))
          elif c > int64 (List.length l) then Ply(Dval.list typ [])
          else Ply(DList(typ, List.skip (int c) l))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "take" 0
      ...
      fn =
        (function
        | _, _, [ DList(typ, l); DInt c ] ->
          if c < 0L then Ply(DList(typ, [])) // VTTODO is this right, or Dval.list []?
          elif c >= int64 (List.length l) then Ply(DList(typ, l)) // VTDODO or Dval.list l?
          else Ply(DList(typ, List.take (int c) l))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
      ...
      fn =
        ...
        | state, _, [ DList(_, l); DFnVal b ] ->
          uply {
            ...
            return Dval.list Unknown result // VTTODO could use the return type of the passed-in fn
          }
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
    ...
    { name = fn "map2shortest" 0
      ...
      fn =
        (function
        | state, _, [ DList(_, l1); DList(_, l2); DFnVal b ] ->
          uply {
        ...
        | state, _, [ DList(_, l1); DList(_, l2); DFnVal b ] ->
          uply {
            if List.length l1 <> List.length l2 then
              ...
              return
                result
                |> Dval.list Unknown // VTTODO could use the return type of the passed-in fn
                |> Dval.optionSome
          }
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
    ...
    { name = fn "zipShortest" 0
      ...
      fn =
        ...
        | _, _, [ DList(typ1, l1); DList(typ2, l2) ] ->
          ...
          |> Dval.list (Known(KTTuple(typ1, typ2, [])))
          |> Ply
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
    ...
    { name = fn "unzip" 0
      ...
      fn =
        ...
        | _, _, [ DList(typ, l) ] ->
          ...
          let (typ1, typ2) =
            match typ with
            | Known(KTTuple(typ1, typ2, [])) -> typ1, typ2
            | Unknown -> Unknown, Unknown
            | _ -> incorrectArgs ()

          let (l1, l2) = l |> List.rev |> List.fold f ([], [])

          DTuple(Dval.list typ1 l1, Dval.list typ2 l2, []) |> Ply
      ...
      fn =
        ...
        | state, _, [ DList(typ, l); DFnVal fn ] ->
          uply {
            ...
            let fakeKey = List.tryFind (fun (k, _) -> Dval.isFake k) result

            match fakeKey with
            | Some(key, _) -> return key
            | None ->
              let keyType = Unknown // VTTODO we could extract this from the passed-in fn

              let groups =
                result
                |> List.groupBy fst
                |> List.map (fun (key, elementsWithKey) ->
                  let l = List.map snd elementsWithKey
                  DTuple(key, Dval.list typ l, []))
                |> Dval.list (Known(KTTuple(keyType, typ, [])))
      ...
      fn =
        ...
        | state, _, [ DList(typ, l); DFnVal fn ] ->
          uply {
            ...
            | Ok(a, b) -> return DTuple(Dval.list typ a, Dval.list typ b, [])
