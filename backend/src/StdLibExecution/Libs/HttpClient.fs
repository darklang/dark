...
let constants : List<BuiltInConstant> =
  [ { name = constant "HttpClient" "formContentType" 0
    ...
    { name = constant "HttpClient" "plainTextContentType" 0
      returnType = TDict TString
      description =
    ...
    { name = constant "HttpClient" "htmlContentType" 0
      returnType = TDict TString
      description =
        "Returns a header <type Dict> with {{'Content-Type'}} set for html requests
        or responses"
      constant =
        Ply(DDict(Map.ofList [ "Content-Type", DString "text/html; charset=utf-8" ]))
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }

    { name = constant "HttpClient" "jsonContentType" 0
      returnType = TDict TString
      description =
