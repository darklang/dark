...
let insertTypes (types : List<PT.PackageType.PackageType>) : Task<unit> =
  task {
    ...
    |> List.map (fun typ ->
      ...
      let (Hash hashStr) = typ.hash // Use the hash already in the type, don't recompute
...
let insertValues (values : List<PT.PackageValue.PackageValue>) : Task<unit> =
  task {
    ...
    |> List.map (fun v ->
      ...
      let (Hash hashStr) = v.hash // Use the hash already in the value
...
let insertFns (fns : List<PT.PackageFn.PackageFn>) : Task<unit> =
  task {
    ...
    |> List.map (fun fn ->
      ...
      let (Hash hashStr) = fn.hash // Use the hash already in the function
