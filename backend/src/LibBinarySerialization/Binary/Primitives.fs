...
module Writer =
  ...
  let writeOption (writer: BinaryWriter) (writeValue: BinaryWriter -> 'T -> unit) (value: 'T option) =
    match value with
  ...
  let writeList (writer: BinaryWriter) (writeItem: BinaryWriter -> 'T -> unit) (items: 'T list) =
    writeVarint writer items.Length
    items |> List.iter (writeItem writer)
  
  /// Write an array with count prefix
  let writeArray (writer: BinaryWriter) (writeItem: BinaryWriter -> 'T -> unit) (items: 'T array) =
    writeVarint writer items.Length
  ...
  let writeUInt64 (writer: BinaryWriter) (value: uint64) =
    writer.Write(value)
...
module Reader =
  ...
  let readOption (reader: BinaryReader) (readValue: BinaryReader -> 'T) : 'T option =
  ...
  /// Read a UInt64 value
  let readUInt64 (reader: BinaryReader) : uint64 =
    reader.ReadUInt64()
...
module Serialization =
  ...
  let serializeWithHeader (typeId: TypeId) (writePayload: BinaryWriter -> unit) : byte[] =
