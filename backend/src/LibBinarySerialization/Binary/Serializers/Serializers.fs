...
/// Serialize NEList<'a>
module NEList =
  let write (writeItem: BinaryWriter -> 'a -> unit) (writer: BinaryWriter) (nel: NEList<'a>) =
...
module NameResolutionError =
  ...
  let read (reader: BinaryReader) : NameResolutionError =
    ...
    | b -> raise (BinaryFormatException(CorruptedData $"Invalid NameResolutionError tag: {b}"))

/// Serialize NameResolution<'a>
module NameResolution =
  let write (writeValue: BinaryWriter -> 'a -> unit) (writer: BinaryWriter) (result: NameResolution<'a>) =
    match result with
    | Ok value ->
        writer.Write(0uy)
  ...
  let read (readValue: BinaryReader -> 'a) (reader: BinaryReader) : NameResolution<'a> =
    match reader.ReadByte() with
    | 0uy -> Ok (readValue reader)
    | 1uy -> Error (NameResolutionError.read reader)
    | b -> raise (BinaryFormatException(CorruptedData $"Invalid NameResolution tag: {b}"))
...
module FQTypeName =
  module Package =
    ...
    let read (reader: BinaryReader) : FQTypeName.Package =
...
module FQConstantName =
  ...
  let read (reader: BinaryReader) : FQConstantName.FQConstantName =
    ...
    | b -> raise (BinaryFormatException(CorruptedData $"Invalid FQConstantName tag: {b}"))
...
module FQFnName =
  ...
  module Package =
    ...
    let read (reader: BinaryReader) : FQFnName.Package =
      Reader.readGuid reader
