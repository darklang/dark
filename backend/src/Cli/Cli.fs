...
let main (args : string[]) =
  try
    // How long the process took to reach here: `cli.total` starts after resource extraction and
    // can't see runtime init, assembly loading or JIT. Read the switch before measuring anything,
    // because `Process.GetCurrentProcess()` is not free on a command this short.
    ...
    Telemetry.time "cli.keepLocalEdits" [] (fun () ->
      ...
      | held ->
        ...
        if kept <> [] then
          ...
          let more =
            let n = (if edited = [] then List.length kept else List.length edited) - 3
