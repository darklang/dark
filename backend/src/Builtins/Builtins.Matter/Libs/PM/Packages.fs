...
let private branchOfParam (branchId : string) : Option<string> =
  if branchId = "" then None else Some branchId
    ...
    { name = fn "pmSearchNames" 0
      ...
      fn =
        ...
        | _, _, _, [ DString branchId; query as DRecord(_, _, _, _fields) ] ->
          uply {
            let searchQuery = PT2DT.Search.SearchQuery.fromDT query
            // Through the branch OVERLAY rather than a branch chain. A branch is not a scope you walk up
            // any more -- it is its ops laid over main -- so there is no chain, and the overlay is what
            // makes a branch's items show up in ls/view/tree/search rather than only in eval.
            let pm = LibDB.PackageManager.ptForBranch (branchOfParam branchId)
            let! results = pm.search searchQuery
