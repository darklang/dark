...
let fns () : List<BuiltInFn> =
  [ { name = fn "equals" 0
    ...
    { name = fn "testApply" 0
      typeParams = []
      parameters =
        [ Param.make "fn" (TVariable "fn") "the function under test, of any arity"
          Param.make
            "inputs"
            (TVariable "inputs")
      ...
      description =
        ...
        + "several, and passed whole to one. `Stdlib.Test.apply` wraps it, and says why "
        + "it is native."
      fn =
        ...
        // How many more arguments the function takes. `None` if the function cannot be
        // found. The call below then reports that.
        let remaining
          ...
          uply {
            ...
            | Ok args ->
              ...
              | Error(rte, cs) ->
                ...
                // Render the message with the Dark printer on this state, so names in
                // it resolve on the branch the test runs on.
                let! rendered = Exe.runtimeErrorToString state rte
                let message =
                  match rendered with
                  | Ok(DString message) -> message
                  // The printer failed. Return text that says so instead of raising.
                  // A test that compares messages then fails and shows this text.
