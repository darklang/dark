...
let fns () : List<BuiltInFn> =
  [ { name = fn "uint16Mod" 0
    ...
    { name = fn "uint16ToString" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DUInt16 a |]) -> Ply(DString(string a))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "uint16ToFloat" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DUInt16 a |]) -> Ply(DFloat(float a))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "uint16Sqrt" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DUInt16 a |]) -> Ply(DFloat(sqrt (float a)))
        | _ -> incorrectArgs ())
    ...
    { name = fn "uint16FromUInt8" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DUInt8 a |]) -> DUInt16(uint16 a) |> Ply
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "uint16BitwiseNot" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DUInt16 a |]) -> Ply(DUInt16(~~~a))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "uint16ShiftLeft" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DUInt16 a; DUInt16 b |]) -> Ply(DUInt16(a <<< int b))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
