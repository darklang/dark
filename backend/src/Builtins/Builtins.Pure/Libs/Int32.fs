  ...
  [ { name = fn "int32Mod" 0
    ...
    { name = fn "int32Remainder" 0
      ...
      fn =
        ...
        (function
        | struct (_, vm, _, [ DInt32 v; DInt32 d ]) ->
          (try
            v % d |> DInt32 |> resultOk
           with e ->
             if d = 0 then
    ...
    { name = fn "int32Sqrt" 0
      ...
      fn =
        (function
        | struct (_, _, _, [ DInt32 a ]) -> Ply(DFloat(sqrt (float a)))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "int32ToFloat" 0
      ...
      fn =
        (function
        | struct (_, _, _, [ DInt32 a ]) -> Ply(DFloat(float a))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "int32ToString" 0
      ...
      fn =
        (function
        | struct (_, _, _, [ DInt32 int ]) -> Ply(DString(string int))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "int32FromInt8" 0
      ...
      fn =
        (function
        | struct (_, _, _, [ DInt8 a ]) -> DInt32(int32 a) |> Ply
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "int32FromUInt8" 0
      ...
      fn =
        (function
        | struct (_, _, _, [ DUInt8 a ]) -> DInt32(int32 a) |> Ply
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "int32FromInt16" 0
      ...
      fn =
        (function
        | struct (_, _, _, [ DInt16 a ]) -> DInt32(int32 a) |> Ply
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "int32FromUInt16" 0
      ...
      fn =
        (function
        | struct (_, _, _, [ DUInt16 a ]) -> DInt32(int32 a) |> Ply
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "int32ShiftLeft" 0
      ...
      fn =
        (function
        | struct (_, _, _, [ DInt32 a; DInt32 b ]) -> Ply(DInt32(a <<< int b))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
