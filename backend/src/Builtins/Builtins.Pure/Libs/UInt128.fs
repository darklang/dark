...
let fns () : List<BuiltInFn> =
  [ { name = fn "uint128Mod" 0
    ...
    { name = fn "uint128ToString" 0
      ...
      fn =
        (function
        | struct (_, _, _, [ DUInt128 a ]) -> Ply(DString(string a))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "uint128ToFloat" 0
      ...
      fn =
        (function
        | struct (_, _, _, [ DUInt128 a ]) -> Ply(DFloat(float a))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "uint128Sqrt" 0
      ...
      fn =
        (function
        | struct (_, _, _, [ DUInt128 a ]) -> Ply(DFloat(sqrt (float a)))
        | _ -> incorrectArgs ())
    ...
    { name = fn "uint128FromUInt8" 0
      ...
      fn =
        ...
        | struct (_, _, _, [ DUInt8 a ]) ->
          DUInt128(System.UInt128.op_Implicit a) |> Ply
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "uint128ShiftLeft" 0
      ...
      fn =
        (function
        | struct (_, _, _, [ DUInt128 a; DUInt128 b ]) -> Ply(DUInt128(a <<< int b))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
