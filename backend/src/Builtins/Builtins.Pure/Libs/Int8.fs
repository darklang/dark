...
let fns () : List<BuiltInFn> =
  [ { name = fn "int8Mod" 0
    ...
    { name = fn "int8ToString" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DInt8 a |]) -> Ply(DString(string a))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "int8ToFloat" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DInt8 a |]) -> Ply(DFloat(float a))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "int8Sqrt" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DInt8 a |]) -> Ply(DFloat(sqrt (float a)))
        | _ -> incorrectArgs ())
    ...
    { name = fn "int8ShiftLeft" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DInt8 a; DInt8 b |]) -> Ply(DInt8(a <<< int b))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
