...
let fns () : List<BuiltInFn> =
  [ { name = fn "uint64Mod" 0
    ...
    { name = fn "uint64Sqrt" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DUInt64 a |]) -> Ply(DFloat(sqrt (float a)))
        | _ -> incorrectArgs ())
    ...
    { name = fn "uint64ToFloat" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DUInt64 a |]) -> Ply(DFloat(float a))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "uint64ToString" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DUInt64 int |]) -> Ply(DString(string int))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "uint64FromUInt8" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DUInt8 a |]) -> DUInt64(uint64 a) |> Ply
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "uint64FromUInt16" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DUInt16 a |]) -> DUInt64(uint64 a) |> Ply
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "uint64FromUInt32" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DUInt32 a |]) -> DUInt64(uint64 a) |> Ply
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "uint64BitwiseNot" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DUInt64 a |]) -> Ply(DUInt64(~~~a))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
    ...
    { name = fn "uint64ShiftLeft" 0
      ...
      fn =
        (function
        | struct (_, _, _, [| DUInt64 a; DUInt64 b |]) -> Ply(DUInt64(a <<< int b))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      capabilities = LibExecution.Capabilities.noCaps
