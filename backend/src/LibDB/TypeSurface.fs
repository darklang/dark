...
///
/// Authoring is permissive and live-on-write, so this REPORTS rather than refuses; Dark files what it
/// finds as a CONSTRAINT, the same at-rest lifecycle as an outdated usage.
///
/// SOUND, NOT COMPLETE, deliberately: a rule reports only when both types are fully concrete and no
/// substitution reconciles them. Type variables, custom types (which may be aliases), and anything
/// needing inference are left alone, because a false positive about correct code teaches people to stop
/// reading the list.
///
/// Not a type checker -- `LibExecution.TypeChecker` is the real one and works on VALUES at run time.
/// This is the part that needs no inference: types written down, propagated through lets, compared where
/// they meet. The traversal reaches everywhere an expression nests, but there are only two RULES, and it
/// knowingly looks away from a pipe's arguments (the piped value is prepended at runtime, so AST index 0
/// is parameter 1).
///
...
///   - a function against a rigid scalar. This catches an under-applied call: partial application is
///     ordinary in Dark, so the mistake isn't `f a b` where `f` takes three, it's handing that function
///     to something expecting a `String`.
///
...
let rec private checkExpr
  ...
  uply {
    ...
    | PT.EApply(_, PT.EFnName(_, nr), [], args) ->
      ...
      | Ok resolved ->
        ...
        | PT.FQFnName.Package hash ->
          ...
          | Some fn ->
            let ps = NEList.toList fn.parameters
            // Only positions that exist on BOTH sides: an over-applied call has more arguments than
            // parameters, and `List.zip` on lists of different lengths throws.
      ...
      let! inCases =
        ...
        |> Ply.List.mapSequentially (fun (case_ : PT.MatchCase) ->
          uply {
            ...
            // `symbolsUsed` for a match pattern, and keeping a local the pattern happens to shadow would
            // hand this the outer type for an inner name -- a confidently WRONG finding.
    ...
    // Sub-expressions are walked, but a pipe's call ARGUMENTS are not checked positionally: the piped
    // value is prepended at runtime, so AST index 0 is parameter 1 (see the module doc).
...
/// Hashes already checked under THIS build, and found clean. Lives in F# next to the check because it is
/// an implementation detail of the check, and because the fill has to be callable from the reload.
module Cache =
  ...
  /// status` asks for constraints on every invocation, and a cold cache makes that a walk of the whole
  /// package tree (ruinous under tracing, which records every call's arguments). The cost belongs where
  /// the tree is already being rebuilt, once. A known-BAD hash is deliberately not recorded: a finding
  /// has to keep existing until the code changes.
  let checkAndRecord
    (getFn : PT.FQFnName.Package -> Ply<Option<PT.PackageFn.PackageFn>>)
    (hashes : List<string>)
    ...
    task {
      ...
      // takes down the package load for the whole store, which an advisory check has no business doing.
      // A throwing item reports nothing and is NOT recorded as clean, so it is retried next time.
      let! results =
        todo
  ...
  /// Every live fn, checked and recorded. Reads the hash list itself rather than being handed one --
  /// the caller is asking "what is wrong in the store", not about a particular set.
  let checkAll
    (getFn : PT.FQFnName.Package -> Ply<Option<PT.PackageFn.PackageFn>>)
    : Task<List<string * Mismatch>> =
    task {
