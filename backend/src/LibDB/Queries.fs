...
/// Deserialize an op_blob (as stored in package_ops) into a PackageOp. The one F#
/// primitive Dark needs to read STRUCTURED ops -- the binary format isn't Dark-decodable.
/// The QUERY that selects the blobs lives in Dark (Stdlib.Sqlite); this is just the decode,
/// so it's reusable for any op read (recent, pending, a change's ops) rather than a bespoke
/// per-query builtin. `id` is used only for error context, not the decode.
...
let private getDependentsByLocationsChunk
  ...
  task {
    if List.isEmpty targets then
      ...
      // Return the dependent HASHES only. Resolving each to a location is the
      // caller's job, because on a branch the answer isn't in `locations` at all: a
      // branch's items deliberately have no rows there (that's the name isolation),
      // so joining here would make every branch-authored dependent invisible to
      // propagation. The caller merges the branch overlay over main and decides.
      let sql =
        $"""
...
///
/// LIST-valued on purpose: content-addressing means one hash can be live at SEVERAL
/// names (every `(x: Int64): Int64 = x + 1L` in the store is literally the same
/// item). Collapsing to one location here silently drops the other dependents from a
/// propagation, which is a wrong answer, not a rough one.
...
/// Every op NOT tagged to a branch, committed or not. Branch ops are branch-pending
/// rather than main WIP.
///
/// Named "WIP" for history and it is worth knowing that it does NOT mean
/// "uncommitted": there IS a `commit_hash` column, and this query deliberately
/// ignores it. `Draft.rebuild` depends on that -- it re-inserts what this returns,
/// so if it filtered to the draft the rebuild would delete all of history and put
/// back only the uncommitted part.
...
/// Map of WIP op id -> its current origin_ts. Used by WipRefresh to PRESERVE
/// authoring stamps across a discard+reinsert: an op that survives re-stabilization
/// unchanged (same content id) keeps its original origin_ts instead of being
/// re-stamped with `nextOriginTs`. Without this, reinserting the whole ~10k-op store
/// in one process inflates `lastOriginTs` ~1ms/op (~10s into the FUTURE), and those
/// future stamps then make the next genuine update look stale to the fold's
/// timestamp-LWW. The COMMIT each main op was committed into, for ops that have one.
/// WipRefresh deletes and re-inserts the whole main log when a hash changes, so
/// without carrying this forward every refresh would silently un-commit the entire
/// history and `dark status` would report the world as your draft. Same reason
/// `getWipOpOriginTs` exists, for the same rewrite.
...
/// Load the set of package fn hashes currently marked `Harmful`.
/// Backs `PackageManager.isHarmful` via a cache, which the interpreter
/// consults before each package-fn call.
///
...
/// The explicit propagation choices of one kind that apply on <param branchId> ("" = main): the branch's
/// own rows plus main's. A choice is (owner, modules, name) -> policy, where `name = ""` covers a whole
/// MODULE rather than one item.
///
/// MIRRORS `Darklang.SCM.Propagation`, which owns the same table and resolves it the same
/// most-specific-first way for display. Change both or neither: the two disagreeing means `dark propagate`
/// shows you one thing and the cascade does another.
///
/// Scoping matters in BOTH directions. Without the filter, main's cascade would honour a pin made on an
/// unrelated branch. Including main's rows is the inheritance half: a branch starts from main's choices
...
let private getPropagationPolicy
  ...
  task {
    // A main row is inherited ONLY where the branch has no row of its OWN for that
    // key, whatever policy that row names. Inheriting unconditionally put the same
    // key in both the pin set and the follow set, and `isPinned` consults pins first
    // -- so main's `pin` beat the branch's explicit `follow` and the cascade did the
    // opposite of what `dark propagate` displayed. That disagreement is the single
    // most confusing way this feature can fail, which is why the docstring above
    // warns about it.
    let! rows =
      Sql.query
