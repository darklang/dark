...
let rtTyp (submodules : List<string>) (name : string) (version : int) : TypeName.T =
...
module FQName =
  ...
  let toDT (nameMapper : 'name -> Dval) (u : FQName.T<'name>) : Dval =
    let caseName, fields =
      match u with
    ...
    Dval.enum (rtTyp [ "FQName" ] "T" 0) caseName fields

  let fromDT (nameMapper : Dval -> 'name) (d : Dval) : FQName.T<'name> =
    match d with
...
module TypeName =
  ...
  let toDT (u : TypeName.T) : Dval = FQName.toDT Name.toDT u
  let fromDT (d : Dval) : TypeName.T = FQName.fromDT Name.fromDT d
...
module FnName =
  ...
  let toDT (u : FnName.T) : Dval = FQName.toDT Name.toDT u
  let fromDT (d : Dval) : FnName.T = FQName.fromDT Name.fromDT d
...
module ConstantName =
  ...
  let toDT (u : ConstantName.T) : Dval = FQName.toDT Name.toDT u
  let fromDT (d : Dval) : ConstantName.T = FQName.fromDT Name.fromDT d
...
module TypeReference =
  let rec toDT (t : TypeReference) : Dval =
    let name, fields =
      ...
      | TFn(args, ret) -> "TFn", [ DList(List.map toDT args); toDT ret ]
  ...
  let rec fromDT (d : Dval) : TypeReference =
    ...
    | DEnum(_, _, "TFn", [ DList args; ret ]) ->
      TFn(List.map fromDT args, fromDT ret)
    ...
    | DEnum(_, _, "ERecord", [ DInt id; typeName; DList fieldsList ]) ->
      let fields =
        ...
        |> List.collect (fun field ->
          ...
          | _ -> [])
      ERecord(uint64 id, TypeName.fromDT typeName, fields)
    ...
    | DEnum(_, _, "ERecordUpdate", [ DInt id; record; DList updates ]) ->
      let updates =
        ...
        |> List.collect (fun update ->
          ...
          | DTuple(DString name, expr, _) -> [ (name, fromDT expr) ]
          | _ -> [])
      ERecordUpdate(uint64 id, fromDT record, updates)
