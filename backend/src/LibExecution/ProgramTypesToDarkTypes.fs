    ...
    let toDT (i : PT.Infix) : Dval =
      let name, fields =
        ...
        | PT.InfixFnCall infixFnName ->
          "InfixFnCall", [ InfixFnName.toDT infixFnName ]
  ...
  module PipeExpr =
    ...
    let fromDT (exprFromDT : Dval -> PT.Expr) (d : Dval) : PT.PipeExpr =
      ...
      | DEnum(_,
              _,
              "EPipeEnum",
              [ DInt id; typeName; DString caseName; DList fields ]) ->
        PT.EPipeEnum(
          uint64 id,
          NameResolution.fromDT TypeName.fromDT typeName,
          caseName,
    ...
    let rec toDT (e : PT.Expr) : Dval =
      let name, fields =
        ...
        | PT.EVariable(id, varName) ->
          "EVariable", [ DInt(int64 id); DString varName ]
      ...
      let toDT (d : PT.TypeDeclaration.Definition) : Dval =
        let caseName, fields =
          ...
          | PT.TypeDeclaration.Alias typeRef ->
            "Alias", [ TypeReference.toDT typeRef ]

          | PT.TypeDeclaration.Record fields ->
            "Record",
            [ fields |> NEList.toList |> List.map RecordField.toDT |> DList ]
