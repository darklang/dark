...
module Dval =
  ...
  let list (typ : KnownType) (list : List<Dval>) : Dval =
    DList(ValueType.known typ, list)

  let dict (typ : KnownType) (entries : List<string * Dval>) : Dval =
    DDict(ValueType.known typ, Map entries)

  let dictFromMap (typ : KnownType) (entries : Map<string, Dval>) : Dval =
    DDict(ValueType.known typ, entries)
  ...
  let optionSome (innerType : KnownType) (dv : Dval) : Dval =
    DEnum(
      optionType,
      optionType,
      ignoreAndUseEmpty [ ValueType.known innerType ],
      "Some",
      [ dv ]
    )
