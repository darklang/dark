...
type VMState =
  { registers : Registers
    // "scope trees" - go up the stack to find the variable
    // issue v8 deals with
    // given outer fn and inner fn, consider what happens when you call the inner fn
    // (think lambdaImpl)
    // implicit param, placed into register when used
...
let rec execute
  ...
  uply {
    if pc >= code.Length then
      return DUnit // Or maybe this should be an error?
    else
      let instruction = code[pc]
      match instruction with
      | LoadConst(reg, value) ->
        vmState.registers[reg] <- value
        return! execute state vmState code (pc + 1)

      // define what load/store mean

      // "load" memory -> register
      | LoadVar(reg, name) ->
        // TODO: normal find, otherwise error
        vmState.registers[reg] <- Map.findUnsafe name vmState.variables
        return! execute state vmState code (pc + 1)

      | StoreVar(name, reg) ->
        let value = vmState.registers[reg]
        let newVariables = Map.add name value vmState.variables
        return! execute state { vmState with variables = newVariables } code (pc + 1)

      | Call(resultReg, fnName, typeArgs, argRegs) ->
        // may need implicit arg 'regs' or something (can wait)

        let args = argRegs |> NEList.map (fun r -> vmState.registers[r])
        let! result = callFn state fnName typeArgs args
        vmState.registers[resultReg] <- result
        return! execute state vmState code (pc + 1)

      | Jump offset -> return! execute state vmState code (pc + offset)

      | JumpIfFalse(condReg, offset) ->
        match vmState.registers[condReg] with
        | DBool false -> return! execute state vmState code (pc + offset)
        | _ -> return! execute state vmState code (pc + 1)

      | Return reg -> return vmState.registers[reg]

      | MakeList(resultReg, itemRegs) ->
        let items = itemRegs |> List.map (fun r -> vmState.registers[r])
        vmState.registers[resultReg] <- DList(ValueType.Unknown, items)
        return! execute state vmState code (pc + 1)

      // | MakeTuple(resultReg, firstReg, secondReg, restRegs) ->
      //   let first = vmState.registers[firstReg]
      //   let second = vmState.registers[secondReg]
      //   let rest = restRegs |> List.map (fun r -> vmState.registers[r])
      //   vmState.registers[resultReg] <- DTuple(first, second, rest)
      //   return! execute state vmState code (pc + 1)

      // | MakeDict(resultReg, entries) ->
      //   let dict =
      //     entries |> List.map (fun (k, r) -> (k, vmState.registers[r])) |> Map.ofList
      //   vmState.registers[resultReg] <- DDict(ValueType.Unknown, dict)
      //   return! execute state vmState code (pc + 1)

      | MakeRecord(resultReg, typeName, fields) ->
        let fieldMap =
          fields
          |> List.map (fun (name, r) -> (name, vmState.registers[r]))
          |> Map.ofList
        vmState.registers[resultReg] <-
          DRecord(typeName, typeName, VT.typeArgsTODO, fieldMap)
        return! execute state vmState code (pc + 1)

      // | MakeEnum(resultReg, typeName, caseName, fieldRegs) ->
      //   let fields = fieldRegs |> List.map (fun r -> vmState.registers[r])
      //   vmState.registers[resultReg] <-
      //     DEnum(typeName, typeName, VT.typeArgsTODO, caseName, fields)
      //   return! execute state vmState code (pc + 1)

      | GetField(resultReg, objReg, fieldName) ->
        match vmState.registers[objReg] with
        | DRecord(_, _, _, fields) ->
          match Map.tryFind fieldName fields with
          | Some value ->
            vmState.registers[resultReg] <- value
            return! execute state vmState code (pc + 1)
          | None ->
            return
              raiseRTE
                state.tracing.callStack (RuntimeError.ofString "TODO")
                //(RuntimeError.oldError $"Field {fieldName} not found")
        | other ->
          return
            raiseRTE
              state.tracing.callStack
              (RuntimeError.ofString $"Expected a record, got {other}")

      | SetField(objReg, fieldName, valueReg) ->
        match vmState.registers[objReg] with
        | DRecord(typeName, origTypeName, typeArgs, fields) ->
          let newFields = Map.add fieldName vmState.registers[valueReg] fields
          vmState.registers[objReg] <-
            DRecord(typeName, origTypeName, typeArgs, newFields)
          return! execute state vmState code (pc + 1)
        | other ->
          return
            raiseRTE
...
and callFn
  ...
  uply {
    ...
    | FQFnName.Package pkg ->
      ...
      match! state.packageManager.getFn pkg with
      | Some fn ->
        //let fn = packageFnToFn fn
        //let (id, code, localCount) = fn.impl

        let boundArgs =
          NEList.map2 (fun (p: PackageFn.Parameter) actual -> (p.name, actual)) fn.parameters args
          |> NEList.toList
          |> Map

        let localVars = Map.mergeFavoringRight state.symbolTable boundArgs

        let localCount = 0 // todo? maybe this should be in RT.PackageFn.PackageFn
        let newVMState =
          { registers = Array.zeroCreate localCount
            variables = localVars
            callStack = [] } //(0, vmState.variables) :: vmState.callStack }
        return! execute state newVMState fn.body 0
      | None ->
        // TODO
        return DUnit
  }

let eval (state : ExecutionState) (instructions : Instructions) : Ply<Dval> =
  let vmState =
    { registers = Array.zeroCreate 256 // Or some other appropriate size
      variables = Map.empty
      callStack = [] }

  execute state vmState instructions 0
