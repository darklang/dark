...
///
/// These hashes are used to uniquely identify package items and reference them
/// across the system. The hashes must be stable and deterministic - the same
/// input should always produce the same hash.
///
/// IMPORTANT: Hashes are based on structure/content only, NOT on names.
/// This allows items to be renamed without changing their hash.
...
/// This uses a simple format that captures the essential structure
module private Serialization =
  ...
  let rec expr (e : Expr) : string =
    // For expressions, we create a simplified structural representation
    // We don't need perfect round-tripping, just stable hashing
    match e with
    | EUnit _ -> "Unit"
    | EBool(_, b) -> $"Bool({b})"
    | EInt8(_, i) -> $"Int8({i})"
...
/// Hash a TypeDeclaration to generate a unique identifier for a package type
...
/// Hash a PackageValue based on its body expression only (not name)
module PackageValue =
...
/// Hash a PackageFn based on its signature and body only (not name)
module PackageFn =
  let hash
    (
    ...
    // Note: parameter names are included because they affect the structure
    // (e.g., they're used in the body), but the function name itself is not
    let paramStrs =
      parameters
      |> NEList.toList
      |> List.map (fun p -> $"{p.name}: {Serialization.typeReference p.typ}")
      |> String.concat ", "
...
/// Convenience functions for hashing full package items
let hashPackageType (typ : PackageType.PackageType) : Hash =
  TypeDeclaration.hash typ.declaration

let hashPackageValue (value : PackageValue.PackageValue) : Hash =
  PackageValue.hash value.body

let hashPackageFn (fn : PackageFn.PackageFn) : Hash =
  PackageFn.hash (fn.parameters, fn.returnType, fn.body)
