...
module Error =
  ...
  let toRuntimeError (e : Error) : RuntimeError =
    match e with
    | MismatchedRecordFields(typeName, extraFields, missingFields, context) ->
      let fields =
        [ RT2DT.TypeName.toDT typeName
          extraFields |> Set.toList |> List.map DString |> DList
          missingFields |> Set.toList |> List.map DString |> DList
          Context.toDT context ]

      RuntimeError.enum [ "TypeChecker" ] "Error" 0 "MismatchedRecordFields" fields
