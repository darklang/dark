...
let fns : List<BuiltInFn> =
  ...
  [ { name = fn "runtimeError" 0
      ...
      fn =
        (function
        | _, _, [ DString errorString ] -> Ply(DError(SourceNone, RuntimeError.oldError errorString))
        | _ -> incorrectArgs ())
    ...
    { name = fn "justWithTypeError" 0
      ...
      fn =
        (function
        | _, _, [ DString msg ] -> Ply(Dval.optionSome (DError(SourceNone, RuntimeError.oldError msg)))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "okWithTypeError" 0
      ...
      fn =
        (function
        | _, _, [ DString msg ] -> Ply(Dval.resultOk (DError(SourceNone,RuntimeError.oldError msg)))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
