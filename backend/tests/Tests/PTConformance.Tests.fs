...
/// (`packages/darklang/languageTools/programTypes.dark`).
///
/// These are two hand-maintained copies of the same shapes. We decided NOT to single-source them via codegen
/// (QUESTIONS.md): we keep them consistent by hand well enough that an alarm is sufficient, and generating
/// the Dark mirror from the F# kernel isn't worth the weeks. What makes the alarm sufficient rather than
/// wishful is that it FAILS THE BUILD -- drift stops being something you notice later.
///
...
///   - it's the drift guard the kernel-hash capability wants. A pinned ABI type whose shape moved is exactly
///     the "identity <=> frozen structure" rule being broken, and this is where you'd find out.
///
/// Scope is the PINNED ABI types (the `type/LanguageTools.ProgramTypes.*` entries in package-ref-hashes.txt).
/// That set is the ABI by definition -- it's what the F# kernel embeds in the values it constructs -- so it's
/// the set where divergence actually costs something. Unpinned helper types are free to differ.
...
/// here is a failure. Keep this list short and keep the reasons honest -- an entry here is a decision to
/// tolerate drift, so it should read like one.
...
let private pinnedProgramTypes () : List<string * string> =
  // Same file the kernel reads. `__SOURCE_DIRECTORY__` is backend/tests/Tests, so walk up to the repo root.
  let path =
...
/// ProgramTypes module -- so `FQFnName.Builtin` finds the F# `PT.FQFnName.Builtin`.
///
/// Built by scanning the assembly rather than by naming types one by one: a hand-list would silently stop
/// matching the moment someone moved a type, which is the failure mode this test exists to catch.
...
/// design (NEList vs List, Int64 vs Int) and comparing them would produce noise that trains people to
/// ignore this test. Names and arity are where the converters actually break.
type private Shape =
