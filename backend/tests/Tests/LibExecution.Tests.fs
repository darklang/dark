...
// FUTURE: drop `RT.Program.dbs`. Canvas-era relic: toplevels_v0 is global, so the
// per-canvas whitelist scopes nothing. DB builtins and Interpreter.VarNotFound would
// do a cached `toplevels_v0` name lookup instead, and `executionStateFor` stops
// taking `dbs`. Big enough for its own PR; the perf motivation is already gone.
    ...
    | Ok _ ->
      let! result =
        LibExecution.Execution.executeFunction
          ...
          errorMessageFn
          []
          (NEList.ofList (RT.DUuid PT.BranchId.Main.Guid) [ actual ])
...
let t
  ...
  testTask $"line{lineNumber}" {
    try
      ...
      // Per-expr trace-value dumping returns when the new tracer lands
      // (per-AST-node values are a follow-up).
      ignore<unit> ()
