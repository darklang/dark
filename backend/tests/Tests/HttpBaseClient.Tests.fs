/// This module coordinates testing of our `HttpBaseClient` stdlib functions.
///
/// There are a variety of http client tests located in
/// `../testfiles/httpbaseclient`. They all follow a standard format; this module
...
type TestCase =
  { /// What we expect the webserver to receive when the HttpBaseClient is used
...
let makeTest versionName filename =
  ...
  testTask $"HttpBaseClient files: {filename}" {
...
let webserver () =
  ...
  |> fun h -> h.ConfigureLogging(configureLogging "test-httpbaseclient-server")
  |> fun h ->
