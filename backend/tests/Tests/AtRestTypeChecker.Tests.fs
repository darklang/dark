...
let private unitTests =
  testList
    ...
    [ test "checks a body against its declared return type" {
      ...
      test "unwrap checks container, error and enclosing return types" {
        let resultName =
        ...
        let body returnName =
          PT.ELet(
            900UL,
            PT.LPWildcard 901UL,
            PT.EUnwrap(902UL, PT.EArg(903UL, 0)),
            PT.EEnum(
              904UL,
              nr returnName,
              [],
        ...
        oneArgFn
          ...
          PT.TInt
          (PT.EUnwrap(906UL, PT.EArg(907UL, 0)))
        |> CheckerApi.checkPackageFunction environment
        |> expectDiagnostic Checker.InvalidUnwrap
        ...
        oneArgFn
          (result PT.TInt PT.TString)
          (result PT.TInt PT.TString)
          (PT.EUnwrap(908UL, PT.EArg(909UL, 0)))
        |> CheckerApi.checkPackageFunction environment
        |> expectDiagnostic Checker.TypeMismatch
        // The outer function may return an ordinary value: only the lambdan unwraps.
        let lambda =
        ...
        let badLambda =
          PT.ELambda(
            ...
            PT.EUnwrap(917UL, PT.EArg(918UL, 0))
          )
        ...
        |> CheckerApi.checkPackageFunction environment
        |> expectDiagnostic Checker.InvalidUnwrap
        let aliasName = PT.FQTypeName.package "unwrap-result-alias"
        let aliases =
          environment
          |> Checker.TypeEnvironment.addType
            aliasName
        ...
        |> CheckerApi.checkPackageFunction environment
        |> expectChecked
        // The extracted value is typed at the `?`, so arithmetic on it in the
        ...
        oneArgFn
          ...
          (PT.ELet(
            ...
            PT.EUnwrap(925UL, PT.EArg(926UL, 0)),
            PT.EEnum(
              927UL,
              nr optionName,
              [],
        ...
        |> CheckerApi.checkPackageFunction environment
        |> expectChecked
        // An error-type mismatch names `?`, with the declared error as expected.
        match
          ...
          Expect.isTrue
            (report.diagnostics
             |> List.exists (fun d ->
               ...
               && d.context = Checker.At Checker.UnwrappedError
               && d.expected = Some Checker.TString
               && d.actual = Some Checker.TBool))
            $"expected an UnwrappedError mismatch, got {report.diagnostics}"
        | other -> failtestf "Expected Failed, got %A" other
        // A generalized helper `fun row -> Some row.item?` keeps its field
        // constraint tied to each use: `?` solves the field's type to its
        // container before generalization, and the captured constraint must see it.
        ...
        let intRow, intRowDeclaration = record "unwrap-int-row" PT.TInt
        let stringRow, stringRowDeclaration = record "unwrap-string-row" PT.TString
        let rows =
          environment
        ...
        oneArgFn
          (customType (PT.FQTypeName.package "unwrap-missing-alias"))
          (option PT.TInt)
          (PT.EEnum(
            953UL,
            nr optionName,
            [],
            "Some",
            [ PT.EUnwrap(954UL, PT.EArg(955UL, 0)) ]
          ))
        |> CheckerApi.checkPackageFunction environment
        ...
        oneArgFn
          PT.TInt
          (customType (PT.FQTypeName.package "unwrap-missing-alias"))
          (PT.EUnwrap(956UL, PT.EArg(957UL, 0)))
        |> CheckerApi.checkPackageFunction environment
        |> expectDiagnostic Checker.InvalidUnwrap
        // A bare `fun row -> row.item?`: the record is tied to the `?` only
        // through the field access, and must stay tied to it for `get input`.
        let nestedRow, nestedRowDeclaration =
          record "unwrap-nested-row" (option PT.TInt)
        oneArgFn
