...
/// These lock the two properties concurrent agents depend on:
...
///     neither can resolve OR fetch the other's. That is what lets N agents run N branches
///     concurrently (separate processes over a shared read-only core) without cross-talk.
/// Reuses the existing composable-PM infra only -- no new substrate. If this ever breaks, the
/// branch model's foundation is broken.
...
/// Parse a branch source into stabilized ops (the real authoring path: parse, then
/// compute SCC-aware content hashes).
...
let isolationFromCore =
  testTask "a branch fn resolves + executes on its overlay but is INVISIBLE to core" {
    ...
    // ...but NOT on the shared core (it never leaks into main).
    let! onCore = pmPT.findFn fooLoc |> Ply.toTask
    Expect.isNone onCore "foo does NOT leak into the shared core"

    // and it EXECUTES (through overlay-over-core: the body calls a core fn).
...
let isolationBetweenBranches =
  testTask
    ...
    // A cannot fetch B's def and vice-versa: an overlay holds only its own ops (falling back
    // to core, which has neither). This is what keeps N concurrent branches from cross-talking.
    ...
    // each executes to ITS OWN answer, concurrently over the same core.
...
/// Wipe every trace of a test branch: its ops, its frontier tags, its per-name bases, its registry row.
/// Called defensively before AND after, since the store is shared.
///
/// The ONE place in this file allowed to spell these tables out. Per-test copies of the same DELETE are
/// as many chances to miss a table when the schema moves; if a branch grows another table, it is added
/// here and every test picks it up.
...
/// Pretend the parent moved every name this branch touched, by staling the recorded bases.
///
/// The conflict tests need a diverged name, and doing it this way rather than by actually changing the
/// parent keeps them off the shared main projection, which every other test in this file is reading
/// concurrently. Named once because two tests want it and both had their own copy.
...
let storeThenOverlay =
  testTask
    ...
    // load the branch's ops back from the store + overlay -> foo resolves.
...
let markMergedFlipsEffective =
  testTask
    ...
    // stored branch-pending (effective=0)
...
/// A merged branch stays addressable by name; a switch under that name starts a new one.
///
/// This is a UX contract, not an implementation detail: `dark branches` lists merged branches, so every
/// verb that takes a branch name has to accept the names it just printed. It regressed once already, in
/// the form of `dark diff feat` answering "no branch \"feat\"" and helpfully suggesting `dark branches`,
/// which is where the name came from.
/// Concurrent `resolveOrCreate` for one name yields ONE branch.
///
/// This is the case `DARK_BRANCH` makes ordinary: several agents start in their own shells with the same
/// branch name exported, and they all reach for it at once. If each minted its own, they would author into
/// different branches while believing they shared one, and a later lookup would silently pick whichever won
/// the ordering. Verified by hand with 8 concurrent CLI processes; this is the version that keeps working.
let concurrentCreateYieldsOneBranch =
  testTask "racing to create one branch name produces one branch" {
    ...
    // Same name, at once, from several callers.
    let! results =
...
let processOverlaySelects =
  testTask
    ...
    // Asks `ptForBranch` for the branch the process is on, rather than an ambient "whatever overlay is
    // loaded" accessor. Those were two ways to ask the same question, and only one of them can express
    // "answer for a branch I'm not sitting on", which is what a daemon or the LSP needs.
...
let branchesOffBranches =
  testTask "a branch off another sees its parent's frontier (branches off branches)" {
    ...
    // A's overlay is only A's own: a parent does NOT see its child's ops.
...
/// Regression guard for the branch->main isolation leak (commit 1c10605abf): main authoring's
/// WipRefresh must NOT see a branch's ops. getWipOps excludes op_branches-tagged ops, so its count
/// is exactly (all package_ops - this branch's ops). If a future change lets branch ops back into
/// WIP, discardWipOps + re-insert would fold them into main -- the exact leak this locks out.
let getWipOpsExcludesBranch =
  testTask
    ...
    // subtracting only our own count assumes we're the only branch in the store. We aren't, and asserting
    // that we are makes this fail for whatever unrelated test happened to leave a branch behind.
...
/// Regression guard for the applied-flag isolation bug (Seed.applyUnappliedOps' final sweep). When
/// branch M merges, the fold marked EVERY applied=0 op applied=1 -- including sibling branch S's
/// still-pending (effective=0) ops. S's ops then couldn't fold when S itself merged
/// (markMergedEffective flips them effective=1, but applyUnappliedOps only folds applied=0), so S's
/// later-authored binding silently lost -- convergence broke by merge order. The fix scopes the sweep
/// to `applied=0 AND effective=1`. Here: store S's ops, merge M (flip effective=1 + fold), assert S's
/// ops are STILL applied=0 (untouched by M's merge).
let mergeDoesNotConsumeSiblingPendingOps =
  testTask
    ...
    // Cleanup: drop both branches' ops + the location M folded into main.
    do! cleanupBranch bS
    do! cleanupBranch bM
    do!
...
/// End-to-end convergence guard (the PURPOSE of the applied-sweep fix, not just its mechanism): two
/// branches bind the SAME name to DIFFERENT hashes, the second authored LATER. Merge the OLDER one
/// first, then the NEWER -- main must end on the NEWER binding (origin_ts LWW). Before the fix, the
/// older merge's sweep marked the newer branch's ops applied=1, so the newer merge folded nothing and
/// main wrongly kept the OLDER binding -- convergence depended on merge order. Folds through the real
/// path (markMergedEffective + applyUnappliedOps); cleans up the folded location after.
let sameNameMergesConvergeToLater =
  testTask
    ...
    // Merge the OLDER first, then the NEWER (the order that regressed).
    do! mergeFold bOld
    do! mergeFold bNew
...
/// imported branch resolves its fn (isolated) and gets fresh local bases -- the cross-instance
/// invariant that a branch stays a branch on the receiving side.
let branchTransferImportReDerivesBases =
  testTask
    ...
    // the imported branch resolves its fn via its overlay, isolated from core.
...
/// A name a branch binds ONLY by resolving a conflict still needs a per-name base.
///
/// `recordNameBases` matching `SetName` alone leaves a name bound solely by a `Resolve` without a
/// `branch_name_bases` row. The base is what the conflict detector needs to prove BOTH sides moved, so that
/// name can never conflict again, and `dark diff` renders it as `+ new` rather than a change.
/// `bindingFromOp` counts `Resolve` as a binding; this locks the two together, since disagreeing is the bug.
let resolveAloneRecordsANameBase =
  testTask "a name bound only by Resolve still gets a per-name base" {
    ...
    // Borrow a real (location, target) off a SetName rather than hand-building a hash: the point of the
    ...
    // A Resolve and NOTHING else. Before the fix this recorded zero bases.
    do!
...
/// Locks per-name RESOLUTION (scm-spec 7). Sets up a conflict by staling the recorded base (same trick as
/// the rebase test -- no fold into shared main), then: take-theirs untags the branch's SetName so its
/// overlay no longer binds the name (falls back to the parent) and clears the conflict; keep-mine leaves
/// the branch binding the name (re-stamped) and clears the conflict. Isolation-safe.
  ...
  testTask
    ...
    let setupConflict (b : string) =
      task {
        ...
        // simulate the parent having moved the name since the fork: stale the recorded base.
        do! staleNameBases b
      }

    // TAKE-THEIRS: branch's binding dropped, conflict cleared.
    do! setupConflict bT
    ...
    // KEEP-MINE: branch keeps binding the name, conflict cleared.
    do! setupConflict bM
...
/// archived_at + merged_at so a review queue reused after reject/approve is active + visible again, while
/// parent stays first-write-wins. Isolation-safe.
let reuseBranchIdRevives =
  testTask
    ...
    // archive + mark merged, then re-create (reuse) and expect both flags cleared.
    // Set the flag directly: this test is about createBranch's revive-on-reuse, and `archive` is Dark now
    // (SCM.Branches). Driving the setup through SQL keeps the test on its actual subject.
    do!
      Sql.query
...
/// Guards BOTH halves of the expression-value fix (431f903b8f). ISOLATION: the branch author path folds a
/// value's AddValue CONTENT into package_values (so eval can read its rt_dval), but must NOT fold the SetName,
/// so the NAME never lands in main's `locations`. CORRECTNESS: after the content fold, evaluateAllValues must
/// materialise an EXPRESSION body's Dval into rt_dval (else it stays NULL and getValue returns nothing -- the
/// original bug). Here: parse a module-level `val` with a fn-call body, fold ONLY its AddValue, assert the
/// content row exists but no locations row does, then evaluate and assert rt_dval = 7. Cleans up after.
let branchValueContentFoldIsolatesName =
  testTask
    ...
    // Bug #1 (431f903b8f) CORRECTNESS half: applyOps stores rt_dval NULL (see PackageOpPlayback.fs);
    // the branch author path MUST then run evaluateAllValues so an EXPRESSION-valued branch value
    // materialises its Dval. Without that step rt_dval stays NULL and getValue reads nothing -- the
    // original bug (expression-valued branch values errored / returned nothing). Assert 3+4 -> 7.
...
let mergeCountsWhatItFlipped =
  testTask
    ...
    // Merging again flips NOTHING -- every tagged op is already effective. Reporting the tag count here
    // is how `MergeOutcome.merged` claimed work it hadn't done, which is the one thing every command on
    // this branch is supposed to have stopped doing.
...
let rebuildKeepsBranchPolicy =
  testTask "a projection rebuild re-folds branch-scoped propagation decisions" {
    ...
    // What a projection rebuild does to this table: clear it. It then re-folds only `effective = 1`
    // ops, and branch ops are `effective = 0` by design -- so without an explicit re-fold the row is
    // deleted and never comes back, silently, on a flag documented as safe recovery.
...
let branchPMIsPerBranch =
  testTask "ptForBranch answers about a branch this process is NOT on" {
    ...
    // Asking about the branch we're on works, and so does asking about one we're not. The second is
    // what a process-global overlay alone cannot do, and it's what the LSP and any daemon will need.
    ...
    // Isolation still holds in both directions: neither branch's overlay contains the other's.
    ...
    // Switching is a process operation, not a restart -- this is what `dark switch` needs in the REPL.
...
let branchNamesResolveButDontShadowMain =
  testTask
    ...
    // Main cannot name this hash: a branch's SetNames never fold into `locations`. Without the overlay
    // the caller has nothing to render but 64 hex characters.
    ...
    // ... but main WINS when it has an answer. Identical content is one item, so a hash is routinely live
    // at several names; preferring the branch's would render a main item under a branch label.
    ...
    // Kind is part of the question: a fn hash is not a type location.
    let asType =
...
/// The three ways a run picks its branch, and their precedence.
///
/// `github/main` had all three -- flag, `DARK_BRANCH`, and a stored default -- and each is scoped tighter
/// than the one below on purpose: the FLAG is this command, the ENV is this SHELL, the config is this
/// machine. The env tier is the one that lets several agents work on several branches at once without
/// fighting over a single config key; `dark switch` writes that key, so without it the only way to run
/// concurrent branches is to pass the flag on every single command.
///
/// It was dropped in the carve and restored here. This asserts the resolution order rather than the CLI
/// plumbing, which is what actually has to hold.
...
/// A merge that does not travel is the gap this closes: the merged OPS already crossed (they are main ops
/// once merged, and the two mains converge on identical hashes), but the FACT of the merge did not, so a
/// colleague's copy of the branch still listed as live work they could keep authoring on.
...
/// The receiving side has no obligation to know every branch its peers have. Branch ids travel with a
/// bundle, so the branches you actually share match; the rest are none of this store's business.
let branchEventForUnknownBranchIsIgnored =
  testTask "a BranchEvent for a branch this store has never seen folds to nothing" {
    ...
    // The assertion is that this does not throw and does not invent a row.
    let! _ = LibDB.Inserts.insertAndApplyOps [ op ]
...
///
/// A merge event arriving from another machine flips its branch's frontier to effective=1 mid-fold. If the
/// applied=1 sweep runs afterwards, it marks those ops applied without anything having folded them, and
/// they are never looked at again -- so the branch reads `[merged]` next to a main that does not have its
/// code. The sweep runs BEFORE the fold for exactly this reason.
///
/// The shape is the same one the sweep's own comment already warned about, arriving by the other door.
let foldDoesNotStrandOpsItMadeEffective =
  testTask
    ...
    // The event has to arrive the way a SYNC delivers it: inserted unapplied-and-effective, then folded
    // by `applyUnappliedOps`. That is what puts the flip and the applied=1 sweep inside ONE pass, which
    // is the whole hazard. Authoring it locally instead folds it in its own call, and the branch ops are
    // then picked up by a later pass no matter which order the sweep runs in -- a test that passes
    ...
    // Nothing may be left claiming to have been applied without having been folded. Asserted over the
    // ids captured BEFORE the event, because the event clears the branch tags: a query that looks them
    // up by tag afterwards finds nothing and passes for the wrong reason.
    ...
    // And the point of all of it: the code is actually in main now.
...
/// Dark code outside the SCM silos must not query `locations`.
///
/// `locations` is MAIN's projection and has no `branch_id`. A read that goes straight to it answers about
/// main while the caller is standing on a branch, and answers plausibly, so nothing downstream can tell.
/// The overlay helpers in `SCM.PackageOps` are the branch-aware way to ask.
///
/// Checked by reading the source, because the failure is invisible at run time on a single-branch store.
/// `matter.dark` is exempt: a relay holds no branches of its own, so main's projection IS its answer.
...
let tests =
  // These mutate the process-global branch overlay AND delete from `package_ops`, either of which can
  // make a concurrent reader see the store mid-change.
