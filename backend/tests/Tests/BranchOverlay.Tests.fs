...
///
/// `SCM.Ids` exists to enforce "compare against `mainBranchId`, never a literal", and the type system
/// carries that for Dark code: an id is a `Uuid` and a name is a `String`. A SQL string is opaque to
/// both, which is why the rule drifts THERE and only there. `Ids`' own comment records it drifting twice
/// already, once when main's id was "" and once when it was "main"; a third had survived in two recursive
/// parent walks in `packageOps.dark`, where `WHERE b.parent_id != 'main'` was simply dead against a store
/// that holds main's UUID in that column.
///
