...
/// authoring path makes: single-hop, transitive, multi-dependent, the pin that stops
/// it, crossing owners, the no-op, mutual recursion, landing on the final version
/// after several edits, and the case where content is shared between two names (only
/// the edited name's dependents move).
