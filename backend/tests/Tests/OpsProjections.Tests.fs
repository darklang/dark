...
/// re-folded because the log is addressed by content rather than by insertion order. None of that was
/// tested: the determinism tests prove the fold is a function of the log, not that the log is addressed by
/// what it contains.
///
...
/// `Inserts.computeOpHash`. That distinction is the whole test. The test store is deleted and re-folded
/// before every run, so every id in it was minted by `Inserts.computeOpHash` moments earlier: a test that
/// recomputes with that same function compares it against itself and passes no matter what it does. The
/// self-comparing version was tried first: breaking the truncation deliberately left it passing happily.
///
/// What this pins: that the id is the first sixteen bytes of the op's content hash. What it still cannot
/// catch is a change to `Hashing.computeOpHash` itself, since the store would be rebuilt under the new
/// definition. A golden-value test over a fixed op would cover that, and does not exist.
/// A fixed op hashes to a fixed value.
///
/// The companion to the test below, and it covers the one thing that one cannot. That test derives the
/// expected id from `Hashing.computeOpHash`, so it proves the store agrees with the hash function; if the
/// hash function itself changed, the store would simply be rebuilt under the new definition and both would
/// agree on a different answer.
///
/// This pins the function to a literal. It is meant to fail when someone changes how ops are hashed, and
/// that failure is the point rather than a nuisance: the id is an op's identity, so changing it means every
/// existing store's ids stop matching their content, every peer disagrees about which ops it already has,
/// and `INSERT OR IGNORE` stops deduping. If you meant it, update the literal and plan a migration. If you
/// did not, you just found out early.
///
/// A `SetName` rather than an `AddFn`, deliberately: it carries no expression tree, so this pins the op
/// hashing and not the whole AST serializer.
///
/// The literal moved once, when `SetName` gained the hash it replaced. That was a deliberate format change
/// with a clean break of every store, which is exactly the event this test exists to make somebody decide
