...
--
-- BRANCH-SCOPED, like everything else a branch can change. `branch_id = ''` is main. A branch
...
--
-- A pure cache, sound because an item's hash IS its content: "this hash type-checks" cannot stop
-- being true while the CHECKER stays the same, and editing an item produces a different hash.
-- `build_hash` keeps the cache from outliving the checker that filled it: widening a rule later
-- would otherwise silently never fire on anything already marked clean.
--
-- The check is whole-store and `dark status` runs it on every invocation, so the first run after a
-- build pays and the rest look only at what is new. A projection: the next check refills it.
