...
let check_all_canvases (span : Span.t) : (unit, Exception.captured) Result.t =
  ...
  try
    let endpoints_and_canvases =
      Telemetry.with_span span "check_all_canvases.filter_map" (fun span ->
          List.filter_map current_endpoints ~f:(fun endp ->
              ...
              with e ->
                ...
                None))
    in
    Telemetry.with_span span "check_all_canvases.iter" (fun span ->
