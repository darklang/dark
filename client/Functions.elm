...
ufmToF ufm =
    let
        ps =
            List.filterMap ufpToP ufm.parameters

        sameLength =
            List.length ps == List.length ufm.parameters
    in
...
replaceParamName search replacement uf =
    ...
    in
    if List.any (\p -> B.toID p == sId) paramNames then
        let
            newMetadata =
                case replacement of
                    PParamName new ->
            ...
            newBody =
                ...
                case ( sContent, rContent ) of
                    ( Just o, Just r ) ->
                        let
                            uses =
                                AST.uses o uf.ast |> List.map PExpr
                        in
                        List.foldr
...
replaceParamTipe search replacement uf =
    ...
    in
    if List.any (\p -> B.toID p == sId) paramTipes then
        let
            newMetadata =
                case replacement of
                    PParamTipe new ->
