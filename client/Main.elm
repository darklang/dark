...
init { editorState, complete } location =
    let
        savedEditor =
            Editor.fromString editorState

        m0 =
            Editor.editor2model savedEditor
    ...
    if shouldRunIntegrationTest then
        m2
            ! [ RPC.integrationRPC m integrationTestName
              , visibilityTask
              ]
    ...
    case focus of
        FocusNext tlid pred ->
            let
                ...
                predPd =
                    Maybe.andThen (TL.find tl) pred

                next =
                    TL.getNextBlank tl predPd
            in
            case next of
                Just pd ->
    ...
    let
        ...
        closeBlanks newM =
            ...
            m.cursorState
                ...
                |> Maybe.withDefault []
                |> (\rpc ->
                        if tlidOf newM.cursorState == tlidOf m.cursorState then
                            []
        ...
        ( newm, newcmd ) =
            ...
            case mod of
                ...
                Select tlid p ->
                    let
                        newM =
                            { m | cursorState = Selecting tlid p }
                    in
                    newM ! closeBlanks newM
                ...
                SelectCommand tlid id ->
                    let
                        m2 =
                            { m | cursorState = SelectingCommand tlid id }

                        ( m3, acCmd ) =
                            processAutocompleteMods m2
                                [ ACEnableCommandMode
                                , ACRegenerate
                                ]
                    in
                ...
                UpdateAnalysis tlars ->
                    let
                        m2 =
                            { m | analysis = Analysis.replace m.analysis tlars }
                    in
                    processAutocompleteMods m2 [ ACRegenerate ]
                ...
                ExecutingFunctionRPC tlid id ->
                    let
                        params =
                            { function = ( tlid, id, Analysis.cursor m tlid ) }
                    in
                ...
                ExecutingFunctionComplete targets ->
                    let
                        isComplete target =
                            not <| List.member target targets
...
update_ msg m =
    ...
    case msg of
                    ...
                    Selecting tlid mId ->
                        ...
                        case event.keyCode of
                                                ...
                                                Just id ->
                                                    case TL.findExn tl id of
                                                        PExpr _ ->
                                                            let
                                                                replacement =
                                                                    AST.addThreadBlank id h.ast
                                                            in
                                                            RPC
                                                                ( [ SetHandler tl.id tl.pos { h | ast = replacement } ]
                            ...
                            Key.Up ->
                                ...
                                if event.shiftKey then
                                    Selection.selectUpLevel m tlid mId

                                else
                                    Selection.moveUp m tlid mId
                            ...
                            Key.Down ->
                                ...
                                if event.shiftKey then
                                    ...
                                    Selection.moveDown m tlid mId

                            Key.Right ->
                                if event.altKey then
                                    Selection.moveCursorBackInTime m tlid

                                else
                                    Selection.moveRight m tlid mId

                            Key.Left ->
                                if event.altKey then
                                    Selection.moveCursorForwardInTime m tlid

                                else
                                    Selection.moveLeft m tlid mId

                            Key.Tab ->
                                -- NB: see `stopKeys` in ui.html
                                if event.shiftKey then
                                    Selection.selectPrevBlank m tlid mId

                                else
                                    Selection.selectNextBlank m tlid mId
                            ...
                            Key.N ->
                                if event.ctrlKey then
                                    Selection.selectNextSibling m tlid mId
                        ...
                        if event.ctrlKey then
                            case event.keyCode of
                                ...
                                Key.Enter ->
                                    ...
                                    else if AC.isSmallStringEntry m.complete then
                                        Many
                        ...
                        case m.currentPage of
                            ...
                            Toplevels center ->
                                case event.keyCode of
                                    ...
                                    Key.Up ->
                                        Viewport.moveUp center

                                    -- NB: see `stopKeys` in ui.html
                                    Key.Down ->
                                        Viewport.moveDown center

                                    -- NB: see `stopKeys` in ui.html
                                    Key.Left ->
                                        Viewport.moveLeft center

                                    Key.Right ->
                                        Viewport.moveRight center

                                    Key.Zero ->
                                        Viewport.moveTo { x = 0, y = 0 }

                                    Key.Tab ->
                                        Selection.selectNextToplevel m Nothing
        ...
        -- just keep this here to prevent the page from loading
        ------------------------
        -- mouse
        ------------------------
        ...
        MouseWheel deltaCoords ->
            let
                pos =
                    Viewport.pagePos m.currentPage
        ...
        DragToplevel _ mousePos ->
            case m.cursorState of
                Dragging draggingTLID startVPos _ origCursorState ->
                    let
                        xDiff =
                            mousePos.x - startVPos.vx

                        yDiff =
                            mousePos.y - startVPos.vy

                        m2 =
                            TL.move draggingTLID xDiff yDiff m
                    in
                    Many
        ...
        ToplevelMouseDown targetTLID event ->
            let
                _ =
                    Debug.log "tlmousedown" event
            in
            if event.button == Defaults.leftButton then
                let
                    tl =
                        TL.getTL m targetTLID
                in
                case tl.data of
                    TLFunc _ ->
                        NoChange
                    ...
                    _ ->
                        Drag targetTLID event.pos False m.cursorState
        ...
        BlankOrClick targetTLID targetID _ ->
            let
                _ =
                    Debug.log "blankorclick " ( targetTLID, targetID )
            in
            case m.cursorState of
                Deselected ->
        ...
        BlankOrDoubleClick targetTLID targetID _ ->
            let
                _ =
                    Debug.log "blankorDclick " ( targetTLID, targetID )
            in
        ...
        DeleteUserFunctionParameter uf upf ->
            let
                replacement =
                    Functions.removeParameter uf upf

                newCalls =
                    Refactor.removeFunctionParameter m uf upf
            in
        ...
        -- RPCs stuff
        -----------------
        RPCCallback focus calls (Ok ( newToplevels, newDeletedToplevels, newAnalysis, globals, userFuncs, unlockedDBs )) ->
            if focus == FocusNoChange then
                ...
                let
                    m2 =
                        TL.upsertAll m newToplevels

                    m3 =
                        { m2 | userFunctions = userFuncs }

                    newState =
                        processFocus m3 focus
                in
                Many
        ...
        InitialLoadRPCCallback focus extraMod (Ok ( toplevels, deletedToplevels, new_analysis, globals, userFuncs, unlockedDBs )) ->
            let
                m2 =
                    { m | toplevels = toplevels, userFunctions = userFuncs }

                newState =
                    processFocus m2 focus
            in
            Many
        ...
        ReceiveAnalysis json ->
            let
                analysis =
                    JSD.decodeValue (JSD.list RPC.decodeTLAResult) json
            in
            case analysis of
                Ok analysis ->
                    UpdateAnalysis analysis
