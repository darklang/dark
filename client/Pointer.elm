...
toContent pd =
    let
        bs2s s =
            s |> B.toMaybe |> Maybe.withDefault "" |> Just
    in
    case pd of
        PVarBind v ->
            bs2s v
...
strmap fn pd =
    case pd of
        ...
        PEventModifier d ->
            PEventModifier (fn EventModifier d)

        PEventName d ->
            PEventName (fn EventName d)

        PEventSpace d ->
            PEventSpace (fn EventSpace d)

        PDBColName d ->
            PDBColName (fn DBColName d)

        PDBColType d ->
            PDBColType (fn DBColType d)

        PDarkType _ ->
            pd

        PDarkTypeField d ->
            PDarkTypeField (fn DarkTypeField d)
