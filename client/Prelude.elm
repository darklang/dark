...
tlCursorID tlid idx =
    let
        stringID =
            toString (deTLID tlid) ++ toString idx

        intID =
            Result.withDefault 0 (String.toInt stringID)
    in
    ID intID
...
deMaybe msg x =
    case x of
        ...
        Nothing ->
            impossible ( "got Nothing but expected something", msg )
...
assert fn a =
    if fn a then
        ...
        impossible ( "assertion failure", a )
