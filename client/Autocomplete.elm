module Autocomplete exposing
    ...
    , asString
    , asTypeString
    , compareSuggestionWithActual
    , containsOrdered
    , documentationForItem
    , empty
    , enableCommandMode
    , filter
    , findCompatibleThreadParam
    , findFunction
    , findParamByType
    , focusItem
    , generateFromModel
        ...
        _ ->
            ...
            case String.indexes (String.toLower actual) (String.toLower suggestion) of
                ...
                index :: _ ->
                    let
                        prefix =
                            String.slice 0 index suggestion

                        suffix =
                            String.slice (index + String.length actual) (String.length suggestion) suggestion
                    in
...
appendQuery str a =
    let
        q =
            if isStringEntry a then
                String.dropRight 1 a.value ++ str ++ "\""
...
highlighted a =
    LE.getAt a.index (List.concat a.completions)
...
documentationForItem aci =
    case aci of
        ACFunction f ->
            if String.length f.description /= 0 then
                Just f.description
...
isStaticItem item =
    not (isDynamicItem item)


qLiteral : String -> Maybe AutocompleteItem
qLiteral s =
    if RT.isLiteral s then
        Just (ACLiteral s)
    ...
    if
        String.length s > 0
    then
        if String.startsWith (String.toLower s) "null" then
            Just (ACLiteral "null")
        ...
        else if String.startsWith (String.toLower s) "nothing" then
...
qHTTPHandler s =
    if String.length s == 0 then
        Just (ACOmniAction NewHTTPHandler)
...
qHTTPRoute s =
    if String.startsWith "/" s then
        Just (ACOmniAction (NewHTTPRoute s))
...
qEventSpace s =
    if Util.reExactly "[A-Z]+" s then
        Just (ACOmniAction (NewEventSpace s))
...
toDynamicItems isOmni query =
    let
        ...
        omni =
            if isOmni then
                [ qNewDB, qHTTPHandler, qHTTPRoute, qEventSpace ]
...
withDynamicItems target query acis =
    let
        new =
            toDynamicItems (target == Nothing) query

        withoutDynamic =
            List.filter isStaticItem acis
    in
    new ++ withoutDynamic
...
refilter query old =
    ...
    let
        ...
        newCompletions =
            filter fudgedCompletions query

        newCount =
            newCompletions |> List.concat |> List.length
...
filter list query =
    let
        ...
        ( dynamic, candidates0 ) =
            List.partition isDynamicItem list
...
generateFromModel m a =
    let
        ...
        varnames =
            Analysis.varnamesFor m a.target
...
findCompatibleThreadParam { parameters } tipe =
    parameters
        ...
        |> Maybe.andThen
            (\fst ->
                if RT.isCompatible fst.tipe tipe then
                    Just fst
