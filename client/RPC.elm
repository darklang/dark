module RPC exposing
    ...
    , encodeCursorState
    , encodeDBMigrationKind
    , encodeDarkType
    , encodeExecuteFunctionRPCParams
    , encodeExpr
    , encodeHandler
    , encodeNDarkType
    , encodeNExpr
    ...
    , encodePointerData
    , encodeRPCParams
    , encodeSerializableEditor
    , encodeSpec
    , encodeSpecTypes
    , encodeTipe
    , encodeUserFunction
    , encodeUserFunctionMetadata
    , encodeUserFunctionParameter
    , executeFunctionRPC
    , getAnalysisRPC
    , initialLoadRPC
    , integrationRPC
...
executeFunctionRPC params =
    let
        ...
        request =
            Http.post url json decodeExecuteFunctionRPC
    in
    Http.send ExecuteFunctionRPCCallback request
...
getAnalysisRPC params =
    let
        ...
        request =
            Http.post url json decodeGetAnalysisRPC
    in
...
initialLoadRPC focus =
    let
        ...
        request =
            Http.post url Http.emptyBody decodeInitialLoadRPC
    in
...
encodeOp call =
    ...
    case call of
        ...
        UndoTL tlid ->
            ev "UndoTL" [ encodeTLID tlid ]

        RedoTL tlid ->
            ev "RedoTL" [ encodeTLID tlid ]

        DeleteTL tlid ->
            ev "DeleteTL" [ encodeTLID tlid ]

        MoveTL tlid pos ->
            ev "MoveTL" [ encodeTLID tlid, encodePos pos ]

        SetFunction uf ->
            ev "SetFunction" [ encodeUserFunction uf ]

        DeleteFunction tlid ->
            ev "DeleteFunction" [ encodeTLID tlid ]
...
encodeTipe t =
    ...
    case t of
        ...
        TBelongsTo s ->
            ev "TBelongsTo" [ JSE.string s ]

        THasMany s ->
            ev "THasMany" [ JSE.string s ]

        TDbList a ->
            ev "TDbList" [ encodeTipe a ]
...
encodeNExpr expr =
    ...
    case expr of
        ...
        Lambda vars body ->
            ev "Lambda"
                [ List.map (encodeBlankOr JSE.string) vars |> JSE.list
                , e body
                ]
        ...
        Variable v ->
            ev "Variable" [ JSE.string v ]

        Value v ->
            ev "Value" [ JSE.string v ]

        Thread exprs ->
            ev "Thread" [ JSE.list (List.map e exprs) ]

        ObjectLiteral pairs ->
            let
                encoder =
                    JSON.encodePair (encodeBlankOr JSE.string) e
            in
...
decodeLiveValue =
    let
        toLiveValue value tipe json exc =
            { value = value
...
decodeAResult =
    let
        toAResult astValue liveValues availableVarnames inputValues =
            { astValue = astValue
...
decodeHandlerSpec =
    let
        toHS module_ name modifier input output =
            { module_ = module_
            , name = name
            , modifier = modifier
            , types =
...
decodeHandler =
    let
        toHandler ast spec tlid =
            { ast = ast, spec = spec, tlid = tlid }
    in
...
decodeDBMigration =
    let
        toDBM v kind rollf rollb target =
            { startingVersion = v
            , kind = kind
            , rollforward = rollf
            , rollback = rollb
...
decodeDB =
    let
        toDB name cols version old active =
            { name = name
            , cols = cols
            , version = version
            , oldMigrations = old
...
decodeUserFunctionParameter =
    let
        toParam name tipe args option desc =
            { name = name
            , tipe = tipe
            , block_args = args
            , optional = option
...
decodeUserFunctionMetadata =
    let
        toFn name params desc returnTipe infix =
            { name = name
            , parameters = params
            , description = desc
