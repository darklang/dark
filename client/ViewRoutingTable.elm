...
ordering a b =
    case ( a, b ) of
        ...
        -- to the end
        ( a, b ) ->
            if a == missingEventRouteDesc then
                GT

            else if b == missingEventRouteDesc then
                LT
...
viewGroup showLink showUndo ( spacename, entries ) =
    let
        def s =
            Maybe.withDefault missingEventRouteDesc s
            ...
            div "handler"
                ([ div "name"
                 ...
                 , div "extra"
                    ...
                    ++ (if showUndo == ShowUndo then
                            [ undoButton e.tlid (Toplevels pos) ]
        ...
        routes =
            div "routes" (List.map entryHtml entries)
    ...
    tls
        ...
        |> (\entries ->
                if collapse == CollapseVerbs then
                    List.map (T2.map collapseByVerb) entries
...
viewDeletedTLs tls =
    let
        routes =
            viewRoutes tls DontCollapseVerbs DontShowLink ShowUndo

        dbs =
            viewRestorableDBs tls

        h =
            header "Deleted" tls Nothing
    in
    Html.details
...
span class subs =
    Html.span [ Attrs.class class ] subs


text : String -> String -> Html.Html Msg
text class msg =
    span class [ Html.text msg ]
    ...
    let
        ...
        event =
            case page of
                ...
                Just _ ->
                    eventNoPropagation "click" (\_ -> handler)
    in
    Html.a
        ([ event
...
view404s f404s =
    let
        ...
        routes =
            div "404s" (List.map fofHtml f404s)
    in
...
viewRoutingTable m =
    let
        sections =
            viewRoutes m.toplevels CollapseVerbs ShowLink DontShowUndo
                ++ [ viewDBs m.toplevels ]
                ++ [ view404s m.f404s ]
                ++ [ viewUserFunctions m ]
        ...
        html =
            Html.div
                ...
                sections
    in
    placeHtml m (Viewport.toAbsolute m { vx = 0, vy = 0 }) html
