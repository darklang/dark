...
onlyTL m =
    let
        len =
            List.length m.toplevels

        _ =
            if len == 0 then
                Debug.crash "no toplevels"

            else if len > 1 then
                Debug.crash ("too many toplevels: " ++ toString m.toplevels)
...
field_access_closes m =
    case m.cursorState of
        Entering (Filling _ id) ->
            let
                ast =
                    onlyTL m |> TL.asHandler |> deMaybe "test" |> .ast
            in
            if (AST.allData ast |> List.filter P.isBlank) == [] then
                pass

            else
                fail (TL.allBlanks (onlyTL m))
...
field_access_pipes m =
    case onlyExpr m of
        Thread [ F _ (FieldAccess (F _ (Variable "request")) (F _ "body")), Blank _ ] ->
            pass
...
field_access_nested m =
    case onlyExpr m of
        FieldAccess (F _ (FieldAccess (F _ (FieldAccess (F _ (Variable "request")) (F _ "body"))) (F _ "field"))) (F _ "field2") ->
            pass
...
pipe_within_let m =
    case onlyExpr m of
        Let (F _ "value") (F _ (Value "3")) (F _ (Thread [ F _ (Variable "value"), F _ (FnCall "assoc" [ Blank _, Blank _ ] _) ])) ->
            pass
...
left_right_works m =
    ...
    case m.cursorState of
        Selecting tlid (Just id) ->
            let
                pd =
                    TL.getTL m tlid |> (\tl -> TL.find tl id)
            in
            case pd of
                Just (PEventSpace _) ->
                    pass
...
editing_does_not_deselect m =
    case m.cursorState of
        Entering (Filling tlid id) ->
            ...
            in
            case pd of
                Just (PExpr (F _ (Value "\"hello zane\""))) ->
                    pass
...
case_sensitivity m =
    if List.length m.toplevels /= 3 then
        fail m.toplevels
            ...
            |> List.map
                (\tl ->
                    case tl.data of
                        TLDB { name, cols } ->
                            case ( name, cols ) of
                                ( "TestUnicode", [ ( F _ "cOlUmNnAmE", F _ "Str" ), ( Blank _, Blank _ ) ] ) ->
                                    pass
                        ...
                        TLHandler h ->
                            case h.ast of
                                F _ (Thread [ F _ (Value "{}"), F _ (FnCall "assoc" [ F _ (Value "\"cOlUmNnAmE\""), F _ (Value "\"some value\"") ] _), F _ (FnCall "DB::insert" [ F _ (Variable "TestUnicode") ] _) ]) ->
                                    pass

                                F id (Thread [ F _ (Variable "TestUnicode"), F _ (FnCall "DB::fetchAll" [] _), F _ (FnCall "List::head" [] _), F _ (Lambda [ F _ "var" ] (F _ (FieldAccess (F _ (Variable "var")) (F _ "cOlUmNnAmE")))) ]) ->
                                    Analysis.getLiveValue m tl.id id
                                        |> Maybe.map
                                            (\lv ->
                                                if lv.value == "\"some value\"" then
                                                    pass
...
focus_on_ast_in_new_empty_tl m =
    case (onlyHandler m).ast of
        Blank id ->
            if idOf m.cursorState == Just id then
                pass

            else
                fail ( id, m.cursorState )
...
focus_on_cond_in_new_tl_with_if m =
    case onlyExpr m of
        If cond _ _ ->
            if idOf m.cursorState == Just (B.toID cond) then
                pass
    ...
    case m.cursorState of
        Selecting _ mId ->
            ...
            then
                ...
                fail ( m.toplevels, m.cursorState )
        ...
        |> List.map
            (\tl ->
                case tl.data of
                    TLDB { name, cols } ->
                        case cols of
                            [ ( F id "field6", F _ "String" ), ( F _ "field2", F _ "String" ), ( Blank _, Blank _ ) ] ->
                                case m.cursorState of
        ...
        |> List.map
            (\tl ->
                case tl.data of
                    TLDB { name, cols } ->
                        case cols of
                            [ ( F id "field1", F _ "String" ), ( F _ "field2", F _ "Int" ), ( Blank _, Blank _ ) ] ->
                                case m.cursorState of
                                    Selecting _ (Just sid) ->
                                        if sid == id then
                                            pass
        ...
        |> List.map
            (\tl ->
                case tl.data of
                    ...
                    _ ->
                        fail ( "Shouldn't be other handlers here", tl.data )
            )
        |> RE.combine
        |> Result.map (\_ -> ())
...
feature_flag_works m =
    ...
    in
    case ast of
        F _ (Let (F _ "a") (F _ (Value "13")) (F _ (FeatureFlag (F _ "myflag") (F _ (FnCall "Int::greaterThan" [ F _ (Variable "a"), F _ (Value "10") ] _)) (F _ (Value "\"A\"")) (F _ (Value "\"B\""))))) ->
            pass

        _ ->
            fail ( ast, m.cursorState )
...
variable_extraction m =
    ...
    in
    case ast of
        F _ (Let (F _ "foo") (F _ (Value "1")) (F _ (Let (F _ "bar") (F _ (Value "2")) (F _ (Let (F _ "new_variable") (F _ (FnCall "+" [ F _ (Variable "foo"), F _ (Variable "bar") ] _)) (F _ (Let (F _ "baz") (F _ (Value "5")) (F _ (Variable "new_variable"))))))))) ->
            pass
...
editing_stays_in_same_place_with_enter m =
    case ( m.cursorState, onlyExpr m ) of
        ( Selecting _ id1, Let (F id2 "v2") _ _ ) ->
            if id1 == Just id2 then
                pass

            else
                fail ( m.cursorState, onlyExpr m )
...
editing_starts_a_thread_with_shift_enter m =
    case ( m.cursorState, onlyExpr m ) of
        ( Entering (Filling _ id1), Let _ (F _ (Thread [ F _ (Value "52"), Blank id2 ])) _ ) ->
            if id1 == id2 then
                pass
...
object_literals_work m =
    case ( m.cursorState, onlyExpr m ) of
        ( Entering (Filling tlid id), ObjectLiteral [ ( F _ "k1", Blank _ ), ( F _ "k2", F _ (Value "2") ), ( F _ "k3", F _ (Value "3") ), ( F _ "k4", Blank _ ), ( Blank _, Blank _ ) ] ) ->
            let
                tl =
                    TL.getTL m tlid
