...
extractVariable m tl p =
    ...
    case ( p, tl.data ) of
        ( PExpr e, TLHandler h ) ->
            ...
            Many
                [ RPC
                    ( [ SetHandler tl.id tl.pos newHandler ]
                    , FocusNoChange
                    )
...
extractFunction m tl p =
    if not (TL.isValidID tl (P.toID p)) then
        NoChange
...
renameFunction m old new =
    let
        renameFnCalls ast old new =
            let
                transformCall newName old =
                    let
                        transformExpr name old =
                            case old of
            ...
            m.userFunctions
                |> List.filterMap
                    (\uf ->
                        let
                            newAst =
                                renameFnCalls uf.ast old new
                        in
                        if newAst /= uf.ast then
            ...
            case nExpr of
                FnCall name list _ ->
                    if name == fnName then
                        ...
                        List.any (isFunctionInExpr fnName) list
                    ...
                    (\tl ->
                        case TL.asHandler tl of
                            ...
                            Just h ->
                                let
                                    newAst =
                                        transformCallsInAst f h.ast uf
                                in
                                if newAst /= h.ast then
                                    Just (SetHandler tl.id tl.pos { h | ast = newAst })
            ...
            m.userFunctions
                |> List.filterMap
                    (\uf ->
                        let
                            newAst =
                                transformCallsInAst f uf.ast uf
                        in
                        if newAst /= uf.ast then
                            Just (SetFunction { uf | ast = newAst })
