module AST exposing
    ( addLambdaBlank
    , addListLiteralBlanks
    , addObjectLiteralBlanks
    , addThreadBlank
    , allCallsToFn
    , allData
    , ancestors
    ...
    , isThreadBlank
    , listThreadBlanks
    , maybeExtendListLiteralAt
    , maybeExtendObjectLiteralAt
    , maybeExtendThreadAt
    , parentOf
    , parentOf_
    , replace
        ...
        rn nexpr =
            case nexpr of
                ...
                Thread exprs ->
                    let
                        ( blanks, filled ) =
                            List.partition B.isBlank exprs

                        blankids =
                            List.map B.toID blanks
    ...
    case expr of
        F id (ObjectLiteral pairs) ->
            pairs
                |> List.filterMap
                    (\( k, v ) ->
                        if B.isBlank k && B.isBlank v then
                            Nothing

                        else
                            Just ( k, closeObjectLiterals v )
                    )
                |> (\l ->
                        if l /= [] then
                            l
...
closeListLiterals expr =
    case expr of
        F id (ListLiteral exprs) ->
            let
                exprs2 =
                    List.map closeListLiterals exprs

                exprs3 =
                    List.filter B.isF exprs2
            in
    ...
    if id == B.toID expr then
        ...
        case expr of
            F tid (Thread exprs) ->
                let
                    replaced =
                        extendThreadChild id exprs
                in
                if replaced == exprs then
                    traverse atb expr

                else
                    F tid (Thread replaced)
            ...
            case parentOf id expr of
                (F olid (ObjectLiteral pairs)) as old ->
                    let
                        ...
                        newPairs =
                            pairs ++ [ ( newKey, newExpr ) ]

                        new =
                            F olid (ObjectLiteral newPairs)
...
maybeExtendThreadAt id expr =
    case expr of
        ...
        _ ->
            traverse (maybeExtendThreadAt id) expr
...
uses var expr =
    let
        is_rebinding newbind =
            case newbind of
                Blank _ ->
                    False

                F _ potential ->
                    if potential == var then
                        True
    ...
    case expr of
        ...
        F _ nexpr ->
            case nexpr of
                ...
                Lambda vars lexpr ->
                    if List.any is_rebinding vars then
                        []
            ...
            (\pd ->
                case pd of
                    PExpr (F id (FnCall name params r)) ->
                        if name == s then
                            Just (F id (FnCall name params r))
    ...
    let
        ...
        poList xs =
            xs
                ...
                |> List.head
    in
    if List.member eid (children expr |> List.map P.toID) then
        Just expr
...
replace_ search replacement parent expr =
    let
        r =
            replace_ search replacement (Just expr)
    ...
    if B.toID expr == sId then
                            ...
                            Just (F _ (Thread (first :: _))) ->
                                case e of
                                    F id (FnCall fn ((_ :: rest) as args) r) ->
                                        if B.toID first == sId then
                                            F id (FnCall fn args r)

                                        else
                                            F id (FnCall fn rest r)
        ...
        case ( expr, replacement ) of
            ( F id (FeatureFlag msg cond a b), PFFMsg newMsg ) ->
                if B.toID msg == sId then
                    F id (FeatureFlag newMsg cond a b)
            ...
            ( F id (FieldAccess obj field), PField replacement ) ->
                if B.toID field == sId then
                    F id (FieldAccess obj (B.replace sId replacement field))
    ...
    let
        ...
        cNExpr nexpr =
            case nexpr of
                ...
                Lambda vars body ->
                    Lambda (List.map cString vars) (c body)
                    ...
                    PExpr boe ->
                        case boe of
                            ...
                            F id e ->
                                case e of
                                    Variable name ->
                                        if Set.member (deID id) definedAndUsed then
                                            Nothing
