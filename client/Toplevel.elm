...
moveTL xOffset yOffset tl =
    let
        newPos =
            { x = tl.pos.x + xOffset, y = tl.pos.y + yOffset }
    in
    { tl | pos = newPos }
    ...
    case pred of
        Just pred ->
            let
                ...
                index =
                    LE.elemIndex pred ps |> Maybe.withDefault -1

                remaining =
                    List.drop (index + 1) ps

                blanks =
                    List.filter P.isBlank remaining
            in
            blanks
                |> List.head
    ...
    case next of
        Just next ->
            let
                ...
                index =
                    LE.elemIndex next ps |> Maybe.withDefault (List.length ps)
...
siblings tl p =
    case tl.data of
        TLHandler h ->
            ...
            in
            if List.member p toplevels then
                toplevels
...
getChildrenOf tl pd =
    let
        ...
        specChildren () =
            let
                h =
                    asHandler tl |> deMaybe "getChildrenOf - spec"
            in
...
replace p replacement tl =
    let
        ha () =
            tl |> asHandler |> deMaybe "TL.replace"

        fn () =
            tl |> asUserFunction |> deMaybe "TL.replace"
        ...
        astReplace () =
            case tl.data of
                TLHandler h ->
                    ...
                    in
                    { tl | data = TLHandler { h | ast = newAST } }

                TLFunc f ->
                    let
                        newAST =
                            AST.replace p replacement f.ast
                    in
                    { tl | data = TLFunc { f | ast = newAST } }
        ...
        specTypeReplace () =
            let
                ...
                newSpec =
                    SpecTypes.replace p replacement h.spec
            in
            { tl | data = TLHandler { h | spec = newSpec } }
        ...
        specHeaderReplace bo =
            let
                ...
                newSpec =
                    SpecHeaders.replace id bo h.spec
            in
            { tl | data = TLHandler { h | spec = newSpec } }
        ...
        PFFMsg bo ->
            let
                ...
                ast =
                    AST.replace p replacement h.ast
            in
            { tl | data = TLHandler { h | spec = spec2, ast = ast } }
