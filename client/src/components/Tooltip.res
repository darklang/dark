...
module InfoIcon = {
  ...
  let generic = (~style="", body: Html.html<'msg>): Html.html<'msg> =>
    Icons.fontAwesome(~style=%twc("text-sm pl-1 text-grey1"), "info-circle")->Tooltip.add(
      body,
      ~style,
    )
...
let tooltip = (
  ...
  let infoText: Html.html<'msg> = InfoIcon.generic(~style, info)
  let error: Html.html<'msg> =
    error
    ->Tc.Option.map(~f=SettingsViewComponents.errorSpan)
    ->Tc.Option.unwrap(~default=Html.noNode)
  Html.div(
    list{},
    list{
      Html.div(
