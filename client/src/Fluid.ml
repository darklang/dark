        ...
        let newPatternAndExprs =
          List.map patternsAndExprs ~f:(fun (pattern, expr) ->
              let toksToPattern tokens pID =
                ...
                | [ (id, value, "pattern-true") ]
                | [ (id, value, "pattern-false") ] ->
...
let pasteOverSelection ~state ~ast data : ast * fluidState =
  ...
  | ( Some (EVariable (_, varName))
    ...
    , Some { startPos } ) ->
      ...
      in
      ( newAST
      , { state with newPos = collapsedSelStart + String.length varName } )
    ...
    List.indexedMap
      ~f:(fun i item ->
        ...
        let versionView =
          ...
          then
            Html.span [ Html.class' "version" ] [ Html.text versionDisplayName ]
          else Vdom.noNode
        in
        Html.li
          [ Attrs.classList
          ...
          ; Html.span
              [ Html.class' "types" ]
              [ Html.text <| AC.asTypeString item ]
          ])
      acis
  in
    ...
    |> Option.map ~f:(fun ti ->
           ...
           then
             ...
             | None, LoadableSuccess dval ->
                 ...
                 ( [ Html.text text; viewCopyButton tlid text ]
                 , true
                 , ti.startRow )
             | Some _, _ ->
                 none
