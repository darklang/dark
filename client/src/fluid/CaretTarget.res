...
@ocaml.doc(" [forPPStringOpenQuote id offset] produces an ARMPattern PPString caretTarget
* pointing to an [offset] into the open quote of the pattern string with [id].
* [offset] may NOT be negative as it cannot represent something out of string bounds. ")
let forPPStringOpenQuote = (id: id, offset: int): t => {
  astRef: ARMPattern(id, PPString),
  offset: offset,
}

@ocaml.doc(" [forPPStringText id offset] produces an ARMPattern PPString caretTarget
...
@ocaml.doc(" [forARStringCloseQuote id offset] produces an ARMPattern PPString caretTarget
