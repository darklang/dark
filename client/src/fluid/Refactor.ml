...
let createNewDB (m : model) (maybeName : dbName option) (pos : pos) :
  ...
  then Model.updateErrorMod (Error.set ("There is already a DB named " ^ name))
    ...
    let tlid = gtlid () in
    let pgid = gid () in
    (* When db name is provided by user already (like from the quick menu) 
     * then we are focusing on the first field. If the name is missing
     * then we allowing user to jump out (the autogenerated name will be
     * used) or user can just type the name, overwriting the current one.
     *)
    let focuson = match maybeName with Some _ -> next | None -> pgid in
    let pageChanges = [SetPage (FocusedDB (tlid, true))] in
    let rpcCalls =
      [ CreateDBWithBlankOr (tlid, pos, pgid, name)
      ; AddDBCol (tlid, next, Prelude.gid ()) ]
    in
    Many
      ( AppendUnlockedDBs (StrSet.fromList [TLID.toString tlid])
      :: AddOps (rpcCalls, FocusExact (tlid, focuson))
      :: pageChanges )
