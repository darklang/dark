...
(** toPrettySexp is an S-exp like formatting that contains newlines and
 * truncated strings, which is meant to be human readable-ish. *)
let toPrettySexp (expr : E.t) : string =
  let rec toPrettySexp ~(indent : int) (expr : E.t) : string =
    let iStr = String.repeat ~count:indent " " in
    let r = toPrettySexp ~indent in
    let rin = toPrettySexp ~indent:(indent + 2) in
