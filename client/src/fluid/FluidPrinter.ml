...
let rec toTokens' (collapsedExprIds : id list) (e : E.t) (b : Builder.t) :
    Builder.t =
  let fromExpr e b = toTokens' collapsedExprIds e b in
  let open Builder in
  ...
  in
  let isCollapsed id = List.member collapsedExprIds ~value:id in
  let nest
  ...
  | EIf (id, cond, if', else') ->
      ...
      then
        b
        |> add (TIfKeyword id)
        |> add (TCollapsed (E.toID cond))
        |> add (TSep id)
        |> add (TIfThenKeyword id)
        |> add (TSep id)
        |> add (TCollapsed (E.toID if'))
        |> add (TSep id)
        |> add (TIfElseKeyword id)
        |> add (TCollapsed (E.toID else'))
      else
        b
        |> add (TIfKeyword id)
  ...
  | EFnCall (id, fnName, args, ster) ->
      ...
      in
      if isCollapsed id
      then b |> add (TSep id) |> add (TCollapsed id)
      else b |> addArgs fnName id args
...
let toTokens (collapsedExprIds : id list) (e : E.t) : tokenInfo list =
  toTokens' collapsedExprIds e Builder.empty
  |> Builder.asTokens
  |> tidy
  |> validateTokens
  |> infoize
