...
(** A split is part of a token stream generated by a builder. The tokenInfo
 * list may represent the entire AST or it may be a subset of the AST if the
...
module Builder = struct
  ...
  (** [split ~f b] adds a sub-builder, used for spliting parts of the AST into
    * a separate token stream. [b] is returned for continued use, while [f] is
    * called with a new builder (the split). *)
...
let rec toTokens' (e : E.t) (b : Builder.t) : Builder.t =
  ...
  | EFeatureFlag (id, _name, cond, disabled, enabled) ->
       ...
       * split the token stream in two, with only the disabled case in the main
...
(** [tokenizeWithSplits e] returns a list of all splits generated by tokenizing the
  * expression [e]. Always returns at least one split, which should be used as
  * the "main" editor panel. If the builder was split into sub-builders, then
  * will return >1 split.
  *
...
(** [tokensForSplit ~index e] generates all splits for the given expression
  * [e] and then returns just the tokens for the split given by [index].
  *
...
let tokensForSplit (e : FluidExpression.t) ~(index : int) : tokenInfo list =
  ...
  |> recoverOpt
       ~default:[]
       ("could not find tokens for split index=" ^ Int.toString index)
