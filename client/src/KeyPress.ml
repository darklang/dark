...
let optionDefaultHandler (event : Keyboard.keyEvent) (m : model) : modification
    option =
...
let handler (event : Keyboard.keyEvent) (m : model) : modification =
  ...
  |> List.foldl
    (fun h (acc: modification option) -> (match acc with
           None -> (h event m)
         | Some _ -> acc))
    None
  |> (fun modification -> match modification with
        Some m -> m
      | None -> NoChange)
