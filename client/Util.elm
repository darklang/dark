module Util exposing
    ...
    , toIntWithDefault
    , transformFromStringEntry
    , transformToStringEntry
    , uniqueCombinations
    , windowSize
    , zip
    )
...
letter2int s =
    s |> String.uncons |> Maybe.withDefault ( '!', "" ) |> Tuple.first |> Char.toCode |> (-) (Char.toCode 'a') |> (*) -1
...
listContainsOrdered needle haystack =
    case ( needle, haystack ) of
        ...
        ( n :: ns, h :: hs ) ->
            if n == h then
                listContainsOrdered ns hs

            else
                listContainsOrdered (n :: ns) hs
