...
class TestReleaseImage(unittest.TestCase):
  ...
  def test_release_dockerfile_sdk_matches_global_json(self):
    ...
    dockerfile = os.path.join(root, "Dockerfile.release")
    globaljson = os.path.join(root, "backend", "global.json")
    if not os.path.exists(dockerfile):
      self.skipTest("no Dockerfile.release")
    ...
    self.assertIsNotNone(
      match, "Dockerfile.release should pin the SDK via ARG DOTNET_SDK_VERSION")
    self.assertEqual(
      match.group(1), pinned,
      "Dockerfile.release SDK version must match backend/global.json")
