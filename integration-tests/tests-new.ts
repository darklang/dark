...
import { apiserverBaseUrl, clearMessages, test } from "./fixture";

// TODO: Reconsider how we expose/consume these utility fns
// (this is a bit noisy)
import {
  acHighlightedValue,
  awaitAnalysis,
  ...
  expectContainsText,
  expectExactText,
  expectPlaceholderText,
  fluidACHighlightedValue,
  get,
...
// Note: `test` here is a custom Playwright fixture - see definition.
test.describe.parallel("Integration Tests", async () => {
  ...
  test("rename_db_fields", async ({ page }, testInfo) => {
    ...
    await page.waitForResponse(
      `${apiserverBaseUrl}/api/test-rename_db_fields/add_op`,
    );
  ...
  test("rename_db_type", async ({ page }, testInfo) => {
    ...
    await page.keyboard.press("Enter");
    await page.waitForResponse(
      `${apiserverBaseUrl}/api/test-rename_db_type/add_op`,
    );
  ...
  // TODO: re-enable this
  // This is disabled as our test fixture assumes we want the consent dialog
  // to not be presented. Need to figure out how to resolve.
  ...
  // disabled tests to follow up on

  // TODO: maybe these are better with Blazor and the new setup. try re-enabling them
  // TODO: standardize the formatting below; it's pretty mixed
...
*/
  ...
  //
  // Note: since this test has been commented out, this script has been updated such that
  // the same browser context is used across tests. So, re-enabling this test will require
  // some special care to handle the admin login required here.
  //
