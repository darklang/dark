/******/ (() => {
  // webpackBootstrap
  /******/ "use strict";
  var __webpack_exports__ = {}; // CONCATENATED MODULE: ../../../JSInterop/Microsoft.JSInterop.JS/src/dist/Microsoft.JSInterop.js
  ...
  })(DotNet || (DotNet = {})); // CONCATENATED MODULE: ./Rendering/RenderBatch/RenderBatch.ts
  ...
  (function (FrameType) {
    ...
    FrameType[(FrameType["elementReferenceCapture"] = 6)] =
      "elementReferenceCapture";
    FrameType[(FrameType["markup"] = 8)] = "markup";
  })(FrameType || (FrameType = {})); // CONCATENATED MODULE: ./Rendering/Events/EventFieldInfo.ts
  ...
  class EventDelegator {
    ...
    setListener(element, eventName, eventHandlerId, renderingComponentId) {
      ...
      if (existingHandler) {
        ...
        this.eventInfoStore.update(
          existingHandler.eventHandlerId,
          eventHandlerId,
        );
      } else {
    ...
    removeListener(eventHandlerId) {
      ...
      if (info) {
        ...
        const elementEventInfos = this.getEventHandlerInfosForElement(
          element,
          false,
        );
    ...
    onGlobalEvent(evt) {
      ...
      eventNameAliases &&
        eventNameAliases.forEach(alias =>
    ...
    dispatchGlobalEventToAllElements(eventName, browserEvent) {
      ...
      while (candidateEventTarget) {
        ...
        const handlerInfos = this.getEventHandlerInfosForElement(
          candidateElement,
          false,
        );
        if (handlerInfos) {
          ...
          if (
            handlerInfo &&
            !eventIsDisabledOnElement(candidateElement, browserEvent.type)
          ) {
            ...
            if (!eventArgsIsPopulated) {
              ...
              eventArgs = (
                eventOptionsIfRegistered === null ||
                eventOptionsIfRegistered === void 0
                  ? void 0
                  : eventOptionsIfRegistered.createEventArgs
              )
            ...
            dispatchEvent(
              this.browserRendererId,
              {
                ...
                eventName: eventName,
                eventFieldInfo: EventFieldInfo.fromEvent(
                  handlerInfo.renderingComponentId,
                  browserEvent,
                ),
              },
              eventArgs,
        ...
        candidateEventTarget =
          eventIsNonBubbling || stopPropagationWasRequested
            ? undefined
            : path.shift();
      }
    }
    getEventHandlerInfosForElement(element, createIfNeeded) {
      if (element.hasOwnProperty(this.eventsCollectionKey)) {
        ...
        return (element[
          this.eventsCollectionKey
        ] = new EventHandlerInfosForElement());
      } else {
        return null;
      }
    }
  ...
  class EventInfoStore {
    constructor(globalListener) {
      ...
      eventNameAliasRegisteredCallbacks.push(
        this.handleEventNameAliasAdded.bind(this),
      );
    }
    add(info) {
  ...
  class EventHandlerInfosForElement {
    ...
    preventDefault(eventName, setValue) {
      ...
      return this.preventDefaultFlags
        ? this.preventDefaultFlags[eventName]
        : false;
    }
    stopPropagation(eventName, setValue) {
      ...
      return this.stopPropagationFlags
        ? this.stopPropagationFlags[eventName]
        : false;
    }
  }
...
*/
  ...
  function toLogicalRootCommentElement(start, end) {
    ...
    const parentLogicalElement = toLogicalElement(
      parent,
  ...
  function permuteLogicalChildren(parent, permutationList) {
    ...
    permutationList.forEach(listEntry => {
      const marker = (listEntry.moveToBeforeMarker = document.createComment(
        "marker",
      ));
  ...
  const sharedSvgElemForParsing = document.createElementNS(
  ...
  class BrowserRenderer {
    ...
    updateComponent(batch, componentId, edits, referenceFrames) {
      ...
      const rootElementToClear =
        elementsToClearOnRootComponentRender[componentId];
      ...
      const ownerDocument =
        (_a = getClosestDomElement(element)) === null || _a === void 0
          ? void 0
          : _a.getRootNode();
    ...
    applyEdits(batch, componentId, parent, childIndex, edits, referenceFrames) {
      ...
      for (
        let editIndex = editsOffset;
        editIndex < maxEditIndexExcl;
        editIndex++
      ) {
        ...
        switch (editType) {
          case EditType.prependFrame: {
            ...
            const frame = batch.referenceFramesEntry(
              referenceFrames,
              frameIndex,
            );
          ...
          case EditType.updateText: {
            ...
            const siblingIndex = editReader.siblingIndex(edit);
            const textNode = getLogicalChild(
              parent,
              childIndexAtCurrentDepth + siblingIndex,
            );
          ...
          case EditType.permutationListEntry: {
            ...
            permutationList.push({
              fromSiblingIndex:
                childIndexAtCurrentDepth + editReader.siblingIndex(edit),
              toSiblingIndex:
                childIndexAtCurrentDepth + editReader.moveToSiblingIndex(edit),
            });
            break;
          }
      ...
      switch (frameType) {
        ...
        case FrameType.elementReferenceCapture:
          if (parent instanceof Element) {
            applyCaptureIdToElement(
              parent,
              frameReader.elementReferenceCaptureId(frame),
            );
    ...
    insertElement(
      ...
      let inserted = false;
      // Apply attributes
      const descendantsEndIndexExcl =
        frameIndex + frameReader.subtreeLength(frame);
      for (
        let descendantIndex = frameIndex + 1;
        descendantIndex < descendantsEndIndexExcl;
        descendantIndex++
      ) {
        const descendantFrame = batch.referenceFramesEntry(
          frames,
          descendantIndex,
        );
      ...
      if (newDomElementRaw instanceof HTMLOptionElement) {
        ...
        // Situation 2
        setDeferredElementValue(
          newDomElementRaw,
          newDomElementRaw[deferredValuePropname],
        );
      }
    }
    trySetSelectValueFromOptionElement(optionElement) {
      ...
      if (isMultipleSelectElement(selectElem)) {
        optionElement.selected =
          selectElem._blazorDeferredValue.indexOf(optionElement.value) !== -1;
      } else {
    ...
    insertComponent(batch, parent, childIndex, frame) {
      const containerElement = createAndInsertLogicalContainer(
        parent,
        childIndex,
      );
    ...
    insertMarkup(batch, parent, childIndex, markupFrame) {
      const markupContainer = createAndInsertLogicalContainer(
        parent,
        childIndex,
      );
    ...
    applyAttribute(batch, componentId, toDomElement, attributeFrame) {
      ...
      const eventHandlerId = frameReader.attributeEventHandlerId(
        attributeFrame,
      );
      if (eventHandlerId) {
        const eventName = stripOnPrefix(attributeName);
        this.eventDelegator.setListener(
          toDomElement,
          eventName,
          eventHandlerId,
          componentId,
    ...
    applyInternalAttribute(
      batch,
      element,
      internalAttributeName,
      attributeFrame,
    ) {
      const attributeValue = attributeFrame
        ? batch.frameReader.attributeValue(attributeFrame)
        : null;
      if (
        internalAttributeName.startsWith(
          eventStopPropagationAttributeNamePrefix,
        )
      ) {
        // Stop propagation
        const eventName = stripOnPrefix(
          internalAttributeName.substring(
            eventStopPropagationAttributeNamePrefix.length,
          ),
        );
        this.eventDelegator.setStopPropagation(
          element,
          eventName,
          attributeValue !== null,
        );
      } else if (
        internalAttributeName.startsWith(eventPreventDefaultAttributeNamePrefix)
      ) {
        // Prevent default
        const eventName = stripOnPrefix(
          internalAttributeName.substring(
            eventPreventDefaultAttributeNamePrefix.length,
          ),
        );
        this.eventDelegator.setPreventDefault(
          element,
          eventName,
          attributeValue !== null,
        );
        ...
        throw new Error(
          `Unsupported internal attribute '${internalAttributeName}'`,
        );
      }
    }
      ...
      switch (element.tagName) {
        ...
        case "TEXTAREA": {
          ...
          if (
            value &&
            element instanceof HTMLSelectElement &&
            isMultipleSelectElement(element)
          ) {
    ...
    insertFrameRange(
      ...
      for (let index = startIndex; index < endIndexExcl; index++) {
        ...
        const numChildrenInserted = this.insertFrame(
          batch,
          componentId,
          parent,
          childIndex,
  ...
  function stripOnPrefix(attributeName) {
    ...
    throw new Error(
      `Attribute should be an event name, but doesn't start with 'on'. Value: '${attributeName}'`,
    );
  }
  ...
  function setMultipleSelectElementValue(element, value) {
    ...
    for (let i = 0; i < element.options.length; i++) {
      element.options[i].selected =
        value.indexOf(element.options[i].value) !== -1;
    }
  }
  ...
  function attachRootComponentToLogicalElement(
    ...
    if (!browserRenderer) {
      browserRenderer = browserRenderers[
        browserRendererId
  ...
  function attachRootComponentToElement(
    elementSelector,
    componentId,
    browserRendererId,
  ) {
    ...
    const element =
      getAndRemovePendingRootComponentContainer(elementSelector) ||
      document.querySelector(elementSelector);
    if (!element) {
      throw new Error(
        `Could not find any element matching selector '${elementSelector}'.`,
      );
    }
  ...
  function init(dotNetHelper, spacerBefore, spacerAfter, rootMargin = 50) {
    ...
    function intersectionCallback(entries) {
      entries.forEach(entry => {
        ...
        const spacerSeparation = rangeBetweenSpacers.getBoundingClientRect()
          .height;
        const containerSize =
  ...
  class BootConfigResult {
    ...
    static async initAsync(loadBootResource, environment) {
      ...
      const bootConfigResponse =
        loaderResponse instanceof Promise
          ? await loaderResponse
          : await defaultLoadBlazorBootJson(
              loaderResponse !== null && loaderResponse !== void 0
                ? loaderResponse
                : "_framework/blazor.boot.json",
            );
  ...
  function attachInteropInvoker() {
    ...
    const dotNetDispatcherNotifyByteArrayAvailableMethodHandle = bindStaticMethod(
      "Microsoft.AspNetCore.Components.WebAssembly",
      "Microsoft.AspNetCore.Components.WebAssembly.Services.DefaultWebAssemblyJSRuntime",
  ...
  class WebAssemblyComponentAttacher {
    ...
    resolveRegisteredElement(id) {
      ...
      if (!Number.isNaN(parsedId)) {
        return toLogicalRootCommentElement(
          this.componentsById[parsedId].start,
          this.componentsById[parsedId].end,
        );
      } else {
        return undefined;
      }
  ...
  function discoverPersistedState(node) {
    ...
    if (node.nodeType === Node.COMMENT_NODE) {
      ...
      const value =
        parsedState && parsedState.groups && parsedState.groups["state"];
      if (value) {
  ...
  function resolveComponentComments(node, type) {
    ...
    while (childNodeIterator.next() && childNodeIterator.currentElement) {
      ...
      if (componentComment) {
        ...
        const childResults = resolveComponentComments(
          childNodeIterator.currentElement,
          type,
        );
  ...
  const blazorCommentRegularExpression = new RegExp(
    /^\s*Blazor:[^{]*(?<descriptor>.*)$/,
  );
  function getComponentComment(commentNodeIterator, type) {
    ...
    if (candidateStart.textContent) {
      const definition = blazorCommentRegularExpression.exec(
        candidateStart.textContent,
      );
      const json =
        definition && definition.groups && definition.groups["descriptor"];
      if (json) {
        try {
          const componentComment = parseCommentPayload(json);
          switch (type) {
            case "webassembly":
              return createWebAssemblyComponentComment(
                componentComment,
                candidateStart,
                commentNodeIterator,
              );
            case "server":
              return createServerComponentComment(
                componentComment,
                candidateStart,
                commentNodeIterator,
              );
          ...
          throw new Error(
            `Found malformed component comment at ${candidateStart.textContent}`,
          );
        }
      } else {
        return;
  ...
  function createServerComponentComment(payload, start, iterator) {
    ...
    if (!Number.isInteger(sequence)) {
      throw new Error(
        `Error parsing the sequence '${sequence}' for component '${JSON.stringify(
          payload,
        )}'`,
      );
    }
  ...
  function createWebAssemblyComponentComment(payload, start, iterator) {
    ...
    if (!prerenderId) {
      return {
        ...
        // blazor.boot.json
        parameterDefinitions:
          parameterDefinitions && atob(parameterDefinitions),
  ...
  function validateEndComponentPayload(json, prerenderedId) {
    ...
    if (prerenderedEndId !== prerenderedId) {
      throw new Error(
        `End of component comment prerendered property must match the start comment prerender id: '${prerenderedId}', '${prerenderedEndId}'`,
      );
    }
  }
  ...
  async function boot(options) {
    ...
    started = true;
    setDispatchEventMiddleware(
      (browserRendererId, eventHandlerId, continuation) => {
    ...
    Blazor._internal.applyHotReload = (id, metadataDelta, ilDeta) => {
      DotNet.invokeMethod(
        "Microsoft.AspNetCore.Components.WebAssembly",
        "ApplyHotReloadDelta",
        id,
    ...
    const componentAttacher = new WebAssemblyComponentAttacher(
    ...
    Blazor._internal.registeredComponents = {
      ...
      getAssembly: id =>
        BINDING.js_string_to_mono_string(componentAttacher.getAssembly(id)),
      getTypeName: id =>
        BINDING.js_string_to_mono_string(componentAttacher.getTypeName(id)),
      getParameterDefinitions: id =>
        BINDING.js_string_to_mono_string(
          componentAttacher.getParameterDefinitions(id) || "",
        ),
      getParameterValues: id =>
        BINDING.js_string_to_mono_string(
          componentAttacher.getParameterValues(id) || "",
        ),
    };
    Blazor._internal.getPersistedState = () =>
      BINDING.js_string_to_mono_string(discoverPersistedState(document) || "");
