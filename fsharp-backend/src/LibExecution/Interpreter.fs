...
let rec eval' (state : ExecutionState) (st : Symtable) (e : Expr) : DvalTask =
  ...
  let evalMatchNew patternId matchExpr (cases : List<Pattern * Expr>) =
    ...
    /// - new vars (name * value)
    /// - traces
    let rec checkPattern dv pattern : bool * List<string * Dval> * List<id * Dval> =
      match pattern with
      ...
      | PConstructor (id, name, args) ->
        ...
        | "Error", [ p ], DResult (Error v) ->
          ...
          if passes then
            ...
            false, newVars, (id, incomplete id) :: traceArgsIncomplete [ p ] @ traces
      ...
      for (pattern, rhsExpr) in cases do
        if hasMatched && isRealExecution then
          ...
          if not hasMatched && passes then
            ...
            state.tracing.traceDval false patternId matchVal
  ...
  uply {
    ...
    | EMatch (id, matchExpr, cases) ->
      let evalMatch = if state.test.useNewLogic then evalMatchNew else evalMatchOld
