...
let run () : (unit, Exception.captured) Result.t =
  ...
  Event_queue.with_transaction (fun transaction ->
      try
        ...
        | None ->
          ...
          Ok ()
        | Some event ->
          let _ = Canvas.load_for_event event in
          Ok ()
      with
      | e ->
...
let dequeue_and_evaluate_all () : string = ""
  (* (1* iterate all non-test canvases *1) *)
  (* let execution_id = Util.create_id () in *)
  (* Log.infO "queue_worker" *)
  (*   ~data:"Worker starting" *)
  (*   ~params:["execution_id", Log.dump execution_id]; *)
  ...
  (*     ~f:(fun endp -> *)
  (*         try *)
  (*           Some (endp, C.load_all endp []) (1* serialization can fail, attempt first *1) *)
  (*         with *)
  (*         | e -> *)
  (*           let bt = Exception.get_backtrace () in *)
