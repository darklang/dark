...
let rec expr_to_string ~(indent:int) (e: expr) : string =
  ...
  let rec nexpr_to_string ~indent (nexpr: nexpr) : string =
    ...
    | If (cond, ifbody, elsebody) ->
      "if " ^ es ~indent:(indent + 3) cond
      ^ nl ^ "then"
      ^ nli ^ esi ifbody
      ^ nl ^ "else "
      ^ nli ^ esi elsebody
    | FnCall (name, args) ->
      List.fold ~init:name args
        ~f:(fun old arg ->
            let argstr =
              let old_length =
    ...
    | Lambda (vars, body) ->
      ...
      ^ " -> "
      ^ nli ^ es ~indent:(indent + 3) body
      ^ " )"
    | Thread exprs ->
      List.map ~f:(es ~indent:(indent+3)) exprs |> String.concat ~sep:(nl ^ "|> ")
    | FieldAccess (obj, field) ->
      if needs_parens obj
      then "(" ^ es obj ^ ")." ^ bs field
      else es obj ^ "." ^ bs field
    | ListLiteral exprs ->
      List.fold ~init:"" exprs
        ~f:(fun old arg ->
           let argstr =
              let indent =
                (indent + 1
...
let to_string (tl: toplevel) : string =
  ...
  match tl.data with
  | Handler h ->
    bs h.spec.module_ ^ " " ^ bs h.spec.name ^ " " ^ bs h.spec.modifier
    ^ "\n"
